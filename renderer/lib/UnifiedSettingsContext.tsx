...
function loadTheme(): ThemeMode {
  ...
  // 마이그레이션: 기존 noa_theme_level (0=밤/다크, 1=낮/라이트, 2·3=라이트)
  const legacy = localStorage.getItem("noa_theme_level");
  if (legacy === "0") return "dark";
  if (legacy === "1" || legacy === "2" || legacy === "3") return "light";
  return "dark";
}
...
function syncToLegacyKeys(slots: APIKeySlot[]): void {
  ...
  try {
    ...
    }).catch((err) => { if (typeof console !== 'undefined') console.warn('[UnifiedSettings] async legacy sync failed:', err); });
  }
}
