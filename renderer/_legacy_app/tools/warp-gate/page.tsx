...
const btnPrimary = `${btnCls} !bg-linear-to- from-accent-blue/25 to-accent-blue/15 !border-accent-blue/40`;
const btnAlt = `${btnCls} !bg-linear-to- from-accent-amber/15 to-accent-amber/8 !border-accent-amber/35`;
    ...
    <section className={panelCls}>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className={`${cardCls} grid gap-3`}>
          ...
          <pre className={`min-h-[200px] p-4 rounded-2xl border text-sm leading-relaxed whitespace-pre-wrap overflow-auto font-mono text-sky-100 ${s.terminal.mode === "ALLOW" ? "border-green-400/35 shadow-[inset_0_0_0_1px_rgba(134,240,183,0.08)]" : s.terminal.mode === "DENY" ? "border-red-400/35 shadow-[inset_0_0_0_1px_rgba(255,141,125,0.08)]" : "border-amber-400/25"} bg-linear-to- from-[rgba(0,8,14,0.94)] to-[rgba(4,10,21,0.96)]`}>
      ...
      <div className="grid md:grid-cols-3 gap-4 mt-4">
        ...
        <div className={cardCls}>
          ...
          {selDoc ? (
            <div className={`rounded-2xl p-4 min-h-[300px] ${selDoc.tone === "dark" ? "bg-linear-to- from-[rgba(15,21,34,0.96)] to-[rgba(9,14,25,0.95)] text-sky-100" : "bg-linear-to- from-[rgba(234,232,221,0.95)] to-[rgba(216,210,190,0.92)] text-gray-900"}`}>
...
function ZoneMapPanel({ s, metrics, en, onSelectZone }: { s: GameState; metrics: Metrics; en: boolean; onSelectZone: (id: string) => void }) {
  ...
  return (
    <section className={panelCls}>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="relative min-h-[320px] rounded-2xl border border-white/[0.06] bg-linear-to- from-[rgba(0,8,14,0.6)] to-[rgba(4,10,21,0.8)] overflow-hidden">
          {ZONES.map(z => {
        ...
        <div className={cardCls}><h3 className="font-bold text-sm text-text-primary mb-3">{en ? "Latest Verdict" : "최신 판정 결과"}</h3>
          {res ? (
            <div>
              ...
              {s.finale.unlocked && (
                <div className="mt-4 rounded-2xl p-4 bg-linear-to- from-accent-blue/15 to-transparent border border-accent-blue/25">
