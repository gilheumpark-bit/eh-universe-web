            ...
            .map(slot => (
              <div key={slot.id} className="flex items-center gap-2 px-2 py-2 bg-bg-secondary/50 border border-border rounded-lg group hover:border-accent-purple/30 transition-colors">
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-bold text-white truncate wrap-break-word">{slot.name}</div>
        ...
        {(currentSession.config.savedSlots || []).filter(s => s.tab !== activeTab).length > 0 && (
          <Accordion
            items={[
              {
                id: 'other_tabs',
                title: `${t('saveSlot.otherTabs')} (${(currentSession.config.savedSlots || []).filter(s => s.tab !== activeTab).length})`,
                children: (
...
export function StudioWritingAssistantPanel({
  ...
  // Custom resizing logic similar to RightChatPanel
  const [panelWidth, setPanelWidth] = useState(() => {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem('eh_studio_panel_width');
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (isResizing.current) {
        ...
        localStorage.setItem('eh_studio_panel_width', panelWidth.toString());
      }
    };
  ...
  return (
    <aside 
      className={`relative hidden lg:flex shrink-0 flex-col border-l border-border bg-bg-primary transition-all duration-300`}
      style={{ width: rightPanelOpen ? `${panelWidth}px` : '40px' }}
    >
      {/* Resizer Handle */}
      {rightPanelOpen && (
        <div 
          onMouseDown={handleMouseDown}
          className="absolute left-[-4px] top-0 bottom-0 w-3 cursor-col-resize z-50 group transition-colors"
        >
          <div className="w-px h-full mx-auto group-hover:bg-accent-purple/50 active:bg-accent-purple transition-colors" />
        </div>
      )}

      {/* Toggle button */}
      <button onClick={() => setRightPanelOpen(p => !p)} className="shrink-0 w-full py-2 text-[10px] text-text-tertiary hover:text-text-primary transition-colors border-b border-border font-mono">
        {rightPanelOpen ? '\u25B6' : '\u25C0'}
      </button>
          ...
          <div className="p-4 space-y-3 border-b border-border min-w-0">
            ...
            <Accordion
              ...
              items={[
                {
                  id: 'bridge',
                  title: <span className="flex items-center gap-1.5 text-xs font-bold text-text-tertiary">{'\uD83D\uDCCE'} {t('panel.bridge')}</span>,
                  children: (() => {
                    ...
                    return <p className="text-[11px] text-text-tertiary italic leading-relaxed wrap-break-word overflow-hidden">{txt ? txt.slice(-250) : t('panel.none')}</p>;
                  })()
                },
                {
                  id: 'scene-sheet',
                  title: (
                    <span className={`flex items-center gap-1.5 text-xs font-bold transition-colors ${
                      currentSession.config.sceneDirection ? 'text-text-tertiary' : 'text-amber-400'
                    }`}>
                      {'\uD83C\uDFAC'} {t('panel.scene')} {!currentSession.config.sceneDirection && <span className="text-[11px] ml-1 px-1.5 py-0.5 bg-amber-500/10 rounded text-amber-400">{t('panel.notSet')}</span>}
                    </span>
                  ),
                  defaultOpen: !currentSession.config.sceneDirection,
                  children: (
                    <div className="space-y-1 min-w-0">
                      {currentSession.config.sceneDirection?.hooks?.map((h, i) => <div key={i} className="text-[10px] text-blue-400 wrap-break-word">{'\uD83E\uDE9D'} {h.desc}</div>)}
                      {currentSession.config.sceneDirection?.goguma?.map((g, i) => <div key={i} className={`text-[10px] wrap-break-word ${g.type === 'goguma' ? 'text-amber-400' : 'text-cyan-400'}`}>{g.type === 'goguma' ? '\uD83C\uDF60' : '\uD83E\uDD64'} {g.desc}</div>)}
                      {currentSession.config.sceneDirection?.cliffhanger && <div className="text-[10px] text-red-400 wrap-break-word">{'\uD83D\uDD1A'} {currentSession.config.sceneDirection.cliffhanger.desc}</div>}
                      {!currentSession.config.sceneDirection && (
                        <div className="shrink-0 p-3 bg-bg-secondary rounded-2xl border border-amber-500/20">
                ...
                {
                  id: 'episode-scenes',
                  title: <span className="flex items-center gap-1.5 text-xs font-bold text-text-tertiary">{'\uD83D\uDCCB'} {t('panel.episodeScenes')} ({(currentSession.config.episodeSceneSheets ?? []).length})</span>,
                  children: (
                ...
                {
                  id: 'characters',
                  title: <span className="flex items-center gap-1.5 text-xs font-bold text-text-tertiary">{'\uD83D\uDC64'} {t('panel.chars')} ({currentSession.config.characters.length})</span>,
                  children: (
                    <div className="space-y-1.5 min-w-0">
                      {currentSession.config.characters.length > 0 ? currentSession.config.characters.map(c => (
                        <div key={c.id} className="text-[10px] wrap-break-word">
                ...
                {
                  id: 'format',
                  title: <span className="flex items-center gap-1.5 text-xs font-bold text-text-tertiary">{'\uD83D\uDCD0'} {t('panel.format')}</span>,
                  children: (
