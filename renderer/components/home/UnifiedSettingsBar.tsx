...
import { Clock, Globe, Key, User, LogOut, HardDrive } from "lucide-react";
...
const SettingsClock = () => {
  ...
  if (!time) return <div className="w-[68px] h-8 rounded-full border border-border/50 bg-bg-secondary/60 animate-pulse" />;

  return (
    <div className="inline-flex items-center gap-1.5 px-3 py-2 rounded-full border border-border/50 bg-bg-secondary/60 backdrop-blur-sm text-text-secondary text-xs cursor-default">
      <Clock className="w-3.5 h-3.5 text-accent-purple" />
      <span className="font-mono font-medium tracking-tight mb-px">{time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
    </div>
  );
};
...
export default function UnifiedSettingsBar() {
  const { theme: _theme, toggleTheme: _toggleTheme, enabledSlots } = useUnifiedSettings();
  ...
  return (
    <>
      <div className="flex items-center gap-2 flex-wrap justify-center">
        ...
        {/* Clock Widget (Replaces Theme Toggle) */}
        <SettingsClock />
