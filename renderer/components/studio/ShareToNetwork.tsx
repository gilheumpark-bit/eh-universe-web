...
import EmbedCodePanel from '@/components/studio/EmbedCodePanel';
import {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-bg-primary border border-border rounded-2xl p-6 space-y-5 animate-in zoom-in-95 duration-200" role="dialog" aria-modal="true">
        ...
        <details className="group border border-border rounded-xl overflow-hidden">
          <summary className="flex items-center gap-2 px-4 py-3 cursor-pointer hover:bg-bg-secondary/50 transition-colors">
            <span className="text-[10px] font-bold uppercase tracking-wider text-text-tertiary group-open:text-accent-amber transition-colors">
          ...
          <div className="px-4 pb-4 border-t border-border pt-3">
            <EmbedCodePanel
              type="world-doc"
              id={config.title?.replace(/\s+/g, '-').toLowerCase() || 'untitled'}
