  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-[10px] font-black text-accent-amber uppercase tracking-widest font-mono drop-shadow-sm flex items-center gap-2">
          <MapIcon className="w-3 h-3 text-accent-amber" />
        ...
        <div className="flex gap-2">
          ...
          <button onClick={addTerritory}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all active:scale-95 hover:scale-[1.02] ${
              linkMode 
                ? 'bg-accent-red text-white border border-accent-red/50 shadow-[0_5px_15px_rgba(168,92,82,0.2)] hover:shadow-[0_0_20px_rgba(168,92,82,0.4)]' 
                : 'bg-accent-amber text-white border border-accent-amber/50 shadow-[0_5px_15px_rgba(184,149,92,0.2)] hover:shadow-[0_0_20px_rgba(184,149,92,0.4)]'
            }`}
          >
      ...
      {linkMode && (
        <div className="flex flex-wrap gap-1 items-center bg-accent-amber/5 border border-accent-amber/15 rounded-xl p-2 backdrop-blur-sm">
          <span className="text-[9px] text-accent-amber/80 font-mono px-1">{isKO ? '연결 유형:' : 'Link type:'}</span>
          {(Object.keys(LINK_COLORS) as TerritoryLink['type'][]).map(type => (
            <button key={type} onClick={() => setLinkType(type)}
              className={`px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${
                linkType === type ? 'text-white border-transparent' : 'text-accent-amber/70 border-accent-amber/30 hover:text-accent-amber hover:bg-accent-amber/10'
              }`}
          ...
          {linkFrom && (
            <span className="text-[9px] text-accent-amber ml-2">
      ...
      {/* SVG Map Canvas */}
      <svg ref={svgRef} viewBox={`0 0 ${MAP_W} ${MAP_H}`} className="w-full border border-bg-border rounded-xl shadow-inner bg-bg-secondary"
        style={{ touchAction: 'none', fontFamily: 'var(--font-mono, monospace)' }}
      ...
      >
        ...
        {territories.length === 0 && (
          <g>
            <rect x={MAP_W / 2 - 24} y={MAP_H / 2 - 50} width="48" height="48" rx="24" className="fill-accent-amber/5 stroke-accent-amber/20" strokeWidth="1" />
            <text x={MAP_W / 2} y={MAP_H / 2 - 20} className="fill-accent-amber/60 text-[22px]" textAnchor="middle" dominantBaseline="middle">&#x1F5FA;</text>
            <text x={MAP_W / 2} y={MAP_H / 2 + 16} className="fill-accent-amber/80 text-[10px] font-bold tracking-[0.15em]" textAnchor="middle">
              {isKO ? '지도 없음' : 'NO REGIONS'}
            </text>
            <text x={MAP_W / 2} y={MAP_H / 2 + 34} className="fill-accent-amber/50 text-[9px] font-mono" textAnchor="middle">
      ...
      {territories.length > 0 && (
        <div className="space-y-1">
          {territories.map(t => (
            <div key={t.id} className="flex items-center gap-2 bg-bg-secondary/50 border border-bg-border rounded-lg px-3 py-1.5 backdrop-blur-sm transition-all hover:bg-bg-secondary hover:border-bg-border-strong">
              <span className="w-2.5 h-2.5 rounded-full shrink-0 shadow-sm" style={{ background: t.color || civColors[t.civName] || '#6b7280' }} />
              <input
                value={t.name}
                onChange={e => updateTerritory(t.id, { name: e.target.value })}
                className="flex-1 bg-transparent text-[10px] font-bold text-text-primary outline-none placeholder:text-text-tertiary"
                maxLength={30}
              />
              <select
                value={t.civName}
                onChange={e => updateTerritory(t.id, { civName: e.target.value, color: civColors[e.target.value] })}
                className="bg-bg-tertiary border border-bg-border rounded px-2 py-0.5 text-[9px] text-accent-amber font-mono outline-none focus:border-accent-amber"
              >
              ...
              <button onClick={() => removeTerritory(t.id)} className="text-text-tertiary hover:text-accent-red transition-all">
      ...
      {/* Link legend */}
      <div className="flex flex-wrap gap-3 text-[9px] bg-bg-secondary/30 border border-bg-border p-2 rounded-lg backdrop-blur-sm mt-3">
        {(Object.entries(LINK_COLORS) as [TerritoryLink['type'], string][]).map(([type, color]) => (
          <span key={type} className="flex items-center gap-1.5">
            ...
            <span className="text-text-secondary font-mono capitalize">{type}</span>
          </span>
        ))}
      </div>
    </div>
