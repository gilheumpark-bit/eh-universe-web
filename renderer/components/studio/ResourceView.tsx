...
const ResourceView: React.FC<ResourceViewProps> = ({ language, config, setConfig, onError: _onError }) => {
  ...
  return (
    <div className="max-w-[1400px] mx-auto w-full p-4 md:p-10 space-y-8 lg:space-y-12 animate-in fade-in duration-500">
      ...
      {/* Header Section */}
      <div className="flex items-center justify-between bg-bg-secondary/30 border border-border backdrop-blur-sm p-4 rounded-3xl shadow-sm">
        <div className="flex items-center gap-4 md:gap-6 w-full">
          <div className="p-4 md:p-5 bg-bg-tertiary/50 border border-border rounded-2xl md:rounded-3xl shrink-0">
            <Fingerprint className="w-6 h-6 md:w-8 md:h-8 text-yellow-400" />
          </div>
          <div className="min-w-0 flex-1">
            <h2 className="text-2xl md:text-3xl font-black tracking-tighter uppercase truncate text-text-primary">{t.title}</h2>
            <p className="text-yellow-400/80 text-[10px] md:text-[10px] font-bold tracking-[0.2em] md:tracking-[0.4em] uppercase truncate">{t.subtitle}</p>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <select
                    className="w-full bg-bg-tertiary/50 border border-border rounded-xl pl-11 pr-4 py-4 text-xs font-bold focus:border-blue-500 outline-none appearance-none cursor-pointer"
                    value={newChar.role}
              ...
              <button 
                ...
                className="w-full py-4 bg-white text-black! rounded-xl font-black text-[12px] uppercase tracking-widest hover:bg-blue-600 hover:text-white! transition-all shadow-xl active:scale-95"
              >
                {t.register}
              </button>
            </div>
            ...
            {filteredCharacters.length === 0 ? (
              ...
              <div className={`grid gap-4 md:gap-6 transition-all duration-500 ${
                ...
                {paginatedCharacters.map(char => (
                  <div key={char.id} className="relative group overflow-hidden bg-[#0f0f12] backdrop-blur-xl border border-white/10 hover:border-white/30 md:rounded-[2.5rem] rounded-2xl p-6 transition-all shadow-xl">
                    {/* Grid overlay for vital sign / holographic feel */}
                    <div className="absolute inset-0 pointer-events-none bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-size-[20px_20px] opacity-30"></div>
                    <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(ellipse_at_top_right,rgba(255,255,255,0.05),transparent_50%)]"></div>

                    {/* Visual DNA Bar - Vital Sign ECG effect */}
                    <div className="absolute top-0 left-0 h-1 bg-[linear-gradient(90deg,#3b82f6,#10b981)] shadow-[0_0_10px_#3b82f6] opacity-40 group-hover:opacity-100 transition-opacity" style={{ width: `${char.dna}%` }}></div>
                    
                    <div className="flex justify-between items-start mb-6 relative z-10">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-white font-black border border-white/10 text-xl shadow-inner group-hover:scale-110 transition-transform duration-500">
                          {char.name[0]}
                        </div>
                        <div className="min-w-0">
                          <div className="text-base font-black text-white truncate mb-0.5 group-hover:text-blue-200 transition-colors drop-shadow-sm">{char.name}</div>
                          <div className="flex items-center gap-1">
                             <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                               char.role === 'hero' ? 'bg-blue-500 shadow-[0_0_5px_rgba(59,130,246,0.8)]' :
                    ...
                    <div className="bg-[linear-gradient(to_right,rgba(255,255,255,0.05),transparent)] border-l-2 border-white/20 rounded-r-xl p-4 mb-4 relative group/traits z-10">
                      <ScrollText className="absolute top-4 right-4 w-3.5 h-3.5 text-white/30" />
                      <p className="text-[11px] text-white/80 font-serif leading-relaxed italic line-clamp-4 min-h-16">
                        {char.traits}
                      </p>
                    </div>
                    ...
                    <div className="space-y-2 mb-4">
                      <input
                        ...
                        maxLength={200}
                        className="w-full bg-bg-tertiary/30 border border-border/50 rounded-xl px-3 py-2 text-[10px] outline-none focus:border-blue-500 transition-colors placeholder:text-text-tertiary"
                      />
                      <input
                      ...
                      <input
                        ...
                        maxLength={300}
                        className="w-full bg-bg-tertiary/30 border border-border/50 rounded-xl px-3 py-2 text-[10px] outline-none focus:border-blue-500 transition-colors placeholder:text-text-tertiary font-serif italic"
                      />
                    </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                          <div className="space-y-1">
                            ...
                            <select
                              ...
                              className="w-full bg-bg-tertiary border border-border rounded-lg px-2 py-1.5 text-[10px] outline-none cursor-pointer"
                            >
                          ...
                          <div className="space-y-1">
                            ...
                            <input
                              ...
                              maxLength={100}
                              className="w-full bg-bg-tertiary border border-border rounded-lg px-2 py-1.5 text-[10px] outline-none focus:border-cyan-500 transition-colors placeholder:text-text-tertiary"
                            />
                          </div>
                          <div className="space-y-1">
  ...
  return (
    <div className="bg-bg-secondary/20 border border-white/5 rounded-3xl md:rounded-[2.5rem] p-6 md:p-8 space-y-6">
      ...
      <div className="flex flex-wrap gap-2 items-end">
        <select value={selFrom} onChange={e => setSelFrom(e.target.value)} className="bg-bg-tertiary border border-border rounded-xl px-3 py-2 text-xs outline-none">
          <option value="">{tl('resourceExtra.characterA')}</option>
          {chars.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        <select value={selTo} onChange={e => setSelTo(e.target.value)} className="bg-bg-tertiary border border-border rounded-xl px-3 py-2 text-xs outline-none">
        ...
        <input value={relDesc} onChange={e => setRelDesc(e.target.value)} placeholder={tl('resourceExtra.description')}
          maxLength={200}
          className="flex-1 min-w-[120px] bg-bg-tertiary border border-border rounded-xl px-3 py-2 text-xs outline-none" />
      ...
      {relations.length > 0 && (
        <div className="space-y-1.5">
          {relations.map((rel, i) => {
            ...
            return (
              <div key={i} className="flex items-center justify-between bg-bg-tertiary/30 border border-border/50 rounded-xl px-4 py-2 text-[10px]">
                <span>
