          ...
          <div className="flex-1 overflow-y-auto px-4 py-3">
            ...
            <nav className="space-y-1.5" role="tablist" aria-label="Studio navigation">
              ...
              {([
                ...
                { tab: 'manuscript' as AppTab, icon: FileText, label: t('ui.manuscript'), guided: true, color: 'purple', primary: true },
                ...
                .map(({ tab, icon: Icon, label, color, primary }, idx, arr) => {
                  ...
                  const colorClasses = {
                    amber: {
                      active: 'border-accent-amber/30 bg-linear-to- from-accent-amber/15 to-accent-amber/5 shadow-lg shadow-accent-amber/10',
                      icon: 'border-accent-amber/25 bg-accent-amber/15 text-accent-amber',
                      text: 'text-accent-amber',
                      glow: 'from-accent-amber/10',
                      indicator: 'bg-accent-amber shadow-md shadow-accent-amber/40',
                    },
                    purple: {
                      active: 'border-accent-purple/30 bg-linear-to- from-accent-purple/15 to-accent-purple/5 shadow-lg shadow-accent-purple/10',
                      icon: 'border-accent-purple/25 bg-accent-purple/15 text-accent-purple',
                      text: 'text-accent-purple',
                      glow: 'from-accent-purple/10',
                      indicator: 'bg-accent-purple shadow-md shadow-accent-purple/40',
                    },
                    blue: {
                      active: 'border-accent-blue/30 bg-linear-to- from-accent-blue/15 to-accent-blue/5 shadow-lg shadow-accent-blue/10',
                      icon: 'border-accent-blue/25 bg-accent-blue/15 text-accent-blue',
                      text: 'text-accent-blue',
                      glow: 'from-accent-blue/10',
                      indicator: 'bg-accent-blue shadow-md shadow-accent-blue/40',
                    },
                    green: {
                      active: 'border-accent-green/30 bg-linear-to- from-accent-green/15 to-accent-green/5 shadow-[0_0_20px_rgba(47,155,131,0.1)]',
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Hover glow effect */}
                      {!isActive && (
                        <span className={`absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none bg-linear-to- ${c.glow} to-transparent`} />
                      )}
                    </button>
                    </React.Fragment>
                  );
                    ...
                    {(orderedSessions.length > 50 ? orderedSessions.slice(0, 50) : orderedSessions).map((s, i) => (
                      <button
                        ...
                        className={`group relative flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left transition-all border overflow-hidden ${
                          batchMode && selectedSessionIds.has(s.id)
                            ? 'border-accent-purple/30 bg-accent-purple/15 text-accent-purple shadow-lg shadow-accent-purple/10'
                            : currentSessionId === s.id
                            ? 'border-[rgba(202,161,92,0.3)] bg-linear-to- from-[rgba(202,161,92,0.15)] to-[rgba(202,161,92,0.05)] text-[rgba(246,226,188,0.92)] shadow-[0_0_15px_rgba(202,161,92,0.1)] backdrop-blur-md'
                      ...
                      >
                        ...
                        {currentSessionId !== s.id && !batchMode && (
                          <span className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none bg-linear-to- from-[rgba(202,161,92,0.08)] to-transparent" />
                        )}
