...
const WorldStudioView: React.FC<WorldStudioViewProps> = ({
  ...
  const { getActiveProvider, getApiKey } = useAIProvider();
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-10 pt-6">
        <div className="relative flex flex-wrap items-center gap-2 p-2 bg-bg-secondary/80 backdrop-blur-xl border border-border rounded-2xl shadow-sm overflow-hidden" role="tablist">
          {/* Scanline overlay for holographic feel */}
          <div className="absolute inset-0 pointer-events-none bg-[linear-gradient(rgba(184,149,92,0.05)_50%,transparent_50%)] bg-size-[100%_4px] mix-blend-screen opacity-50"></div>
            ...
            return (
              <button
                ...
                className={`relative z-10 group flex items-center gap-2.5 px-5 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all duration-300 ${
                  active
                    ? "bg-accent-amber/15 text-accent-amber border border-accent-amber/40 shadow-sm -translate-y-px"
                    : "text-text-secondary hover:text-accent-amber hover:bg-accent-amber/5 border border-transparent hover:border-accent-amber/20"
                }`}
              >
                <Icon className={`w-4 h-4 transition-transform duration-300 ${active ? 'scale-110' : 'group-hover:scale-110'}`} />
                <span>{labels[tab]}</span>
                {active && (
                  <span className="absolute -bottom-px left-1/4 right-1/4 h-[2px] bg-accent-amber shadow-sm rounded-full"></span>
                )}
              </button>
            );
          })}
        ...
        <>
          ...
          {config.worldSimData?._latestUpdates && config.worldSimData._latestUpdates.length > 0 && (
            <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-10 pt-3">
              <div className="flex flex-col gap-2 p-4 bg-accent-amber/5 border-l-4 border-l-accent-amber border border-y-border border-r-border rounded-r-xl rounded-l-sm backdrop-blur-sm shadow-sm">
                <div className="flex items-center gap-3">
                  <div className="flex items-center justify-center w-6 h-6 rounded-full bg-accent-amber/10 border border-accent-amber/30">
                    <Cpu className="w-3 h-3 text-accent-amber" />
                  </div>
                  <span className="text-[11px] font-bold text-accent-amber uppercase tracking-widest font-mono">
                    {language === 'KO' ? 'AI 스튜디오 궤도 동기화 (ORBITAL SYNC)' : 'A.I. Orbital Sync Completed'}
                  </span>
                </div>
                <ul className="list-disc list-inside text-[11px] text-text-primary font-mono space-y-1 ml-9">
          ...
          {!config.title && !config.synopsis && (
            <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-10 pt-3">
              <div className="flex items-center gap-3 px-5 py-4 bg-accent-amber/5 border border-accent-amber/20 rounded-xl shadow-sm">
                <ArrowDown className="w-4 h-4 text-accent-amber shrink-0 animate-bounce" />
                <span className="text-xs text-text-primary font-mono tracking-wide">
                  {language === 'EN' ? 'Enter a title and synopsis below — AI will forge your Universe' : language === 'JP' ? 'タイトルとシノプシスを入力すると、AIが世界を設計します' : language === 'CN' ? '输入标题和大纲，AI将设计您的世界' : '아래에 제목과 시놉시스를 입력하면 NOL 컨스트럭트가 우주를 설계합니다'}
                </span>
              </div>
            </div>
          )}
          ...
          <div className="max-w-6xl mx-auto px-4 pb-8 flex gap-3 justify-end">
            ...
            {config.title || config.synopsis ? (
              <>
                <button onClick={() => setSubTab('simulator')} className="group flex items-center justify-center gap-2 px-6 py-3 rounded-xl border border-accent-amber/30 bg-bg-secondary/60 text-[12px] font-black uppercase tracking-widest font-mono transition-all hover:bg-accent-amber/10 hover:border-accent-amber/60 hover:shadow-md text-accent-amber hover:text-text-primary">
                  <Cpu className="w-4 h-4 transition-colors" /> {language === 'EN' ? 'ATLAS SIMULATOR' : language === 'JP' ? 'シミュレーター' : language === 'CN' ? '地图模拟器' : '엔진 시뮬레이션'}
                </button>
                <button onClick={onStart} className="group flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-linear-to-r from-accent-amber/70 to-accent-amber/90 border border-accent-amber/60 text-[12px] font-black uppercase tracking-widest font-mono transition-all hover:scale-[1.02] hover:shadow-lg text-bg-primary shadow-md">
                  <Compass className="w-4 h-4 drop-shadow-sm" /> {startLabel ?? t('planning.commence')}
                </button>
              </>
            ) : null}
            <button 
              ...
              className={`group flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-[12px] font-black uppercase tracking-widest font-mono transition-all duration-300 ${
                saveFlash 
                  ? 'bg-accent-green text-bg-primary border border-accent-green shadow-md' 
                  : 'bg-linear-to-b from-accent-amber/15 to-accent-amber/20 border border-accent-amber/40 text-accent-amber hover:bg-accent-amber/25 hover:shadow-md'
              }`}
            >
              {saveFlash ? <Check className="w-4 h-4 text-bg-primary" /> : <div className="w-1.5 h-1.5 rounded-full bg-accent-amber shadow-sm" />}
