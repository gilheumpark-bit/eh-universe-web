...
const ManuscriptTab: React.FC<ManuscriptTabProps> = ({
  ...
  const handleSceneMode = useCallback((mode: 'radio' | 'visual' | 'edit') => {
    ...
    if (!latestMs?.content) {
      alert(language === 'KO' 
        ? '시연할 원고가 없습니다. [집필] 탭에서 에피소드를 먼저 생성하거나 내용을 작성해주세요.\n(내러티브 엔진 — 시연 데이터 부재)' 
        : 'No manuscript found. Please create an episode or write content in the [Writing] tab first.\n(Narrative Engine — No Demo Data)');
      return;
    }
    ...
    setSceneMode(mode);
  }, [config.manuscripts, config.characters, language]);
