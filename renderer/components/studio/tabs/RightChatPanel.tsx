...
export const RightChatPanel: React.FC<RightChatPanelProps> = React.memo(({ 
  ...
  const [panelWidth, setPanelWidth] = useState(() => {
    if (typeof window !== 'undefined') {
      ...
      return stored ? parseInt(stored) : 380;
    }
    return 380;
  });
  ...
  return (
    <aside 
      className="relative shrink-0 border-l border-border bg-bg-secondary/30 backdrop-blur-xl flex flex-col h-full animate-in fade-in slide-in-from-right duration-500 shadow-2xl"
      style={{ ...(typeof window !== 'undefined' && window.innerWidth >= 1024 ? { width: `${panelWidth}px`, maxWidth: `${panelWidth}px`, flexBasis: `${panelWidth}px` } : { width: '100%', maxWidth: '100%', flexBasis: '100%' }) }}
    >
      {/* Resizer Handle */}
      <div 
        onMouseDown={handleMouseDown}
        className="hidden lg:block absolute left-[-4px] top-0 bottom-0 w-3 cursor-col-resize z-50 group transition-colors"
      >
        <div className="w-px h-full mx-auto group-hover:bg-accent-amber/50 active:bg-accent-amber transition-colors" />
      </div>
