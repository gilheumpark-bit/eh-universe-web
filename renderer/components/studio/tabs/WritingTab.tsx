...
const TTSPlayerBar = dynamic(() => import('@/components/studio/TTSPlayerBar'), { ssr: false, loading: () => null });
...
const WritingTab: React.FC<WritingTabProps> = ({
  ...
  const ttsText = React.useMemo(() => {
    if (writingMode === 'edit' && editDraft.trim()) return editDraft;
    if (writingMode === 'canvas' && canvasContent.trim()) return canvasContent;
    return currentSession.messages
      .filter(m => m.role === 'assistant' && m.content)
      .map(m => m.content.replace(/```json[\s\S]*?```/g, '').trim())
      .join('\n\n');
  }, [writingMode, editDraft, canvasContent, currentSession.messages]);
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative">
      <div className="flex-1 flex overflow-hidden">
        <div className="flex-1 overflow-y-auto">
          <div className={`max-w-6xl w-full mx-auto px-4 md:px-8 lg:px-12 flex flex-col ${currentSession.messages.length === 0 && writingMode === 'ai' ? 'h-full justify-center items-center' : 'py-6 md:py-8 space-y-6 min-h-full'}`}>
            ...
            {(currentSession.messages.length > 0 || writingMode !== 'ai') && (
            <details className="group border border-border rounded-2xl bg-linear-to- from-bg-secondary/30 to-transparent backdrop-blur-sm overflow-hidden">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                ...
                <button 
                  ...
                  className={`group relative flex flex-col items-center gap-2 px-4 py-4 rounded-2xl border transition-all duration-200 ${
                    writingMode === 'ai' 
                      ? 'bg-accent-purple/15 border-accent-purple/40 shadow-lg shadow-accent-purple/10' 
                ...
                <button 
                  ...
                  className={`group relative flex flex-col items-center gap-2 px-4 py-4 rounded-2xl border transition-all duration-200 ${
                    writingMode === 'refine' 
                      ? 'bg-linear-to- from-accent-purple/15 to-accent-blue/15 border-accent-blue/40 shadow-[0_0_20px_rgba(92,143,214,0.15)]' 
                ...
                <button 
                  ...
                  className={`group relative flex flex-col items-center gap-2 px-4 py-4 rounded-2xl border transition-all duration-200 ${
                    writingMode === 'advanced' 
                      ? 'bg-linear-to- from-amber-500/15 to-orange-500/15 border-amber-500/40 shadow-[0_0_20px_rgba(245,158,11,0.15)]' 
              ...
              {writingMode === 'edit' && (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-accent-amber/5 border border-accent-amber/20">
                  <button 
                    ...
                    className="px-5 py-2.5 bg-linear-to- from-accent-amber to-orange-500 text-white rounded-xl text-[11px] font-bold uppercase tracking-wider hover:opacity-90 transition-opacity disabled:opacity-30 shadow-lg"
                  >
            ...
            {writingMode === 'canvas' && (
              <div className="space-y-4">
                ...
                <textarea value={canvasContent} onChange={e => setCanvasContent(e.target.value)} onContextMenu={textMenu.openMenu} className="w-full min-h-[50vh] bg-bg-primary border border-border rounded-xl p-6 text-sm leading-loose font-serif outline-none focus:border-accent-purple transition-colors resize-y" />
              </div>
            )}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <details className="group">
                  ...
                  <div className="mt-2 space-y-1.5">
                    {currentSession.config.characters.map(c => <div key={c.id} className="text-[10px] text-text-secondary font-bold wrap-break-word">{c.name} ({c.role})</div>)}
                  </div>
                </details>
                <details className="group">
      ...
      {/* Bottom Input Area */}
      {currentSessionId && (
        <div className="px-4 md:px-6 pb-4 md:pb-6 bg-linear-to- from-bg-primary via-bg-primary to-transparent pt-8 md:pt-12 shrink-0">
          <div className="max-w-6xl mx-auto relative px-0">
            <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 md:bottom-auto md:-top-10 md:left-4 md:translate-x-0 flex gap-2 items-center">
              ...
              {ttsText.length > 0 && (
                <button onClick={() => setShowTTS(v => !v)} className={`px-3 py-1.5 border rounded-full text-[10px] font-bold transition-all whitespace-nowrap font-mono ${showTTS ? 'bg-accent-purple/15 border-accent-purple/30 text-accent-purple' : 'bg-bg-secondary border-border text-text-tertiary hover:text-text-primary'}`}>🔊 {language === 'KO' ? '낭독' : 'TTS'}</button>
              )}
            </div>
            <div className="relative bg-bg-secondary border border-border rounded-2xl md:rounded-4xl shadow-2xl p-2 pl-4 md:pl-6 flex items-end">
              <textarea
                value={input}
      ...
      {showTTS && ttsText.length > 0 && (
        <div className="px-4 md:px-6 pb-2">
          <div className="max-w-6xl mx-auto">
            <TTSPlayerBar text={ttsText} isKO={language === 'KO'} onClose={() => setShowTTS(false)} />
          </div>
        </div>
      )}
      {textMenu.menuState && (
