...
export default function WebFeaturesInit() {
  useEffect(() => {
    (async () => {
      ...
      // 적응형 로딩 설정 (네트워크 품질 기반)
      const adaptiveConfig = webFeatures.getAdaptiveConfig();
      document.documentElement.dataset.connectionQuality = adaptiveConfig.quality;
      if (adaptiveConfig.reduceAnimations) {
        document.documentElement.classList.add('reduce-motion');
      }

      // 주요 라우트 프리패치 (빠른 연결에서만)
      if (adaptiveConfig.allowPrefetch) {
        webFeatures.prefetchOnVisible(['/studio', '/translation-studio', '/tools']);
      }
