...
describe('analyzeMetrics', () => {
  const mockConfig: Pick<StoryConfig, 'genre' | 'episode' | 'totalEpisodes' | 'povCharacter' | 'setting' | 'primaryEmotion' | 'title' | 'guardrails' | 'characters' | 'platform'> = { genre: 'SF' as StoryConfig['genre'], episode: 1, totalEpisodes: 25, povCharacter: '', setting: '', primaryEmotion: '', title: '', guardrails: { min: 3000, max: 5000 }, characters: [], platform: PlatformType.MOBILE };

  it('returns zeros for empty text', () => {
    const result = analyzeMetrics('', mockConfig as StoryConfig);
  ...
  it('returns values between 0 and 100', () => {
    ...
    const result = analyzeMetrics(text, mockConfig as StoryConfig);
  ...
  it('action-heavy text has higher tension', () => {
    ...
    expect(analyzeMetrics(action, mockConfig as StoryConfig).tension).toBeGreaterThan(analyzeMetrics(calm, mockConfig as StoryConfig).tension);
  });
});
