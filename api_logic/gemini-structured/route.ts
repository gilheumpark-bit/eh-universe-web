...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!execution.result.ok) {
      if ('response' in execution.result) return (execution.result as { response: NextResponse }).response;
      return NextResponse.json({ error: 'Execution failed without response' }, { status: 500 });
    }
