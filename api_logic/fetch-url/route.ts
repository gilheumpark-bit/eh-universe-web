...
export async function GET(req: NextRequest) {
  ...
  if (!rl.ok) {
    const rs = 'retryAfterSec' in rl ? (rl as { retryAfterSec: number }).retryAfterSec : 60;
    return NextResponse.json(
      { error: `요청이 너무 많습니다. ${rs}초 후 다시 시도하세요.` },
      { status: 429, headers: { 'Retry-After': String(rs) } }
    );
  }

  const allowed = assertUrlAllowedForFetch(url);
  if (!allowed.ok) {
    const reason = 'reason' in allowed ? (allowed as { reason: string }).reason : 'Invalid URL';
