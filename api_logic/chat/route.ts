...
function extractChatFields(body: Record<string, unknown>, requestId: string): { ok: true; fields: ParsedChatFields } | { ok: false; response: NextResponse } {
  ...
  if (!validation.valid) {
    return { ok: false, response: NextResponse.json({ error: (validation as {error: string}).error, requestId }, { status: 400 }) };
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!extracted.ok) return (extracted as {ok: false; response: NextResponse}).response;
    ...
    if (!authResult.ok) return (authResult as {ok: false; response: NextResponse}).response;
    ...
    if (
      ...
      && isGeminiAllocationExhaustedError((dispatched as {ok: false; error: string}).error)
    ) {
    ...
    const trackingStream = wrapStreamWithTracking((dispatched as {ok: true; stream: ReadableStream<any>}).stream, ip, !auth.isByok);
