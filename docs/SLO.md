...
**갱신**: 2026-06-22 · **상태**: 초안 (목표치는 베타 기준 제안값 — 실측 데이터 누적 전까지 잠정)
...
| **AI 응답 p95** (`/api/chat` 첫 스트림 시작까지) | p95 ≤ 10s (스트림 시작 기준) | `apiLog` `chat_stream_start`의 `durationMs` (`src/app/api/chat/route.ts`, `src/lib/api-logger.ts` `createRequestTimer`) + `/api/metrics`의 `eh_api_request_duration_ms_p95` | ◐ **프로세스 단위 집계 구현** — `METRICS_ENABLED=on` + 운영 bearer token 필요. 멀티 인스턴스 합산/알람은 Prometheus/OTel 연결 전까지 미완 |
| **AI 요청 성공률** (`/api/chat` 비-4xx 비율) | ≥ 99% (BYOK·레이트리밋 4xx 제외) | `apiLog` 이벤트가 `/api/metrics`의 `eh_api_requests_total{route,event,status}`로 집계됨 | ◐ **프로세스 단위 집계 구현** — 운영 알람과 장기 보존 대시보드는 미완 |
...
| **프런트 체감 성능** (Core Web Vitals) | LCP p75 ≤ 2.5s · INP p75 ≤ 200ms | `/api/vitals` (`src/app/api/vitals/route.ts`) — 클라이언트 web-vitals beacon 수신 + `/api/metrics`의 `eh_web_vitals_value_p75` | ◐ **프로세스 단위 p75 구현** — 멀티 인스턴스 합산/알람은 미완 |
...
2. ADR-0009 Phase 2: `/api/metrics` 프로세스 단위 집계를 Prometheus/OTel exporter와 장기 보존 대시보드로 연결.
3. save-engine 쓰기 성공/실패 카운터 + 주기 beacon (저장 성공률 실측).
4. Web Vitals p75 운영 알람 임계치와 국가/언어별 분해.
