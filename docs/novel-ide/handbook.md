...
>
> v2.3 — Visual Charter v1.0 (창작 과정 확인서) + ARCS 레이어 반영.
>
...
본 핸드북은 **Loreguard Studio** 한정. 현재 공개 제품 표면은 아래로 제한한다.
...
정합 ✓: CMS / 번역사 / CI 가 Loreguard LSP API 호출.
...
부가 가치 — 현행 제품 정체성 0byte 변경. **장부는 뒤에서 자동 누적, 발급은 작가 명시 액션.**
...
| **creative-domain-prompts/** | `lib/ai/creative-domain-prompts/` | 4 도메인 × 7 handler prompt 매트릭스 |
...
- `creative-structured-json` — 구조화 JSON 생성

삭제된 구표면:
- 구 네트워크 검색 에이전트 — 2026-06 기준 활성 제품 표면이 아니며 레지스트리에서도 제거.

### 16.3 4 도메인 창작 프롬프트 매트릭스
...
| 계층 | 모듈 | 시점 |
|---|---|---|
| L1 입력 차단 | `brand-blocklist.ts` + `scan.ts:scanTextForIP` | 사용자 입력 / 외부 참조 문서 등록 전 점검 |
...
| L4 개인 블록리스트 | 권리/IP 점검 설정 | localStorage 작가별 CRUD |
| L5 외부 참조 정제 | `ragService.ts:sanitizeRagResults` | 번역 보강용 검색 결과 `off`/`annotate`/`strict` 모드 |
...
## 17. 외부 워크플로우 — REST/LSP API

### 17.1 카테고리 정합

작가가 Loreguard Studio에서 작품과 과정기록을 관리하고, 출판사 CMS·번역사 도구·CI가 필요할 때 Loreguard API를 호출한다.
Loreguard가 본진이고 외부 시스템은 호출자다.

### 17.2 대표 API

```text
POST /api/lsp/lint        5축 점검
POST /api/lsp/symbols     Symbol Index export
...
```json
{
  ...
  "chapters": [
    { "id": "ep1", "title": "EP1", "content": "김준이 검을 휘둘렀다." }
  ],
  "storyBible": {},
  "requestedBy": "publisher-cms"
}
```
...
- 외부 통합 문서는 `docs/novel-ide/external-integration.md`를 기준으로 한다.
- 현 제품 표면에서 별도 실행 도구 패키지는 제공하지 않는다.
- 코드 구현은 서버 API와 LSP 응답 계약을 우선한다.
...
### Q10. 외부 점검을 자동화하려면?
A. 별도 실행 도구가 아니라 REST/LSP API를 호출한다. 서버 토큰, payload 스키마, 응답 임계값을 `docs/novel-ide/lsp-spec.md` 기준으로 맞춘다.
...
19:00  GitHub Actions 자동 → Loreguard LSP API 호출
       → 점수 84 → PR 통과 → merge
