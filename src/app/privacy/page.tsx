    ...
    >
      ...
      <section className="mb-10">
        ...
        <ul className="list-disc pl-5 space-y-2 text-text-secondary text-sm leading-relaxed">
          ...
          <li>{T({ ko: "창작물(로컬): IndexedDB에 저장되는 원고·세계관·캐릭터 — 기본은 브라우저 로컬 전용, 외부 전송 없음", en: "Creative work (local): manuscripts / world-building / characters saved to IndexedDB — local-only by default, never transmitted externally", ja: "創作物(ローカル): IndexedDB 保存、既定で外部送信なし", zh: "创作内容(本地): 保存于 IndexedDB，默认不外传" })}</li>
      ...
      <section className="mb-10">
        ...
        <ul className="list-disc pl-5 space-y-2 text-text-secondary text-sm leading-relaxed">
          ...
          <li>Vercel (Vercel Inc., US) — {T({ ko: "호스팅·엣지 네트워크·Analytics", en: "hosting, edge network, Analytics", ja: "ホスティング・Analytics", zh: "托管与 Analytics" })}</li>
          <li>Sentry (Functional Software Inc., US) — {T({ ko: "에러 모니터링", en: "error monitoring", ja: "エラー監視", zh: "错误监控" })}</li>
          <li>DGX Spark Gateway (api.ehuniverse.com, KR) — {T({ ko: "AI 추론 (Qwen 3.6-35B-A3B-FP8 MoE 로컬)", en: "AI inference (Qwen 3.6-35B-A3B-FP8 MoE local)", ja: "AI推論 (Qwen 3.6-35B-A3B-FP8 MoE ローカル)", zh: "AI 推理 (Qwen 3.6-35B-A3B-FP8 MoE 本地)" })}</li>
          <li>{T({ ko: "사용자가 BYOK로 연결한 AI 제공사 (Gemini / Claude / OpenAI / Groq 등) — 각 제공사 정책에 따라 입력 데이터 처리", en: "User-connected AI providers (Gemini, Claude, OpenAI, Groq, etc.) — input data processed under each provider's policy", ja: "BYOK連携のAI提供者 — 各社ポリシー適用", zh: "用户自带密钥连接的 AI 提供商 — 遵循各自政策" })}</li>
        </ul>
      ...
      <section className="mb-10">
        ...
        <ul className="list-disc pl-5 space-y-2 text-text-secondary text-sm leading-relaxed">
          <li>{T({ ko: "세션 쿠키: Firebase Auth 세션 유지 — 필수", en: "Session cookies: Firebase Auth session — essential", ja: "セッション Cookie: Firebase Auth — 必須", zh: "会话 Cookie: Firebase Auth — 必需" })}</li>
          <li>{T({ ko: "분석 쿠키: Vercel Analytics — 익명 페이지뷰 집계 (옵트아웃 가능)", en: "Analytics cookies: Vercel Analytics — anonymous pageviews (opt-out available)", ja: "分析 Cookie: Vercel Analytics — 匿名", zh: "分析 Cookie: Vercel Analytics — 匿名" })}</li>
          <li>{T({ ko: "로컬 저장소: 언어·테마·원고·API 키 — 디바이스 내부", en: "Local storage: language/theme/manuscripts/API keys — on-device only", ja: "ローカル保存: 言語・テーマ・原稿・APIキー", zh: "本地存储: 语言、主题、原稿、API 密钥" })}</li>
        </ul>
      </section>
      ...
      <section className="mb-10">
        ...
        <p className="text-text-secondary text-sm leading-relaxed">
          {T({
            ...
            ja: "利用者の創作物はLoreguardのDGXモデル再学習には使用しません。BYOK提供者は各社ポリシー",
            zh: "用户作品不用于 Loreguard 自有 DGX 模型的再训练；BYOK 提供商依各自政策 — 详见 /ai-disclosure。",
          })}
        </p>
      </section>
