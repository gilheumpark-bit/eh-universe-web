  ...
  return (
    ...
    >
      ...
      <section className="mb-10">
        ...
        <p className="text-text-secondary text-sm leading-relaxed">
          {T({
            ko: "결제일로부터 7일 이내에는 청약철회(전액 환불)가 가능합니다. 단, 전자상거래법 제17조 제2항에 따라 디지털 서비스(AI 생성·번역 등)를 이미 사용 개시한 경우, 사용 개시 사실을 사전 고지하였으므로 해당 사용분에 대한 청약철회가 제한될 수 있습니다.",
            en: "Within 7 days of payment you may withdraw for a full refund. However, under Article 17(2) of the E-Commerce Act, if you have already begun using the digital service (AI generation, translation, etc.) after prior notice, withdrawal for the used portion may be restricted.",
            ja: "決済日から7日以内は全額返金可能です。ただし、デジタルサービスの使用を開始した場合、当該使用分の撤回が制限される場合があります。",
            zh: "自付款之日起 7 日内可撤销并全额退款。但若已开始使用数字服务(AI 生成、翻译等),所用部分的撤销可能受限。",
          })}
        </p>
      </section>
      ...
      <section className="mb-10">
        ...
        <ul className="list-disc pl-5 space-y-2 text-text-secondary text-sm leading-relaxed">
          <li>{T({ ko: "이미 소비된 AI 사용량(토큰·생성 횟수) 및 제공 완료된 디지털 산출물.", en: "AI usage already consumed (tokens, generations) and digital deliverables already provided.", ja: "消費済みのAI使用量・提供済みの成果物。", zh: "已消耗的 AI 用量及已交付的数字成果。" })}</li>
          <li>{T({ ko: "사용자 본인 키(BYOK) 또는 로컬 AI 사용 비용 — Loreguard 결제 대상 아님.", en: "Costs of your own keys (BYOK) or local AI — not billed by Loreguard.", ja: "BYOK・ローカルAIの費用 — Loreguardの課金対象外。", zh: "自带密钥(BYOK)或本地 AI 的费用 — 非 Loreguard 收费。" })}</li>
        </ul>
      </section>
      ...
      <section className="mb-10">
        ...
        <p className="text-text-secondary text-sm leading-relaxed">
          {T({
            ...
            en: "Request a refund by email with your payment details (receipt, date). After review, refunds are issued via Stripe to the original payment method within 3–5 business days (card-issuer processing time excluded).",
            ja: "返金は決済情報を添えて下記メールへご請求ください。審査後3〜5営業日以内に Stripe で返金します。",
            zh: "请通过下方邮箱附付款信息申请退款。审核后 3–5 个工作日内经 Stripe 退至原支付方式。",
          })}
        </p>
      </section>
