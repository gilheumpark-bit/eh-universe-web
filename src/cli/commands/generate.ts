...
export async function runGenerate(prompt: string, opts: GenerateOptions): Promise<void> {
  ...
  // [C] Git commit with AI message — execFileSync(argv 배열)로 shell 미경유.
  // AI 출력(aiMsg)이 백틱/세미콜론을 포함해도 shell metachar 해석 차단 → prompt injection RCE 방어.
  if (opts.commit) {
  ...
  // --pr: create PR (requires gh CLI) — execFileSync(argv 배열)로 shell 미경유.
  if (opts.pr) {
    const { execFileSync } = require('child_process');
    try {
      const branchName = `cs/${prompt.replace(/[^a-zA-Z0-9]/g, '-').slice(0, 30)}`;
      // git checkout -b: 이미 존재하면 stderr 발생하지만 무시. shell `|| true` 제거.
      try {
        execFileSync('git', ['checkout', '-b', branchName], { stdio: 'pipe' });
      } catch { /* 브랜치 이미 존재 — push 단계에서 처리 */ }
      execFileSync('git', ['push', '-u', 'origin', branchName], { stdio: 'pipe' });
      const prTitle = `feat(cs): ${prompt.slice(0, 60)}`;
      const prBody = `Generated by CS Quill 🦔\n\nScore: ${pipelineResult.overallScore}/100\nReceipt: ${receipt.id}`;
      execFileSync('gh', ['pr', 'create', '--title', prTitle, '--body', prBody], { stdio: 'pipe' });
      console.log(`\n  🔗 PR 생성: ${prTitle}`);
    } catch {
