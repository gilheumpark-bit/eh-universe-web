          ...
          <div className="p-2 border-b border-white/8">
            <div className="relative">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="프로젝트 검색..."
                className="w-full pl-7 pr-3 py-1.5 text-xs rounded-md bg-white/5 text-white border border-white/8 placeholder:text-white/30 focus:outline-none focus:ring-1 focus:ring-purple-500" />
            </div>
          </div>
          ...
          {showCreateForm && (
            <div className="p-3 border-b border-white/8 bg-white/3">
              <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="프로젝트 이름" autoFocus
                className="w-full px-2 py-1.5 text-xs rounded-md bg-[#0a0e17] text-white border border-white/10 focus:outline-none focus:ring-1 focus:ring-purple-500 mb-2"
              ...
              <div className="flex gap-2 justify-end">
                ...
                <button onClick={handleCreate} disabled={!newName.trim()} className="px-3 py-1 text-xs rounded bg-purple-600 text-white hover:bg-purple-700 disabled:opacity-50">만들기</button>
              </div>
            </div>
          )}
              ...
              <div key={project.id} className="relative group">
                {confirmDeleteId === project.id ? (
                  ...
                  <button onClick={() => handleSwitch(project.id)}
                    className={`w-full flex items-start gap-2 px-3 py-2 text-left hover:bg-white/5 transition-colors ${project.id === currentProjectId ? "bg-white/5 border-l-2 border-l-purple-500" : ""}`}>
                    <FolderOpen className={`w-4 h-4 mt-0.5 shrink-0 ${project.id === currentProjectId ? "text-purple-400" : "text-white/30"}`} />
