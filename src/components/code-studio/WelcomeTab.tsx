...
export default function WelcomeTab({
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center p-8 text-center">
      ...
      <p className="mb-8 text-sm text-text-tertiary">{t.subtitle}</p>
      ...
      <div className="mb-8 flex gap-4">
        {actions.map((a) => (
          <button
            key={a.label}
            onClick={a.onClick}
            className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm text-text-tertiary hover:bg-white/10 transition-colors"
          >
      ...
      {recentFiles.length > 0 && (
        <div className="mb-8 w-full max-w-md">
          <h3 className="mb-2 flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-text-tertiary">
