...
import { readUIPref, writeUIPref, UI_PREF_KEYS } from "@/lib/ui-preferences";
...
// ============================================================
// PART 2.5 — M6 Ergonomics 중첩 accordion (M8 재구성)
// ============================================================
// 기본 접힘. 내부 2그룹:
//   (A) 편안한 집필: 글자 크기 / 자세 / 눈 피로 / 손목
//   (B) 통계·고급 (opt-in): KPM / 탭 복귀 nudge
// 펼침 상태는 ui-preferences로 개별 보존.
// ============================================================

interface M6ErgoNestedGroupProps {
  language: AppLanguage;
  ergo: ErgonomicsSettings;
  setErgoField: <K extends keyof ErgonomicsSettings>(key: K, value: ErgonomicsSettings[K]) => void;
}

const M6ErgoNestedGroup: React.FC<M6ErgoNestedGroupProps> = ({ language, ergo, setErgoField }) => {
  ...
  const toggleErgo = () => {
    const next = !ergoOpen;
    setErgoOpen(next);
    writeUIPref(UI_PREF_KEYS.sessionErgoOpen, next);
  };
  const toggleStats = () => {
    const next = !statsOpen;
    setStatsOpen(next);
    writeUIPref(UI_PREF_KEYS.sessionStatsOpen, next);
  };

  return (
    <div className="rounded-2xl border border-border bg-bg-secondary/10 overflow-hidden">
      {/* 외곽 헤더 — 클릭 시 전체 ergonomics 그룹 열림 */}
      <button
        type="button"
        data-testid="m8-ergo-toggle"
        onClick={toggleErgo}
        aria-expanded={ergoOpen}
        aria-controls="m8-ergo-content"
        className="w-full flex items-center gap-2 px-4 py-3 hover:bg-bg-secondary/30 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-blue"
      >
        <Activity className="w-4 h-4 text-accent-amber shrink-0" />
        <span className="text-[10px] font-black text-text-tertiary uppercase tracking-widest flex-1 text-left">
          {L4(language, {
            ko: "장기 사용 편의 (선택)",
            en: "Long-Session Comfort (Optional)",
            ja: "長時間使用の快適性 (オプション)",
            zh: "长时间使用舒适性 (可选)",
          })}
        </span>
        <span className="text-[10px] text-text-quaternary font-normal normal-case tracking-normal hidden sm:inline">
          {L4(language, {
            ko: "기본값 그대로도 괜찮습니다",
            en: "Defaults are fine for most writers",
            ja: "既定値のままでも大丈夫",
            zh: "保持默认也没问题",
          })}
        </span>
        <ChevronDown className={`w-4 h-4 text-text-tertiary transition-transform ${ergoOpen ? "rotate-180" : ""}`} />
      </button>

      {ergoOpen && (
        <div id="m8-ergo-content" className="p-4 space-y-3 border-t border-border">
          {/* (A) 편안한 집필 그룹 */}
          <div className="space-y-3">
            {/* 글자 크기 프리셋 (Pattern E — "타이포그래피" → "글자 크기") */}
            <div className="p-4 bg-bg-secondary/30 rounded-2xl border border-border">
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["comfort", "compact", "large"] as TypographyPreset[]).map((p) => {
                  ...
                  const labelMap: Record<TypographyPreset, { ko: string; en: string; ja: string; zh: string }> = {
                    comfort: { ko: "보통 (17px)", en: "Normal (17px)", ja: "通常 (17px)", zh: "标准 (17px)" },
                    compact: { ko: "작게 (14px)", en: "Small (14px)", ja: "小さめ (14px)", zh: "紧凑 (14px)" },
                    large: { ko: "크게 (20px)", en: "Large (20px)", ja: "大きめ (20px)", zh: "大号 (20px)" },
                  };
                  return (
                    <button
                      key={p}
              ...
              <p className="mt-2 text-[11px] text-text-tertiary leading-relaxed">
                {L4(language, {
                  ko: "장시간 집필엔 보통, 통독엔 작게, 눈이 피곤하면 크게를 추천합니다.",
                  en: "Normal for long sessions, Small for overview, Large when your eyes tire.",
                  ja: "長時間は通常、通読は小さめ、目が疲れたら大きめを推奨。",
                  zh: "长时间创作推荐标准,通读推荐紧凑,眼疲劳时推荐大号。",
                })}
              </p>
            </div>
          ...
          {/* (B) 통계·고급 그룹 (opt-in, 기본 접힘) */}
          <div className="rounded-xl border border-border/60 bg-bg-primary/40 overflow-hidden">
            <button
              type="button"
              data-testid="m8-stats-toggle"
              onClick={toggleStats}
              aria-expanded={statsOpen}
              aria-controls="m8-stats-content"
              className="w-full flex items-center gap-2 px-3 py-2.5 hover:bg-bg-secondary/30 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-blue"
            >
              <Activity className="w-3.5 h-3.5 text-accent-purple shrink-0" />
              <span className="text-[10px] font-black text-text-tertiary uppercase tracking-widest flex-1 text-left">
                {L4(language, {
                  ko: "통계·고급 (필요할 때만)",
                  en: "Stats & Advanced (only if needed)",
                  ja: "統計・詳細 (必要なときだけ)",
                  zh: "统计·高级 (仅在需要时)",
                })}
              </span>
              <ChevronDown className={`w-3.5 h-3.5 text-text-tertiary transition-transform ${statsOpen ? "rotate-180" : ""}`} />
            </button>

            {statsOpen && (
              <div id="m8-stats-content" className="p-3 space-y-2 border-t border-border/60">
                {/* KPM — 기본 off, 숨김 계층 */}
                <ToggleRow
                  icon={<Activity className="w-4 h-4 md:w-5 md:h-5 text-accent-purple" />}
                  label={L4(language, {
                    ko: "분당 타자수 표시 (KPM)",
                    en: "Show keystrokes per minute (KPM)",
                    ja: "1分あたりの打鍵数表示 (KPM)",
                    zh: "显示每分钟键击数 (KPM)",
                  })}
                  description={L4(language, {
                    ko: "상태바에 분당 타자수가 나타납니다. 기기에만 저장됩니다.",
                    en: "Keystrokes-per-minute appears in the status bar. Stays on this device.",
                    ja: "ステータスバーに分あたりの打鍵数を表示。この端末にのみ保存。",
                    zh: "状态栏显示每分钟击键数,仅保存在本机。",
                  })}
                  checked={ergo.keystrokeHeatmapVisible}
                  onChange={(v) => setErgoField("keystrokeHeatmapVisible", v)}
                />

                {/* Focus drift — 기본 off, 숨김 계층 */}
                <ToggleRow
...
// IDENTITY_SEAL: PART-2.5 | role=m6-ergo-nested | inputs=language+ergo+setErgoField | outputs=nested-accordion
  ...
  return (
    <details className="ds-accordion rounded-2xl bg-bg-secondary/20 border border-border overflow-hidden group">
      ...
      <div className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* ============================================================
            M6 — 장기 사용 편의 (Ergonomics)  [M8 재구성]
            ============================================================
            변경 요지 (M8):
              - 상위 accordion 안의 "nested accordion"으로 분리.
                기본 접힘 → 작가의 첫 화면에서 자력으로 11개 토글을 마주치지 않음.
              - 내부 2그룹:
                  1) 편안한 집필 (기본 노출): 글자 크기 / 자세 / 눈 피로 / 손목
                  2) 통계·고급 (opt-in): KPM / 탭 복귀 nudge → 기본 접힘
              - 모든 값은 "옵트인". 기능 제거 없음.
              - KPM/focus-drift 는 ergonomics-settings 기본값이 이미 OFF.
                M8에서는 UI 노출만 숨김 계층으로 이동.
            ============================================================ */}
        <div className="md:col-span-2 md:col-start-1 mt-2">
          <M6ErgoNestedGroup language={language} ergo={ergo} setErgoField={setErgoField} />
        </div>
      </div>
    </details>
  );
