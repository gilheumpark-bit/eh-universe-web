...
import type { AppLanguage, EpisodeManuscript, TranslatedManuscriptEntry } from "@/lib/studio-types";
import { BAND_META, bandLabel, modeDescription, type TranslationMode, type TranslationProgress, type TranslationTarget } from "@/engine/translation";
...
type ContractionLevel = "none" | "low" | "normal" | "high";

function bindProgressFill(node: HTMLElement | null, percent: number) {
  if (!node) return;
  node.style.setProperty("--studio-progress-fill", `${Math.max(0, Math.min(100, percent))}%`);
}

interface TranslationNovelModeConfigProps {
  isKO: boolean;
  language: AppLanguage;
  mode: TranslationMode;
  setMode: Dispatch<SetStateAction<TranslationMode>>;
  targetLang: TranslationTarget;
  setTargetLang: Dispatch<SetStateAction<TranslationTarget>>;
  band: number;
  setBand: Dispatch<SetStateAction<number>>;
}

export function TranslationNovelModeConfig({
  isKO,
  language,
  mode,
  setMode,
  ...
  band,
  setBand,
}: TranslationNovelModeConfigProps) {
          ...
          return (
            ...
            >
              ...
              <div className="relative z-10 flex flex-col gap-2">
                ...
                <div className="pl-4 font-sans text-[12px] leading-relaxed text-text-tertiary">
                  {info.desc}
                </div>
              </div>
            </button>
      ...
      <div className="relative flex flex-col md:flex-row items-center gap-6 rounded-[1.25rem] border border-white/8 bg-bg-tertiary backdrop-blur-xl p-5 shadow-2xl before:absolute before:inset-0 before:bg-[linear-gradient(to_bottom,rgba(255,255,255,0.03),transparent)] before:pointer-events-none before:rounded-[1.25rem]">
        <div className="space-y-2.5 w-full md:w-auto md:min-w-[160px] relative z-10">
          <label className="flex items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.15em] text-text-tertiary">
            <div className="w-1.5 h-1.5 rounded-full bg-[rgba(184,149,92,0.5)]" />
        ...
        <div className="flex-1 w-full space-y-3 relative z-10">
          ...
          <div className="relative pt-1">
            ...
            />
            <div
              ref={(node) => bindProgressFill(node, ((band - BAND_META.min) / (BAND_META.max - BAND_META.min)) * 100)}
              className="absolute top-1 left-0 h-1.5 bg-[linear-gradient(90deg,rgba(184,149,92,0.4),rgba(184,149,92,0.9))] rounded-full pointer-events-none studio-progress-fill"
            >
              <div className="absolute right-0 top-1/2 -translate-y-1/2 w-4 h-4 bg-white rounded-full shadow-[0_0_10px_rgba(184,149,92,0.8)] border-2 border-border" />
            </div>
          </div>
        </div>
      </div>
...
interface TranslationAdvancedSettingsProps {
  isKO: boolean;
  language: AppLanguage;
  mode: TranslationMode;
  targetGenre: string;
  setTargetGenre: Dispatch<SetStateAction<string>>;
  scoreThreshold: number;
  setScoreThreshold: Dispatch<SetStateAction<number>>;
  contractionLevel: ContractionLevel;
  setContractionLevel: Dispatch<SetStateAction<ContractionLevel>>;
}

export function TranslationAdvancedSettings({
  isKO,
  language,
  mode,
  targetGenre,
  ...
  contractionLevel,
  setContractionLevel,
}: TranslationAdvancedSettingsProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 p-5 rounded-[1.25rem] border border-accent-amber/20 bg-bg-secondary animate-in slide-in-from-top-2 duration-300">
      ...
      <div className="space-y-2">
        <label className="flex items-center gap-1.5 font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-text-secondary">
          ...
          {isKO ? (
            <><TermTooltip term="품질 항목" language={language}>품질 항목</TermTooltip> 기준점</>
          ) : (
            <><TermTooltip term="품질 항목" language={language}>Quality</TermTooltip> Threshold</>
          )}
        </label>
        <input
          type="number"
      ...
      <div className="space-y-2">
        ...
        <select
          ...
          onChange={(e) => setContractionLevel(e.target.value as ContractionLevel)}
...
interface TranslationGlossaryEditorProps {
  isKO: boolean;
  glossary: Record<string, string>;
  glossaryTerm: string;
  setGlossaryTerm: Dispatch<SetStateAction<string>>;
  glossaryTranslation: string;
  setGlossaryTranslation: Dispatch<SetStateAction<string>>;
  saveGlossary: (glossary: Record<string, string>) => void;
}

export function TranslationGlossaryEditor({
  isKO,
  glossary,
  glossaryTerm,
  setGlossaryTerm,
  ...
  saveGlossary,
}: TranslationGlossaryEditorProps) {
  const addTerm = () => {
    if (!glossaryTerm.trim() || !glossaryTranslation.trim()) return;
    saveGlossary({ ...glossary, [glossaryTerm.trim()]: glossaryTranslation.trim() });
    setGlossaryTerm("");
    setGlossaryTranslation("");
  };
    ...
    <details className="rounded-[1.25rem] border border-white/8 bg-black/20 backdrop-blur-md">
      ...
      <div className="px-5 pb-4 space-y-3">
        <div className="flex gap-2">
          ...
          <button type="button" onClick={addTerm} className="px-3 py-2 rounded-lg bg-[rgba(184,149,92,0.15)] text-text-primary font-mono text-[10px] font-bold hover:bg-[rgba(184,149,92,0.25)] transition-colors">+</button>
        </div>
            ...
            {Object.entries(glossary).map(([k, v]) => (
              <span key={k} className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-[rgba(184,149,92,0.1)] border border-border font-mono text-[10px] text-text-primary">
                ...
                <button
                  ...
                  aria-label={isKO ? `용어 ${k} 삭제` : `Delete glossary term ${k}`}
                  className="ml-1 text-accent-red/60 hover:text-accent-red focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-blue rounded"
                >
                  &times;
                </button>
              </span>
...
interface TranslationExecutionControlsProps {
  isKO: boolean;
  batchMode: boolean;
  setBatchMode: Dispatch<SetStateAction<boolean>>;
  manuscripts: EpisodeManuscript[];
  selectedEpisode: number | null;
  setSelectedEpisode: Dispatch<SetStateAction<number | null>>;
  isTranslating: boolean;
  abort: () => void;
  handleTranslate: () => void;
}

export function TranslationExecutionControls({
  isKO,
  batchMode,
  setBatchMode,
  manuscripts,
  ...
  abort,
  handleTranslate,
}: TranslationExecutionControlsProps) {
  return (
      ...
      <div className="flex-1 w-full space-y-2">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            aria-label={isKO ? `배치 모드 ${batchMode ? "끄기" : "켜기"}` : `Turn batch mode ${batchMode ? "off" : "on"}`}
            className={`font-mono text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-blue ${batchMode ? "border-border bg-[rgba(184,149,92,0.15)] text-text-primary" : "border-white/10 text-text-tertiary hover:border-white/20"}`}
          >
...
interface TranslationOperationsTerminalProps {
  progress: TranslationProgress;
  logs: LogEntry[];
  logsEndRef: RefObject<HTMLDivElement | null>;
}

export function TranslationOperationsTerminal({ progress, logs, logsEndRef }: TranslationOperationsTerminalProps) {
  if (progress.status === "idle" && logs.length === 0) return null;
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-[rgba(184,149,92,0.05)] relative z-20">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="w-32 h-1.5 bg-black/60 rounded-full overflow-hidden border border-white/5 relative">
            <ProgressFill
              value={progress.totalChunks > 0 ? (progress.completedChunks / progress.totalChunks) * 100 : 0}
...
interface TranslationResultsGridProps {
  isKO: boolean;
  translatedList: TranslatedManuscriptEntry[];
}

export function TranslationResultsGrid({ isKO, translatedList }: TranslationResultsGridProps) {
  if (translatedList.length === 0) return null;
    ...
    <div className="pt-6 border-t border-border space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {translatedList.map((t, i) => (
          <div key={`${t.episode}-${t.targetLang}-${i}`} className="group relative flex flex-col gap-2.5 rounded-2xl border border-border bg-bg-tertiary backdrop-blur-md p-4 transition-[transform,background-color,border-color,box-shadow,color] duration-300 hover:border-border hover:bg-[rgba(184,149,92,0.05)] hover:-translate-y-1 shadow-lg hover:shadow-[0_10px_30px_rgba(184,149,92,0.1)]">
            <div className="absolute top-0 left-4 right-4 h-px bg-[linear-gradient(90deg,transparent,rgba(184,149,92,0.3),transparent)] opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            <div className="flex items-center gap-3">
...
interface TranslationQueueEmptyStateProps {
  isKO: boolean;
  manuscripts: EpisodeManuscript[];
}

export function TranslationQueueEmptyState({ isKO, manuscripts }: TranslationQueueEmptyStateProps) {
