import { AppTab, StoryConfig } from '@/lib/studio-types';
import { buildNoaSystemHeader } from '@/lib/ai/noa-identity';

export interface TabAssistantContext {
  ko: string;
  en: string;
  systemKo: string;
  systemEn: string;
...
export interface TabAssistantPreset {
  ko: string;
  en: string;
  ja: string;
  zh: string;
...
export const TAB_ASSISTANT_STORAGE_PREFIX = 'noa_tab_chat_';
export const TAB_CONTEXT: Record<string, TabAssistantContext> = {
  world: {
...
export const TAB_PRESETS: Record<string, TabAssistantPreset[]> = {
  world: [
...
export function buildContextSummary(config: StoryConfig | null, tab: AppTab): string {
  if (!config) return '';
  const parts: string[] = [];
