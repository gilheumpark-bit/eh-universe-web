...
// ============================================================
// WorldFactChatFill — "채팅 → AI가 양식 채움 → 사람 검토 → 커밋" 패널 (Phase 1 MVP)
// worldgraph/fill 엔진 사용. Design System v8.0 정합(08-디자인시스템-신규surface spec).
// 현재 fill = 로컬 초안(키 없이 동작). AI 키 연결 시 generateJsonViaSpark(buildFillPrompt)로 교체.
// ============================================================
...
/** [P1 low/functional 2026-06-09] 라벨-동작 일치: 실 AI 키 가용성 동기 체크.
 *  하나라도 연결되면 'AI', 아니면 '로컬 초안' 으로 라벨 표기 → 사용자 기만 0. */
const AI_PROVIDERS: ReadonlyArray<ProviderId> = ['gemini', 'openai', 'claude', 'groq', 'mistral', 'ollama', 'lmstudio'];

function detectAiAvailable(): boolean {
  if (typeof window === 'undefined') return false;
  if (hasDgxService()) return true;
  return AI_PROVIDERS.some((id) => hasStoredApiKey(id));
}
...
export default function WorldFactChatFill({ workId, onCommit }: Props) {
  ...
  useEffect(() => { setAiAvailable(detectAiAvailable()); }, []);

  const handleFill = useCallback(() => {
    const text = chat.trim();
    if (!text) return;
    // [AI 경로] 키 연결 시: generateJsonViaSpark(buildFillPrompt(text)) → parseAIFill(raw, text).
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-[var(--sp-md)] rounded-xl border border-border bg-bg-secondary/60 p-[var(--sp-md)]">
      ...
      <div className="flex flex-col gap-[var(--sp-sm)]">
        ...
        >
          ...
          {aiAvailable ? 'AI가 양식 채우기' : 'AI가 양식 채우기 (로컬 초안)'}
        </button>
        <p className="text-[11px] text-text-tertiary">
          {aiAvailable
            ? '결정론 로컬 초안으로 양식을 즉시 채웁니다. 채운 값은 작가 검토 후 확정하세요.'
            : '현재 로컬 초안(키 미연결). AI 키 연결 시 실 생성으로 교체됩니다.'}
        </p>
      </div>
      ...
      <div className="flex flex-col gap-[var(--sp-sm)]">
          ...
          {draft &&
            (origin === 'USER' ? (
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-accent-amber/15 px-2 py-0.5 text-[11px] font-semibold text-accent-amber">
                <Sparkles className="h-3 w-3" aria-hidden /> AI 초안 · 검토 필요
              </span>
            ))}
        </div>
        ...
        {!draft ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border py-10 text-center">
            ...
            <p className="text-sm text-text-secondary">채팅 후 [AI가 양식 채우기]를 누르면 양식이 채워집니다.</p>
          </div>
        ) : (
