    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="rounded-full border border-accent-purple/20 bg-accent-purple/8 px-2.5 py-0.5 font-mono text-[10px] text-accent-purple">
            NOA-X v2.1
          </span>
        </div>
        <button
          onClick={() => setShowAdvanced(!showAdvanced)}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[11px] transition-all font-mono ${
      ...
      <div className="grid grid-cols-2 gap-3">
        {(["fidelity", "experience"] as TranslationMode[]).map((m) => {
          ...
          return (
            ...
            >
              <div className="font-mono text-[11px] font-bold uppercase tracking-wider text-text-primary">
                {info.title}
              </div>
      ...
      <div className="flex flex-wrap items-center gap-4 p-4 rounded-xl border border-white/8 bg-bg-secondary">
          ...
          <div className="flex gap-1">
            {(["EN", "JP", "CN"] as TranslationTarget[]).map((l) => (
              <button
                ...
                className={`rounded-md border px-3 py-1.5 font-mono text-[11px] font-bold transition-all ${
                  targetLang === l
        ...
        <div className="flex-1 min-w-[200px] space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-[10px] px-2 py-0.5 rounded-sm bg-accent-purple/10 text-accent-purple">{bandLbl} ({(band).toFixed(3)})</span>
          </div>
          <input
            type="range"
            min={BAND_META.min}
      ...
      {showAdvanced && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-4 rounded-xl border border-accent-purple/20 bg-accent-purple/[0.02]">
          <div className="space-y-1">
            <label className="font-mono text-[10px] uppercase tracking-wider text-accent-purple/70">
              {isKO ? "장르 현지화 프리셋" : "Genre Prefix"}
            </label>
            <select
              value={targetGenre}
              onChange={(e) => setTargetGenre(e.target.value)}
              className="w-full rounded-md border border-white/10 bg-bg-secondary px-2 py-1.5 font-mono text-[11px] text-text-secondary outline-none focus:border-accent-purple/40"
            >
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full rounded-md border border-white/10 bg-bg-secondary px-2 py-1.5 font-mono text-[11px] disabled:opacity-50 text-text-secondary outline-none focus:border-accent-purple/40"
            >
      ...
      <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3">
        <div className="flex-1 w-full space-y-1.5">
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none rounded-lg border border-white/10 bg-bg-secondary px-4 py-3 pr-8 font-mono text-[12px] text-text-primary outline-none focus:border-accent-purple/30 transition-colors"
            >
        ...
        {isTranslating ? (
          <button
            onClick={abort}
            className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-lg bg-accent-red/20 px-6 py-3 font-mono text-[12px] font-bold uppercase text-accent-red transition-all hover:bg-accent-red/30 shadow-lg shadow-accent-red/5"
          >
          ...
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-lg bg-accent-purple px-6 py-3 font-mono text-[12px] font-bold uppercase text-bg-primary transition-all hover:brightness-110 disabled:opacity-30 disabled:hover:brightness-100 shadow-lg shadow-accent-purple/20"
          >
      ...
      {(progress.status !== "idle" || logs.length > 0) && (
        <div className="rounded-xl border border-white/8 bg-[#0D0D0D] overflow-hidden flex flex-col h-[280px]">
          ...
          <div className="flex items-center justify-between px-4 py-2 bg-white/[0.02] border-b border-white/5">
            <div className={`flex items-center gap-2 font-mono text-[10px] font-bold uppercase ${progress.status === 'idle' ? 'text-text-tertiary' : statusColor}`}>
            ...
            {translatedList.map((t, i) => (
              <div key={i} className="flex flex-col gap-1.5 rounded-xl border border-white/8 bg-bg-secondary p-3 transition-colors hover:border-accent-green/30">
                <div className="flex items-center gap-2">
                  ...
                  <div className="font-mono text-[12px] font-bold text-text-primary truncate">
                    EP {t.episode} — {t.translatedTitle || `Episode ${t.episode}`}
                  </div>
                </div>
                <div className="flex items-center gap-2 font-mono text-[10px] text-text-tertiary ml-6">
