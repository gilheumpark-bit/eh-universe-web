...
const CharacterTab: React.FC<CharacterTabProps> = ({
  ...
  const handleAutoGenerate = async () => {
    if (!activeSupportsStructured()) {
      const msg = ({ KO: "현재 노아 엔진은 구조화 생성을 지원하지 않습니다. Gemini를 사용해주세요.", EN: "Current NOA engine doesn't support structured generation. Please use Gemini.", JP: "現在のノアエンジンは構造化生成に対応していません。Geminiをご利用ください。", CN: "当前诺亚引擎不支持结构化生成，请使用Gemini。" })[language];
