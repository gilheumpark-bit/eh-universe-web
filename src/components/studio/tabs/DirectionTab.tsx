...
// ============================================================
// PART 1 — Imports, Types & Direction Card Catalog
// ============================================================
...
const CARDS = [
  ...
  },
  {
    ...
    ko: '장면 연출',
    en: 'Scene Direction',
    descKo: '고구마(답답) · 사이다(통쾌) · 훅 · 클리프 · 전환',
    descEn: 'Tension(Goguma) · Release(Cider) · Hooks · Cliffs · Transitions',
    color: 'accent-amber',
...
const DirectionTab: React.FC<DirectionTabProps> = ({
  ...
  const updateSceneDirection = useCallback((nextDirection: SceneDirectionData) => {
    if (!currentSessionId) {
      logger.warn('DirectionTab', 'updateSceneDirection skipped: no active session');
      return;
    }
    updateCurrentSession({
      config: {
  ...
  const ensureSession = (ctx: string): boolean => {
    if (!currentSessionId) {
      logger.warn('DirectionTab', `${ctx} skipped: no active session`);
      return false;
    }
    return true;
  };
  ...
  const sceneSheetProps = {
    ...
    onDirectionUpdate: (data: FullDirectionData) => {
      ...
      const d = data as DirectionPayload;
      updateCurrentSession({
        config: {
          ...(config || INITIAL_CONFIG),
          sceneDirection: {
