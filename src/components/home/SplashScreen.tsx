      ...
      <div className="relative z-10 w-full max-w-2xl mx-auto px-6 py-10 flex flex-col items-center gap-6 sm:gap-8">
        ...
        >
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
            <button
              onClick={onStudio}
              aria-label={L4(lang, { ko: "프로젝트 생성 · 스튜디오 열기", en: "Create project in Studio", ja: "スタジオでプロジェクト作成", zh: "在工作室创建项目" })}
              style={{ color: '#ffffff' }}
          ...
          {projectMenuOpen && (
            <div
              ...
              className="mt-4 grid w-full max-w-[420px] gap-2 rounded-2xl border border-border bg-bg-primary/90 p-2 text-left shadow-lg backdrop-blur-sm"
            >
              <button
                type="button"
                onClick={onStudio}
                className="flex min-h-[52px] items-center gap-3 rounded-xl px-3 py-2 text-left transition-colors hover:bg-bg-secondary focus-visible:ring-2 focus-visible:ring-accent-amber"
              >
                <FilePlus2 className="h-4 w-4 shrink-0 text-accent-amber" />
                <span className="min-w-0">
                  <span className="block text-sm font-bold text-text-primary">
                    {L4(lang, { ko: "새 프로젝트 만들기", en: "Create New Project", ja: "新規プロジェクト作成", zh: "创建新项目" })}
                  </span>
                  <span className="block text-[11px] leading-5 text-text-tertiary">
                    {L4(lang, {
                      ko: "작품 기준을 입력하는 첫 화면으로 이동합니다.",
                      en: "Open the first screen for setting up a new work.",
                      ja: "作品基準を入力する最初の画面へ移動します。",
                      zh: "进入填写作品基准的第一屏。",
                    })}
                  </span>
                </span>
              ...
              <button
                ...
                className="flex min-h-[52px] items-center gap-3 rounded-xl px-3 py-2 text-left transition-colors hover:bg-bg-secondary focus-visible:ring-2 focus-visible:ring-accent-blue"
              >
                ...
                <span className="min-w-0">
                  ...
                  <span className="block text-[11px] leading-5 text-text-tertiary">
                    {hasSavedProjects
                      ...
                      : L4(lang, {
                        ko: "저장된 프로젝트가 있으면 목록을 엽니다.",
                        en: "Open the saved project list when one exists.",
                        ja: "保存済みプロジェクトがある場合に一覧を開きます。",
                        zh: "有已保存项目时打开列表。",
                      })}
                  </span>
                </span>
              </button>
              {projectMenuNotice === "empty" && (
                <div className="rounded-xl border border-accent-blue/20 bg-accent-blue/10 px-3 py-3" role="status" aria-live="polite">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={onStudio}
                      className="inline-flex min-h-10 items-center justify-center rounded-lg bg-accent-amber px-3 text-xs font-bold !text-white transition-colors hover:bg-accent-amber/90 focus-visible:ring-2 focus-visible:ring-accent-amber"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex min-h-10 items-center justify-center rounded-lg border border-border bg-bg-secondary px-3 text-xs font-bold text-text-primary transition-colors hover:bg-bg-tertiary focus-visible:ring-2 focus-visible:ring-accent-green"
                    >
              ...
              <button
                type="button"
                onClick={onProjectImport}
                className="flex min-h-[52px] items-center gap-3 rounded-xl px-3 py-2 text-left transition-colors hover:bg-bg-secondary focus-visible:ring-2 focus-visible:ring-accent-green"
              >
                <Upload className="h-4 w-4 shrink-0 text-accent-green" />
                <span className="min-w-0">
                  <span className="block text-sm font-bold text-text-primary">
                    {L4(lang, { ko: "파일 불러오기", en: "Import Files", ja: "ファイル読み込み", zh: "导入文件" })}
                  </span>
                  <span className="block text-[11px] leading-5 text-text-tertiary">
                    {L4(lang, {
                      ko: "기존 원고나 설정집을 가져옵니다.",
                      en: "Bring in an existing manuscript or setting book.",
                      ja: "既存の原稿や設定集を取り込みます。",
                      zh: "导入已有稿件或设定集。",
                    })}
                  </span>
