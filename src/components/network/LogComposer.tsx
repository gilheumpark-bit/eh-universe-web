  ...
  return (
    <div className="premium-panel-soft p-5 md:p-6">
      <div className="grid gap-4 md:grid-cols-2">
        {showPlanetSelect ? (
          <label className="block">
            <div className="mb-2 text-sm text-text-secondary">{lang === "ko" ? "소속 행성" : "Planet"}</div>
            <select
            ...
            >
              <option value="">{lang === "ko" ? "행성을 선택하세요" : "Select a planet"}</option>
        ...
        <label className="block md:col-span-2">
          <div className="mb-2 text-sm text-text-secondary">{lang === "ko" ? "제목" : "Title"}</div>
          <input
            value={value.title}
            disabled={disabled}
        ...
        <label className="block md:col-span-2">
          ...
          <select
            ...
            className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-text-primary outline-none"
          >
            <option value="">{lang === "ko" ? "선택 안 함" : "No follow-up verdict"}</option>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-end gap-3">
        ...
        >
          {submitting ? (lang === "ko" ? "저장 중..." : "Saving...") : submitLabel}
        </button>
      </div>
    </div>
  );
