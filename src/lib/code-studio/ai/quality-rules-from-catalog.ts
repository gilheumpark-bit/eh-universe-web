...
const QUALITY_SEVERITY_ORDER: IsoQuality[] = [
  'Security',
  'Reliability',
  'Performance',
  'Maintainability',
...
function sortBySeverity(patterns: GoodPatternMeta[]): GoodPatternMeta[] {
...
// IDENTITY_SEAL: PART-1 | role=severity-ordering | inputs=GoodPatternMeta[] | outputs=sortedPatterns

// ============================================================
// PART 2 — Contextual Filtering
// ============================================================

/** Detected file context to boost relevant rule categories. */
type FileContext = 'test' | 'security' | 'performance' | 'general';

/** Detect context from a file path (or empty string for general). */
function detectFileContext(filePath: string): FileContext {
...
function applyContextualBoost(
  ...
  if (context === 'general') return patterns;

  const contextQuality: IsoQuality | null =
    context === 'test' ? 'Reliability'
    : context === 'security' ? 'Security'
    : context === 'performance' ? 'Performance'
    : null;

  if (!contextQuality) return patterns;

  // Partition: boosted rules first, rest after
  const boosted: GoodPatternMeta[] = [];
  const rest: GoodPatternMeta[] = [];
...
function getExampleForRule(ruleId: string): string | null {
...
function selectPromptPatterns(): GoodPatternMeta[] {
  return GOOD_PATTERN_CATALOG.filter(
    (p) => p.signal === 'boost' && (p.confidence === 'high' || p.confidence === 'medium'),
  );
}
...
function groupByQuality(
  ...
  for (const q of QUALITY_SEVERITY_ORDER) {
    ...
    if (items.length > 0) map.set(q, items);
  }
  return map;
}
 ...
 */
export function buildQualityRulesPrompt(
  maxRules = 30,
  filePath = '',
  tokenBudget = 2000,
): string {
  ...
  for (const [quality, items] of grouped) {
    ...
    for (const item of items) {
      ...
      if (estimateTokens(currentText) > tokenBudget) break;
 ...
 */
export function buildFPSuppressionPrompt(filePath = ''): string {
  const context = detectFileContext(filePath);

  let suppressors = GOOD_PATTERN_CATALOG.filter(
