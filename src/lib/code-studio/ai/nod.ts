...
NOD: "데이터가 아직 로딩되지 않은 상태에서 .map()을 호출했습니다. API 응답이 오기 전에 화면을 그리려고 할 때 발생합니다. 해결: 데이터가 있을 때만 .map()을 실행하도록 조건부 렌더링(data && data.map(...))을 추가하세요."
...
Diagnosis mode example:
User: "TypeError: Cannot read properties of undefined (reading 'map')"
NOD: "The code tried to call .map() on data that hasn't loaded yet. This happens when the UI renders before the API response arrives. Fix: Add conditional rendering (data && data.map(...)) so .map() only runs when data exists."
