...
> = {
  ...
  en: {
    ...
    reportVersion: 'Report v',
    limitationVersion: 'Limitation v',
  },
  ja: {
    key: '項目',
    value: '内容',
...
export function renderCertificateMarkdown(
  ...
  const markdownLabels = MARKDOWN_LABELS[language];

  // 섹션 빌드
  const sectionsMd = visibleSections
    .map((section) => {
      const tableHeader = `| ${escapeMarkdown(markdownLabels.key)} | ${escapeMarkdown(markdownLabels.value)} |\n| --- | --- |`;
  ...
  if (cert.sealNumber) {
    visualBlocks.push(`## ${escapeMarkdown(labels.processSeal)}\n\n\`${escapeMarkdown(cert.sealNumber)}\``);
  }
  ...
  if (cert.originSummary) {
    ...
    visualBlocks.push(
      ...
      `| ${escapeMarkdown(markdownLabels.category)} | % |\n| --- | --- |\n` +
  ...
  if (cert.workSessions && cert.workSessions.length > 0) {
    const rows = cert.workSessions
      ...
      .join('\n');
    visualBlocks.push(
      `## ${escapeMarkdown(labels.workSessions)}\n\n| ${escapeMarkdown(markdownLabels.date)} | ${escapeMarkdown(markdownLabels.title)} |\n| --- | --- |\n${rows}`,
    );
  }
...
- **${escapeMarkdown(markdownLabels.view)}**: ${escapeMarkdown(view)}
- **${escapeMarkdown(markdownLabels.generatedAt)}**: ${escapeMarkdown(cert.generatedAt)}
- **${escapeMarkdown(markdownLabels.generatedBy)}**: ${escapeMarkdown(cert.generatedBy)}
- **${escapeMarkdown(markdownLabels.reportVersion)}**: ${escapeMarkdown(cert.reportVersion)}
- **${escapeMarkdown(markdownLabels.limitationVersion)}**: ${escapeMarkdown(cert.limitationTextVersion)}
