...
/**
 * [2026-05-08 — 시장 분석 4차] Dual track XLIFF Export.
 *
 * 한 trans-unit 에 두 alt-trans (Faithful + Market) 동시 포함.
 * 번역가가 Trados/memoQ 에서 두 번역본을 비교하며 검토 가능.
 *
 * 호환성: 기본 target 은 Market (출판본 기본). Faithful 은 alt-trans state-qualifier='leveraged-mt'.
 */
export interface DualXliffSegment {
  source: string;
  faithful: string;
  market: string;
  status?: 'pending' | 'reviewed' | 'final';
  comment?: string;
}

export function exportDualXLIFF(
  segments: DualXliffSegment[],
  sourceLang: string,
  targetLang: string,
  fileName: string = 'translation-dual',
): string {
  const units = segments.map((seg, i) => `    <trans-unit id="${i + 1}" xml:space="preserve">
      <source xml:lang="${sourceLang}">${escapeXml(seg.source)}</source>
      <target xml:lang="${targetLang}" state="${seg.status === 'final' ? 'final' : seg.status === 'reviewed' ? 'translated' : 'new'}">${escapeXml(seg.market)}</target>
      <alt-trans alttranstype="other" extype="source-faithful">
        <target xml:lang="${targetLang}">${escapeXml(seg.faithful)}</target>
      </alt-trans>
${seg.comment ? `      <note>${escapeXml(seg.comment)}</note>` : ''}
    </trans-unit>`).join('\n');
...
<xliff version="1.2" xmlns="urn:oasis:names:tc:xliff:document:1.2">
  <file source-language="${sourceLang}" target-language="${targetLang}" datatype="plaintext" original="${escapeXml(fileName)}">
    <header>
      <note>Dual-track translation: market (target) + faithful (alt-trans). Generated by Loreguard Translation & Localization Workspace.</note>
    </header>
    <body>
${units}
    </body>
...
export function exportTBX(
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>
<martif type="TBX">
  <martifHeader><fileDesc><sourceDesc><p>Loreguard Translation & Localization Workspace</p></sourceDesc></fileDesc></martifHeader>
  <text><body>
${entries}
  </body></text>
</martif>`;
