/**
 * creative-domain-prompts/index.ts — 4 도메인 dispatcher.
 *
 * 창작 구조화 생성 호출 측 (geminiStructuredTaskService) 의 단일 진입점.
 * AppLanguage → CreativeDomain → CreativeDomainPrompts 매핑을 통해
 ...
 *
 * 사용:
 *   import { getDomainPrompts } from '@/lib/ai/creative-domain-prompts';
 *   const prompts = getDomainPrompts(language); // AppLanguage
 *   const prompt = prompts.buildCharactersPrompt({ ... });
 */

import type { AppLanguage } from '@/lib/studio-types';
import type { CreativeDomain, CreativeDomainPrompts } from './types';
...
const DOMAIN_PROMPTS: Record<CreativeDomain, CreativeDomainPrompts> = {
 ...
 */
export function getDomainPromptsByDomain(domain: CreativeDomain): CreativeDomainPrompts {
