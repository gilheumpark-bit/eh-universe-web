...
// 전역 검색 모듈 단위 테스트.
// 외부 UI 모듈 import 금지 — 본 테스트는 global-search-index 만 임포트.
// ============================================================
...
const ITEMS: SearchableItem[] = [
  {
    ...
    body: '문서에는 창작 과정 기록과 IP 패키징 기준이 있다. universe 도메인의 핵심.',
    group: 'universe',
  },
  {
    id: 's-1',
  ...
  },
  {
    ...
    body: '창작 과정 기록과 검토 흐름을 묶어 출고 패키지로 만든다.',
    group: 'process',
  },
  {
    id: 'n-1',
    label: 'LearningGuard',
    body: '학습 과정 기록과 승인 흐름. universe 와 연결.',
    group: 'learning',
  },
  {
    id: 't-1',
...
describe('searchAll — 정상', () => {
  ...
  it('caseSensitive=true 시 일치 항목만', () => {
    ...
    // label 에는 'Loreguard Docs' 라 일치하지 않음.
    const exactLabelHit = sensitive.find((r) => r.item.label === 'Loreguard Docs');
    // u-1 은 body 매치로 포함되지만, 라벨 자체로는 매치 안됨.
