...
describe('contextItemsToAgentContext', () => {
  ...
  it('기준선 라벨도 registry 슬롯에 매핑', () => {
    const ctx = contextItemsToAgentContext([
      { tab: 'global', label: '세계관 전체 기준선', fact: 'W', details: '탑이 도시 한복판에 솟아오른 세계' },
      { tab: 'global', label: '메인 시나리오 기준선', fact: 'M', details: '탑의 통제권을 두고 갈등한다' },
      { tab: 'current-episode', label: '1화 씬시트', fact: 'S', details: '첫 충돌이 발생한다' },
      { tab: 'current-episode', label: '연출 기준선', fact: 'D', details: '건조한 긴장감' },
    ]);
    expect(ctx['world-book']).toContain('탑이 도시 한복판에 솟아오른 세계');
    expect(ctx['story-summary']).toContain('탑의 통제권을 두고 갈등한다');
    expect(ctx['scene-sheet']).toContain('첫 충돌이 발생한다');
    expect(ctx['act-guide']).toContain('건조한 긴장감');
  });
});
