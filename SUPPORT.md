...
| ⚖️ **라이선스 문의** | [gilheumpark@gmail.com](mailto:gilheumpark@gmail.com) |
...
- 재현 단계 (번호 매기기)
- 브라우저 + OS 버전
- 페이지 경로 (`/studio`, `/translation-studio` 등)
- 콘솔 에러 스크린샷
...
- 선택: GitHub 백업 (`Octokit`, OAuth/PAT 1분 설정 — 1클릭 푸시 + 자동 동기화 토글 옵션)
- 선택: Firestore 클라우드 동기화 (Settings → 고급)
- 상세: [/privacy](https://ehsu.app/privacy)

### Q. 라이선스는 어떻게 되나요?
- **소프트웨어** (엔진·파이프라인·UI·CLI): **UNLICENSED / Proprietary / All rights reserved**
- **세계관 원본·샘플 자료**: 소프트웨어와 분리되며, 파일 또는 문서에 별도 표기된 조건을 따릅니다.
- 이전 릴리스(커밋 `414fe9ea` 및 그 이전)를 별도 라이선스로 받은 수령자의 권리는 해당 시점의 고지에 따릅니다.
- 라이선스 문의: `gilheumpark@gmail.com`
