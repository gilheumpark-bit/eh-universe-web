# NOA Rules — CLAUDE.md
# 이 파일은 Claude Code 시작 시 자동 로드됨. 모든 규칙은 선택이 아닌 강제 적용.
...
**불확실 코드 — 자동 마킹**
```python
# [확인 필요] Redis 7.x 호환성 미검증
cache.set(key, value, ex=TTL)

# [미검증 API] 사용 전 존재 확인 필요
# some_library.maybe_function(args)
```
...
**수리 수령증**
```
[수리 L1] auth.py:45 — None 체크 추가 → 실패
[수리 L2] auth.py — 호출 체인 재구성 → 실패
[수리 L3] auth.py — 전체 재작성 → 성공
```
