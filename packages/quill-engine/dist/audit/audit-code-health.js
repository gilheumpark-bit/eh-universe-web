...
// ============================================================
export function auditOperations(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    ...
    let consoleCount = 0;
    for (const f of ctx.files) {
        if (f.path.includes('__tests__') || f.path.includes('.test.'))
            continue;
        if (consoleSkipPaths.some(s => f.path.endsWith(s)))
            continue;
        const lines = f.content.split('\n');
    ...
    let todoCount = 0;
    for (const f of ctx.files) {
        if (todoSkipPaths.some(s => f.path.includes(s)))
            continue;
    ...
    if (dupeCount <= dupeThreshold)
        passed++;
    // Check 6: Dead code (unused exports, unreachable code, etc)
    checks++;
    const fileNodes = ctx.files.map(f => ({ name: f.path, type: 'file', id: f.path, content: f.content }));
...
// ============================================================
export function auditComplexity(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    ...
    let longFunctions = 0;
    for (const f of ctx.files) {
        if (!f.language.includes('typescript') && !f.language.includes('tsx') && !f.language.includes('javascript'))
            continue;
        const lines = f.content.split('\n');
...
// ============================================================
export function auditArchitecture(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    ...
    if (circularRisk <= circularThreshold)
        passed++;
...
export function auditDependencies(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    // Find package.json
