...
// ============================================================
export function auditDesignSystem(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    ...
    for (const f of ctx.files) {
        if (!f.path.includes('/components/') && !f.path.includes('/app/'))
            continue;
        if (f.path.endsWith('.css'))
            continue;
        if (colorSkipPatterns.some(p => f.path.includes(p)))
            continue;
        const matches = f.content.match(/#[0-9a-fA-F]{6}\b/g);
        if (matches)
            hardcodedColors += matches.length;
    }
    ...
    const componentCount = ctx.files.filter(f => (f.path.includes('/components/') || f.path.includes('/app/')) && f.language === 'tsx').length;
    ...
    for (const f of ctx.files) {
        ...
        for (const m of twMatches) {
            if (!standardTwRounded.has(m[1]))
                radiusValues.add(`tw:${m[1]}`);
        }
    }
...
// ============================================================
export function auditAccessibility(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    ...
    for (const f of tsxFiles) {
        ...
        if (hasErrorDisplay && !hasDescribedby)
            errorNoDescribedby++;
    }
...
// ============================================================
export function auditUXQuality(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    const tsxFiles = ctx.files.filter(f => f.language === 'tsx');
    ...
    let errorDisplays = 0;
    for (const f of tsxFiles) {
        if (/error.*message|Error.*display|alert.*error|toast.*error/i.test(f.content))
            errorDisplays++;
    }
    if (errorDisplays >= 3) {
        passed++;
    ...
    let emptyStates = 0;
    for (const f of tsxFiles) {
        if (/empty.*state|no.*items|no.*data|없습니다|비어\s*있|아직.*없|생성하세요|시작하세요|not found|no results/i.test(f.content))
            emptyStates++;
    }
    if (emptyStates >= 5) {
        passed++;
