...
// ============================================================
export function auditSecurity(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    ...
    for (const f of ctx.files) {
        if (f.path.includes('node_modules') || f.path.includes('__tests__'))
            continue;
        if (evalSkipPaths.some(s => f.path.includes(s)))
            continue;
    ...
    for (const f of ctx.files) {
        if (f.path.includes('.env') || f.path.includes('node_modules'))
            continue;
    ...
    let xssRisk = 0;
    for (const f of ctx.files) {
        if (xssSkipPaths.some(p => f.path.includes(p)))
            continue;
        const lines = f.content.split('\n');
        for (const line of lines) {
            if (/audit:safe/.test(line))
                continue;
...
// ============================================================
export function auditPerformance(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    ...
    for (const f of ctx.files) {
        ...
        let loopDepth = 0;
        for (const line of lines) {
            if (/\bfor\s*\(|\bwhile\s*\(|\.forEach\s*\(/.test(line))
                loopDepth++;
    ...
    for (const f of ctx.files) {
        ...
        for (const lib of heavyLibs) {
            ...
            const importLines = f.content.split('\n').filter(l => (l.includes(`from '${lib}`) || l.includes(`from "${lib}`)) && !l.includes('import type'));
...
// ============================================================
export function auditAPIHealth(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    const apiRoutes = ctx.files.filter(f => f.path.includes('/api/') && f.path.endsWith('route.ts'));
    ...
    let hasTimeout = 0;
    for (const f of apiRoutes) {
        if (/timeout|AbortSignal|signal/i.test(f.content))
            hasTimeout++;
    }
...
// ============================================================
export function auditEnvConfig(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    ...
    const hasEnvModule = ctx.files.some(f => /env\.ts|env\.mjs/.test(f.path) && /validateEnv|ENV_VARS/.test(f.content));
    ...
    for (const f of ctx.files) {
        if (!f.path.includes('/api/') && !f.path.includes('server'))
            continue;
    ...
    checks++;
    const hasEnvValidation = ctx.files.some(f => /env\.ts|env\.mjs|validateEnv|zod.*env|t3-env/i.test(f.path) ||
