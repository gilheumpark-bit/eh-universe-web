...
function fid(area) { return `${area}-${++findingCounter}`; }
...
// ============================================================
export function auditTesting(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    const testFiles = ctx.files.filter(f => /\.(test|spec)\.(ts|tsx|js)$/.test(f.path));
    const srcFiles = ctx.files.filter(f => !f.path.includes('__tests__') && !f.path.includes('.test.') && !f.path.includes('.spec.') &&
...
// ============================================================
export function auditErrorHandling(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
...
// ============================================================
export function auditFeatureCompleteness(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    ...
    for (const [name, srcPath] of exportedNames) {
        const importedAnywhere = ctx.files.some(f => f.path !== srcPath && new RegExp(`\\b${name}\\b`).test(f.content));
        if (!importedAnywhere)
            unusedExports++;
    }
    ...
    if (unusedExports <= unusedThreshold) {
        passed++;
    }
    else {
        findings.push({
    ...
    let mockInProd = 0;
    for (const f of ctx.files) {
        if (f.path.includes('__tests__') || f.path.includes('.test.') || f.path.includes('.spec.'))
            continue;
        // Skip audit/translation/article files — they describe concepts, not implement mocks
        if (f.path.includes('audit') || f.path.includes('translations') || f.path.includes('articles'))
            continue;
...
// ============================================================
export function auditDocumentation(ctx) {
    const findings = [];
    let checks = 0;
    let passed = 0;
    ...
    if (ctx.files.some(f => /readme\.md$/i.test(f.path)) || ctx.projectName) {
        passed++;
    }
    else {
    ...
    checks++;
    if (ctx.files.some(f => /changelog|releases|CHANGELOG/i.test(f.path)) || ctx.projectName) {
        passed++;
    }
    else {
