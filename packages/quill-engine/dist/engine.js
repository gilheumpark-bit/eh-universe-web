...
// ============================================================
export function analyzeWithProgram(filePaths, targetFile, code) {
    const findings = [];
    const scopes = [];
    const enginesUsed = ['typescript-ast'];
    ...
    try {
        ...
        host.readFile = (f) => {
            if (f === targetFile || f.endsWith(targetFile))
                return codeToCheck;
            try {
    ...
    function visit(node, depth) {
        ...
        if (ts.isFunctionDeclaration(node) || ts.isFunctionExpression(node) ||
            ...
            if ('parameters' in node) {
                for (const p of node.parameters) {
                    if (ts.isIdentifier(p.name))
                        scope.declared.add(p.name.text);
                }
            }
            scopes.push(scope);
            ...
            // 파라미터 과다
            if ('parameters' in node && node.parameters.length > 5) {
                addFinding({
                    ruleId: 'CMX-002', line: lineOf(node),
                    message: `파라미터 ${node.parameters.length}개 — 5개 초과`,
                    severity: 'warning', confidence: 'high',
        ...
        if ((ts.isFunctionDeclaration(node) || ts.isArrowFunction(node) || ts.isMethodDeclaration(node)) &&
            ...
            ts.forEachChild(node, function checkAwait(child) {
                if (ts.isAwaitExpression(child))
                    hasAwait = true;
                if (!hasAwait)
                    ts.forEachChild(child, checkAwait);
            });
            if (!hasAwait) {
        ...
        if (ts.isConditionalExpression(node) && ts.isConditionalExpression(node.whenTrue)) {
            if (ts.isConditionalExpression(node.whenTrue.whenTrue)) {
...
// ============================================================
export function analyzeWithEsquery(code) {
    try {
        ...
        for (const node of evalCalls) {
            findings.push({
                ruleId: 'SEC-006', line: node.loc?.start?.line ?? 1,
                message: 'eval() 호출 — 보안 위험',
                severity: 'critical', confidence: 'high',
        ...
        if (tripleLoop.length > 0) {
            findings.push({
                ruleId: 'PRF-002', line: tripleLoop[0].loc?.start?.line ?? 1,
...
function runTypMorphDetectors(code, fileName) {
    const out = [];
    try {
        ...
        for (const detector of registry.getDetectors()) {
            if (!TYP_RULE_IDS.has(detector.ruleId))
                continue;
...
function mergeFindingsDedupe(base, extra) {
...
export function runQuillEngine(code, fileName = 'temp.ts') {
    ...
    try {
        ...
        if (!result.enginesUsed.includes('esquery'))
            result.enginesUsed.push('esquery');
    }
