 ...
 */
export const cmx010Detector = {
    ruleId: 'CMX-010',
    detect: (sourceFile) => {
        const findings = [];
        const MAX_DEPTH = 3;
        const reported = new Set(); // avoid duplicates by line
        function measureTernaryDepth(node) {
        ...
        sourceFile.forEachDescendant(node => {
            if (node.getKind() === SyntaxKind.ConditionalExpression) {
                ...
                if (parent && parent.getKind() === SyntaxKind.ConditionalExpression)
                    return;
