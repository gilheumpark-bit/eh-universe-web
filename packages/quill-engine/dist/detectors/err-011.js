...
export const err011Detector = {
    ...
    detect: (sourceFile) => {
        ...
        sourceFile.forEachDescendant((node) => {
            ...
            if (!binding)
                return;
            if (cc.getBlock().getStatements().length === 0)
                return;
            if (catchHasTypeNarrowing(cc))
                return;
            const t = cc.getBlock().getText().replace(/\s+/g, ' ').trim();
            if (t === `throw ${binding};` || t === `throw ${binding}`)
                return;
            findings.push({
                line: cc.getStartLineNumber(),
                message: 'catch에서 instanceof 등 타입 구분이 없습니다. 필요 시 Error 타입 구분을 검토하세요 (ERR-011).',
            });
        });
        return findings;
    },
