...
const CAP = 8;
export function pushUnique(out, seen, line, message) {
    if (out.length >= CAP)
        return;
...
function binaryPlusHasIdentifier(node) {
    let found = false;
    function walk(n) {
        if (found)
            return;
        if (n.getKind() === SyntaxKind.Identifier) {
            const id = n.getText();
            if (!['require', 'undefined', 'null', 'true', 'false'].includes(id))
                found = true;
            return;
        }
...
export function detectSec001(sourceFile) {
    const findings = [];
    const seen = new Set();
...
export function detectSec002(sourceFile) {
    const findings = [];
    const seen = new Set();
    sourceFile.forEachDescendant((node) => {
...
export function detectSec003(sourceFile) {
    const findings = [];
    const seen = new Set();
    ...
    sourceFile.forEachDescendant((node) => {
        ...
        if (expr.getKind() === SyntaxKind.Identifier)
            ...
            name = expr.getLastChildByKind(SyntaxKind.Identifier)?.getText() ?? '';
        if (!riskyCallee.test(name))
            return;
...
/** SEC-004: 경로에 ../ 포함 */
export function detectSec004(sourceFile) {
    const findings = [];
    const seen = new Set();
...
export function detectSec005(sourceFile) {
    const findings = [];
    const seen = new Set();
...
export function detectSec006(_sourceFile) {
    return [];
}
/** SEC-007: __proto__ / prototype 오염 패턴 */
export function detectSec007(sourceFile) {
    const findings = [];
    const seen = new Set();
    sourceFile.forEachDescendant((node) => {
...
export function detectSec008(sourceFile) {
    const findings = [];
    const seen = new Set();
    const redos = /\([^)]*[\+\*][^)]*\)[\+\*]|\[[^\]]*[\+\*][^\]]*\][\+\*]/;
    const text = sourceFile.getFullText();
...
export function detectSec009(sourceFile) {
    const findings = [];
    const seen = new Set();
    const patterns = [
        /\bpassword\s*=\s*['"`][^'"`]+['"`]/i,
...
export function detectSec010(sourceFile) {
    const findings = [];
    const seen = new Set();
...
/** SEC-011: MD5/SHA1 */
export function detectSec011(sourceFile) {
    const findings = [];
    const seen = new Set();
    ...
    lines.forEach((line, i) => {
        if (re.test(line))
            pushUnique(findings, seen, i + 1, 'MD5/SHA1 해시 — 약한 해시 의심');
    });
    return findings;
}
/** SEC-012: DES/RC4 등 */
export function detectSec012(sourceFile) {
    const findings = [];
    const seen = new Set();
    ...
    lines.forEach((line, i) => {
        if (re.test(line))
            pushUnique(findings, seen, i + 1, 'DES/RC4 등 취약 알고리즘 명시 의심');
    });
    return findings;
}
/** SEC-013: jwt.decode / verify 미사용 휴리스틱 */
export function detectSec013(sourceFile) {
    const findings = [];
    const seen = new Set();
    const full = sourceFile.getFullText();
    if (/\bjwt\.verify\s*\(/.test(full))
        return findings;
    sourceFile.forEachDescendant((node) => {
        if (node.getKind() !== SyntaxKind.CallExpression)
            return;
...
/** SEC-014: 세션 ID가 URL에 */
export function detectSec014(sourceFile) {
    const findings = [];
    const seen = new Set();
...
export function detectSec015(sourceFile) {
    const findings = [];
    const seen = new Set();
    sourceFile.forEachDescendant((node) => {
        if (node.getKind() !== SyntaxKind.CallExpression)
        ...
        const c = node;
        const ex = c.getExpression().getText();
        if (!/cookie|res\.cookie|setHeader/i.test(ex))
            return;
        const args = c.getArguments();
        if (args.length < 2)
            return;
        const opt = args[args.length - 1];
        if (opt.getKind() !== SyntaxKind.ObjectLiteralExpression)
            return;
...
export function detectSec016(sourceFile) {
    const findings = [];
    const seen = new Set();
    const lines = sourceFile.getFullText().split('\n');
    lines.forEach((line, i) => {
    ...
    sourceFile.forEachDescendant((node) => {
        ...
        if (!/\bcors\b$/.test(callee) && !/\.cors$/.test(callee))
            return;
        const a0 = c.getArguments()[0];
        if (!a0 || a0.getKind() !== SyntaxKind.ObjectLiteralExpression)
            return;
        const t = a0.getText();
...
export function detectSec017(sourceFile) {
    const findings = [];
    const seen = new Set();
    sourceFile.forEachDescendant((node) => {
        if (node.getKind() !== SyntaxKind.CallExpression)
            return;
        const c = node;
        if (!c.getExpression().getText().endsWith('postMessage'))
            return;
        const args = c.getArguments();
...
/** SEC-018: 민감 값 로그 */
export function detectSec018(sourceFile) {
    const findings = [];
    const seen = new Set();
    ...
    lines.forEach((line, i) => {
        if (re.test(line))
            pushUnique(findings, seen, i + 1, '콘솔에 민감 키워드 로그 — 민감 데이터 로그 의심');
    });
    return findings;
}
/** SEC-019: stack 노출 */
export function detectSec019(sourceFile) {
    const findings = [];
    const seen = new Set();
    ...
    lines.forEach((line, i) => {
        if (re.test(line))
            pushUnique(findings, seen, i + 1, '응답에 err.stack 전달 — stack trace 노출 의심');
    });
    return findings;
}
// ============================================================
...
// ============================================================
/** SEC-020: http:// 리터럴 */
export function detectSec020(sourceFile) {
    const findings = [];
    const seen = new Set();
    const lines = sourceFile.getFullText().split('\n');
    lines.forEach((line, i) => {
...
export function detectSec021(sourceFile) {
    const findings = [];
    const seen = new Set();
    ...
    lines.forEach((line, i) => {
        if (re.test(line))
            pushUnique(findings, seen, i + 1, 'localStorage에 민감 키 저장 의심');
    });
    return findings;
}
/** SEC-022: debugger + console.debug 다량 */
export function detectSec022(sourceFile) {
    const findings = [];
    const seen = new Set();
    sourceFile.forEachDescendant((node) => {
...
/** SEC-023: 사설 IP */
export function detectSec023(sourceFile) {
    const findings = [];
    const seen = new Set();
    ...
    lines.forEach((line, i) => {
        if (/^\s*\/\//.test(line))
            return;
        if (re.test(line))
            pushUnique(findings, seen, i + 1, '사설 IP 대역 하드코딩 의심');
    });
    return findings;
}
/** SEC-024: IDOR 패턴 req.params → 쿼리 */
export function detectSec024(sourceFile) {
    const findings = [];
    const seen = new Set();
    ...
    lines.forEach((line, i) => {
        if (re.test(line))
            pushUnique(findings, seen, i + 1, 'req.params.id 직접 DB 조회 — IDOR 검토 필요');
    });
    return findings;
}
/** SEC-025: Express 라우트 2인자 (미들웨어 없음) 휴리스틱 */
export function detectSec025(sourceFile) {
    const findings = [];
    const seen = new Set();
    sourceFile.forEachDescendant((node) => {
        if (node.getKind() !== SyntaxKind.CallExpression)
        ...
        if (!/^(get|post|put|patch|delete)$/.test(prop))
            return;
        const base = pa.getExpression().getText();
        if (!/^(app|router)$/.test(base))
            return;
        const args = c.getArguments();
        if (args.length !== 2)
            return;
        const pathArg = args[0].getText();
        if (!/\/(api|admin|v\d+)/i.test(pathArg))
            return;
...
export function detectSec026(sourceFile) {
    const findings = [];
    const seen = new Set();
    const full = sourceFile.getFullText();
    if (!/['"]use client['"]/.test(full))
        return findings;
    if (!/\b(role|isAdmin|permissions?)\b/i.test(full))
        return findings;
    if (/getServerSideProps|route\.ts|middleware/i.test(full))
        return findings;
    pushUnique(findings, seen, 1, "'use client' 컴포넌트에서만 권한 검사 — 서버 검증 필요 여부 검토");
    return findings;
}
/** SEC-027: POST에 CSRF 관련 토큰/미들웨어 부재 휴리스틱 */
export function detectSec027(sourceFile) {
    const findings = [];
    const seen = new Set();
    const full = sourceFile.getFullText();
    ...
    sourceFile.forEachDescendant((node) => {
        ...
        if (prop !== 'post' && prop !== 'put' && prop !== 'patch' && prop !== 'delete')
            return;
        if (!/^(app|router)$/.test(pa.getExpression().getText()))
            return;
        ...
        if (!/\/(api|auth|account|admin)/i.test(pathArg))
            return;
