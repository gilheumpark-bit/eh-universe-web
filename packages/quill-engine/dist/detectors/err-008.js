...
export const err008Detector = {
    ...
    detect: (sourceFile) => {
        ...
        sourceFile.forEachDescendant((node) => {
            ...
            if (!sensitiveLiteralInExpression(expr) && !sensitiveIdentifierInThrownExpr(expr))
                return;
            findings.push({
                line: ts.getStartLineNumber(),
                message: 'throw 메시지에 비밀번호·토큰 등 민감 키워드가 포함된 것으로 보입니다. 사용자/로그 노출 경로를 검토하세요 (ERR-008).',
            });
        });
        return findings;
    },
