...
export function hasQuestionDotToken(node) {
    const cn = node.compilerNode;
    return !!cn?.questionDotToken;
}
/** a?.b.c 체인에서 최초 ?. 여부 */
export function expressionRootHasOptionalChain(node) {
    let cur = node;
    for (let depth = 0; depth < 96 && cur; depth++) {
        if (hasQuestionDotToken(cur))
            return true;
        const k = cur.getKind();
...
export function typeHasNull(t) {
    try {
        if (t.isNull())
            return true;
        if (t.isUnion())
            return t.getUnionTypes().some((u) => u.isNull());
    }
    catch {
        /* checker 없음 */
    }
...
export function typeHasUndefined(t) {
    try {
        if (t.isUndefined())
            return true;
        if (t.isUnion())
            return t.getUnionTypes().some((u) => u.isUndefined());
    }
    catch {
        /* ignore */
    }
...
export function isInTryBlockOnly(node) {
    const ts = node.getFirstAncestorByKind(SyntaxKind.TryStatement);
    if (!ts)
        return false;
    const tb = ts.getTryBlock();
    return !!tb && node.getAncestors().includes(tb);
}
function isBooleanLikeExpr(node) {
...
export function isSuspiciousBarBar(be) {
    if (be.getOperatorToken().getKind() !== SyntaxKind.BarBarToken)
        return false;
    return !isBooleanLikeExpr(be.getLeft());
}
/** 무한 루프: while(true) / for(;;) 본문에 break/return/throw 없음 */
export function loopBodyLacksExit(body) {
    if (!body)
        return false;
    let hasExit = false;
...
export function isWhileTrueOrForEver(node) {
    ...
    if (node.getKind() === SyntaxKind.ForStatement) {
        const f = node;
        if (!f.getInitializer() && !f.getCondition() && !f.getIncrementor())
            return true;
    }
    return false;
}
/** case fall-through: break/return/throw 없이 다음 case로 이어짐 */
export function findSwitchFallThroughs(sw) {
    const lines = [];
    const clauses = sw.getCaseBlock().getClauses();
    for (let i = 0; i < clauses.length - 1; i++) {
        const cur = clauses[i];
        if (cur.getKind() === SyntaxKind.DefaultClause)
            continue;
        const cc = cur;
        ...
        if (k === SyntaxKind.Block) {
            const inner = last.getStatements().slice(-1)[0];
            if (inner &&
...
export function findUnreachableInBlocks(sf) {
    const out = [];
    sf.forEachDescendant((node) => {
        ...
        for (const st of stmts) {
            ...
            if (k === SyntaxKind.ReturnStatement || k === SyntaxKind.ThrowStatement)
                terminalHit = true;
            if (k === SyntaxKind.ContinueStatement || k === SyntaxKind.BreakStatement)
                terminalHit = true;
        }
    });
    return out;
...
/** 항상 같은 결과인 if 조건 (리터럴) */
export function isDeadBranchCondition(expr) {
    ...
    if (k === SyntaxKind.NumericLiteral) {
        const v = expr.getLiteralValue?.() ?? Number(expr.getText());
        if (v === 0)
            return 'always-false';
    ...
    if (k === SyntaxKind.Identifier && expr.getText() === 'undefined')
        return 'always-false';
    return null;
}
...
export function isParseIntCall(node) {
    if (node.getKind() !== SyntaxKind.CallExpression)
        return false;
    const c = node;
    const ex = c.getExpression();
    return ex.getKind() === SyntaxKind.Identifier && ex.getText() === 'parseInt';
}
/** arr[arr.length] 형태 (대개 off-by-one / undefined) */
export function isArrayLengthAsIndex(ea) {
    const arg = ea.getArgumentExpression();
    if (!arg || arg.getKind() !== SyntaxKind.PropertyAccessExpression)
        return false;
    const inner = arg;
    if (inner.getName() !== 'length')
        return false;
