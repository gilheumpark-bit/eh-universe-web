...
function computeStaticMetrics(code) {
    ...
    for (const name of fnNames) {
        ...
        if (bodyMatch && bodyMatch[1].includes(name + '('))
...
const STRESS_SYSTEM = 'You are a senior performance engineer. Run a VIRTUAL SIMULATION using the pre-computed static metrics AND your own code analysis.\n\n' +
...
function buildMetricsBlock(m) {
    const warnings = [];
    if (m.nestedLoopDepth >= 2)
        warnings.push(`⚠ O(n^${m.nestedLoopDepth}) 중첩 루프 탐지`);
    if (m.asyncWithoutTryCatch > 0)
        warnings.push(`⚠ try-catch 없는 await ${m.asyncWithoutTryCatch}건`);
    if (m.eventListenerCount > 3)
        warnings.push(`⚠ addEventListener ${m.eventListenerCount}건 — 메모리 릭 위험`);
    if (m.recursiveFunctionCount > 0)
        warnings.push(`⚠ 재귀 함수 ${m.recursiveFunctionCount}건 — 스택 오버플로우 위험`);
    if (m.cyclomaticEstimate > 20)
        warnings.push(`⚠ 분기 복잡도 ${m.cyclomaticEstimate} — 고복잡도`);
    return [
...
export async function analyzeStress(code, fileName, scenario, signal) {
    ...
    try {
        ...
        const grade = errorRate < 0.01 && avgMs < 100 ? 'A' :
...
export async function runStressReport(code, fileName, scenarios, signal) {
