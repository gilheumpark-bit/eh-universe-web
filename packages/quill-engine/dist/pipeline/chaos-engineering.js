...
function computeResilienceMetrics(code) {
...
function buildResilienceBlock(m) {
    const warnings = [];
    if (m.catchWithoutHandler > 0)
        warnings.push(`⚠ 빈 catch 블록 ${m.catchWithoutHandler}건 — 에러 삼킴`);
    if (m.fetchWithoutTimeout > 0)
        warnings.push(`⚠ timeout 없는 fetch ${m.fetchWithoutTimeout}건 — 무한 대기 위험`);
    if (m.retryPatterns === 0 && m.tryCatchBlocks > 0)
        warnings.push(`⚠ retry/backoff 패턴 0건 — 1회 실패 시 포기`);
    if (m.resourceCleanup === 0)
        warnings.push(`⚠ 리소스 정리(finally/cleanup) 0건`);
    return [
...
// ============================================================
const CHAOS_SYSTEM = 'You are a chaos engineering expert. Run a VIRTUAL SIMULATION using the pre-computed resilience metrics AND your own code analysis.\n\n' +
...
export async function simulateFailure(code, fileName, scenario, signal) {
...
export async function runChaosReport(code, fileName, scenarios, signal) {
