...
function findUnreachableCode(content, filePath, fileId) {
    const findings = [];
...
function findUnusedVariables(content, filePath, fileId) {
    const findings = [];
    const lines = content.split('\n');
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (varName.startsWith('_'))
            continue; // underscore prefix = intentionally unused
...
function findUnusedImports(content, filePath, fileId) {
    const findings = [];
    const lines = content.split('\n');
...
export function scanDeadCode(nodes, prefix = '') {
    const allFiles = [];
    function flatten(nodeList, pfx) {
