...
function lintRule1_ContrastTokens(code) {
    const issues = [];
...
function lintRule2_RawTailwindColors(code) {
    const issues = [];
...
function lintRule3_YellowWarning(code) {
    const issues = [];
...
function lintRule4_FocusVisible(code) {
    const issues = [];
...
function lintRule5_StatusCombination(code) {
    const issues = [];
...
function lintRule6_SpacingGrid(code) {
    const issues = [];
    let match;
...
function lintRule7_HexHardcoding(code) {
    const issues = [];
...
function lintRule8_TouchTarget(code) {
    const issues = [];
...
function lintRule9_TransitionAll(code) {
    const issues = [];
...
function lintRule10_ZIndex(code) {
    const issues = [];
...
function lintBonusComponentReuse(code) {
    const issues = [];
...
function lintRule11_ColorBlindPairs(code) {
    const issues = [];
...
// ============================================================
function lintRule12_PreciseTouchTarget(code) {
    const issues = [];
    const lines = code.split('\n');
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
...
function calcCR(l1, l2) {
    const lighter = Math.max(l1, l2);
    const darker = Math.min(l1, l2);
    return (lighter + 0.05) / (darker + 0.05);
}
function lintRule13_DualThemeCR(code) {
    const issues = [];
...
// ============================================================
function lintRule14_ResponsiveOverflow(code) {
    const issues = [];
 ...
 */
export function runDesignLint(code) {
 ...
 */
export function formatDesignLintReport(result) {
 ...
 */
export function checkTokenContrast(textToken, bgToken, threshold = 4.5) {
    const textL = TOKEN_L_VALUES[textToken];
    const bgL = TOKEN_L_VALUES[bgToken];
    if (!textL || !bgL)
        return null;
