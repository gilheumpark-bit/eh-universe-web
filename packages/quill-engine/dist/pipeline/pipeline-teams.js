...
export function runTeam1Simulation(code, _language, _fileName) {
    const findings = [];
...
function extractFunctions(code) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const fnMatch = trimmed.match(/(?:(?:export\s+)?(?:async\s+)?function\s+(\w+)|(?:const|let)\s+(\w+)\s*=\s*(?:async\s*)?\([^)]*\)\s*(?::[^=]*?)?\s*=>)/);
        if (fnMatch) {
...
export function runTeam2Generation(code, _language, _fileName) {
    const findings = [];
...
function checkUnusedImports(code) {
    const findings = [];
    const lines = code.split('\n');
    for (let i = 0; i < lines.length; i++) {
...
function checkAsyncPatterns(code) {
    const findings = [];
    const lines = code.split('\n');
...
function checkRuntimeCompat(code, lines) {
    const findings = [];
...
export function runTeam3Validation(code, _language, _fileName) {
    const findings = [];
    const lines = code.split('\n');
    for (let i = 0; i < lines.length; i++) {
...
// ============================================================
export function runTeam4SizeDensity(code, _language, _fileName) {
    const findings = [];
    const lines = code.split('\n');
...
// ============================================================
export function runTeam5AssetTrace(code, _language, _fileName) {
    const findings = [];
    const lines = code.split('\n');
    // Parse imports
    ...
    const { majors: fMajors, minors: fMinors } = findings.reduce((acc, f) => {
        if (f.severity === 'major')
            acc.majors++;
...
// ============================================================
export function runTeam6Stability(code, _language, _fileName) {
    const findings = [];
    const lines = code.split('\n');
    // Nested loops O(n^2)
    let loopDepth = 0;
    ...
    const { criticals: sCrits, majors: sMajors, minors: sMinors } = findings.reduce((acc, f) => {
...
export function runTeam7ReleaseIP(code, _language, _fileName) {
    const findings = [];
    const lines = code.split('\n');
...
export function runTeam8Governance(code, _language, _fileName) {
    const findings = [];
    const lines = code.split('\n');
    // Nesting depth
    let maxNesting = 0;
 ...
 * 파이프라인 최종 결과 반환 전 호출.
 */
export function filterTeamResultByScope(result, filePath) {
    const filtered = applyScopePolicyToFindings(result.findings, filePath);
    if (filtered.length === result.findings.length)
        return result;
