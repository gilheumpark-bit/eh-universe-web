...
export function calculateWeightedScore(stages, config) {
    let totalWeight = 0;
    let weightedSum = 0;
    for (const s of stages) {
        if (!config.enabledTeams.includes(s.team))
            continue;
...
export function getStatusFromScore(score, config) {
...
export function hasBlockingFailure(stages, config) {
...
export function generateFixes(findings, fileContents) {
    const fixes = [];
...
const STATUS_EMOJI = { pass: 'PASS', warn: 'WARN', fail: 'FAIL' };
const SEVERITY_LABELS = { critical: 'CRITICAL', major: 'MAJOR', minor: 'MINOR', info: 'INFO' };
function formatDate(ts) {
    const d = new Date(ts);
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')} ${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
}
export function generateReport(stages, timestamp) {
    const config = loadPipelineConfig();
    const overallScore = Math.round(calculateWeightedScore(stages.map((s) => ({ team: s.stage, score: s.score })), config));
...
export function getReviewChecklist(role = 'reviewer') {
...
export function compareRuns(runA, runB) {
    ...
    for (const stage of allStages) {
        ...
        if (bCount > aCount)
            newFindings += bCount - aCount;
        if (aCount > bCount)
            resolvedFindings += aCount - bCount;
    }
...
export function recordBeacon(event) {
    try {
...
export function getBeaconHistory() {
    try {
        const raw = localStorage.getItem(BEACON_KEY);
        return raw ? JSON.parse(raw) : [];
    }
...
export function computeEntropy(code) {
    ...
    return Math.max(0, Math.min(100, Math.round(density * 70 + Math.min(commentRatio, 0.2) * 100 + (1 - paddingRatio) * 30 - Math.max(0, commentRatio - 0.3) * 50)));
}
