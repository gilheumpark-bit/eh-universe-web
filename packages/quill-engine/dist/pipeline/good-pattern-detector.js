...
const DETECTION_RULES = [
    ...
    { catalogId: 'GQ-NM-002', test: (_, lines) => {
            ...
            for (let i = 0; i < lines.length; i++) {
                if (/\b(?:is|has|can|should|was|did|will)[A-Z]\w*\s*[:=?]/.test(lines[i]))
                    found.push(i + 1);
            }
    ...
    { catalogId: 'GQ-NM-008', test: (_, lines) => {
            const found = [];
            for (let i = 0; i < lines.length; i++) {
                if (/(?:function|const|let)\s+(?:get|set|create|update|delete|fetch|load|save|handle|process|validate|parse|render|build|init|compute|calculate|format|transform|convert|check|verify|ensure|assert)[A-Z]/.test(lines[i]))
                    found.push(i + 1);
            }
            return { count: found.length, lines: found.slice(0, 5) };
        } },
    ...
    { catalogId: 'GQ-TS-009', test: (_, lines) => {
            const found = [];
            for (let i = 0; i < lines.length; i++) {
                if (/\bis\s+\w/.test(lines[i]) && /:\s*\w+\s+is\s+\w/.test(lines[i]))
                    found.push(i + 1);
            }
    ...
    { catalogId: 'GQ-FN-004', test: (_, lines) => {
            const found = [];
            for (let i = 0; i < lines.length; i++) {
                if (/^\s*if\s*\(.*\)\s*(?:return|throw)\b/.test(lines[i]))
                    found.push(i + 1);
            }
            return { count: found.length, lines: found.slice(0, 5) };
        } },
    ...
    { catalogId: 'GQ-EH-003', test: (_, lines) => {
            ...
            for (let i = 0; i < lines.length; i++) {
                if (/\bcatch\s*\(/.test(lines[i])) {
                    ...
                    if (/\bthrow\b/.test(block) || /\breturn\b/.test(block) || /console\.(?:error|warn)/.test(block))
                        found++;
                }
            }
    ...
    { catalogId: 'GQ-NL-010', test: (_, lines) => {
            let found = 0;
            for (let i = 0; i < lines.length; i++) {
                if (/typeof\s+\w+\s*(?:===|!==)\s*['"]/.test(lines[i]))
                    found++;
                if (/\w+\s+instanceof\s+\w+/.test(lines[i]))
                    found++;
                if (/\bis\w+\s*\(/.test(lines[i]) && /\bif\s*\(/.test(lines[i]))
                    found++;
            }
            return { count: found, lines: [] };
        } },
 ...
 */
export function detectGoodPatterns(code) {
 ...
 */
export function suppressFindings(findings, report) {
 ...
 * 완전 제거가 아닌 경감 처리용.
 */
export function downgradeFindings(findings, report) {
    if (report.suppressedRules.length === 0)
        return findings;
    const suppressSet = new Set(report.suppressedRules);
    const DOWNGRADE = {
