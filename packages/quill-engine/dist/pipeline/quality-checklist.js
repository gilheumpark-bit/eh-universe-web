...
function check(id, domain, label, desc, status, detail, metric, threshold, autoFixable = false) {
    return { id, tier: 'basic', domain, label, description: desc, status, detail, metric, threshold, autoFixable };
}
export function runTier1(code, fileName) {
    ...
    results.push(check('S01', 'safety', { ko: '빈 catch 블록', en: 'Empty catch blocks' }, { ko: '에러를 삼키면 디버깅 불가', en: 'Swallowed errors prevent debugging' }, emptyCatch === 0 ? 'pass' : emptyCatch <= 2 ? 'warn' : 'fail', emptyCatch > 0 ? `${emptyCatch}건 발견` : undefined, emptyCatch, 0, true));
    // S02: eval/exec 사용
    const dangerousCalls = (code.match(/\beval\s*\(|\bFunction\s*\(|new\s+Function\b/g) ?? []).length;
    results.push(check('S02', 'safety', { ko: 'eval/Function 사용', en: 'eval/Function usage' }, { ko: '코드 인젝션 위험', en: 'Code injection risk' }, dangerousCalls === 0 ? 'pass' : 'fail', dangerousCalls > 0 ? `${dangerousCalls}건` : undefined, dangerousCalls, 0));
    // S03: 타입 안전성 (any 사용)
    const anyCount = (code.match(/:\s*any\b|as\s+any\b/g) ?? []).length;
    results.push(check('S03', 'safety', { ko: 'any 타입 사용', en: 'any type usage' }, { ko: '타입 안전성 훼손', en: 'Weakens type safety' }, anyCount === 0 ? 'pass' : anyCount <= 3 ? 'warn' : 'fail', anyCount > 0 ? `${anyCount}건` : undefined, anyCount, 0, true));
    // S04: Null 가드 비율
    ...
    results.push(check('S04', 'safety', { ko: 'Null 가드 비율', en: 'Null guard ratio' }, { ko: '속성 접근 대비 ?. ?? 사용률', en: 'Optional chaining usage vs property access' }, guardRatio >= 15 ? 'pass' : guardRatio >= 5 ? 'warn' : 'fail', `${guardRatio}% (${nullGuards}/${propAccess})`, guardRatio, 15));
    // ── Performance ──
    // P01: 중첩 루프 깊이
    ...
    for (const line of lines) {
        ...
        if (line.includes('}') && curDepth > 0)
            curDepth--;
    }
    results.push(check('P01', 'performance', { ko: '중첩 루프 깊이', en: 'Nested loop depth' }, { ko: 'O(n²)+ 알고리즘 탐지', en: 'Detects O(n²)+ algorithms' }, maxDepth <= 1 ? 'pass' : maxDepth === 2 ? 'warn' : 'fail', `최대 ${maxDepth}단계`, maxDepth, 1));
    // P02: console.log 잔류
    const consoleLogs = (code.match(/console\.\w+\s*\(/g) ?? []).length;
    results.push(check('P02', 'performance', { ko: 'console.log 잔류', en: 'Console statements remaining' }, { ko: '프로덕션 성능 저하 + 정보 노출', en: 'Performance impact + info leakage' }, consoleLogs === 0 ? 'pass' : consoleLogs <= 3 ? 'warn' : 'fail', consoleLogs > 0 ? `${consoleLogs}건` : undefined, consoleLogs, 0, true));
    // P03: 대형 함수 (50줄+)
    const fnLengths = [];
    let fnStart = -1;
    ...
    results.push(check('P03', 'performance', { ko: '대형 함수 (50줄+)', en: 'Large functions (50+ lines)' }, { ko: '가독성·테스트·유지보수 저하', en: 'Readability and maintenance concern' }, largeFns === 0 ? 'pass' : largeFns <= 2 ? 'warn' : 'fail', largeFns > 0 ? `${largeFns}개 함수` : undefined, largeFns, 0));
    // ── Reliability ──
    ...
    for (const line of lines) {
        if (/\btry\s*\{/.test(line))
            inTry++;
        if (/\}/.test(line) && inTry > 0)
            inTry--;
        if (/\bawait\b/.test(line) && inTry === 0)
            unguardedAwait++;
    }
    results.push(check('R01', 'reliability', { ko: 'try-catch 없는 await', en: 'Unguarded await' }, { ko: '비동기 에러 미처리 → 런타임 크래시', en: 'Async errors crash at runtime' }, unguardedAwait === 0 ? 'pass' : unguardedAwait <= 2 ? 'warn' : 'fail', unguardedAwait > 0 ? `${unguardedAwait}건` : undefined, unguardedAwait, 0, true));
    ...
    results.push(check('R02', 'reliability', { ko: 'fetch timeout 미설정', en: 'Fetch without timeout' }, { ko: '네트워크 지연 시 무한 대기', en: 'Infinite hang on network delay' }, untimedFetch === 0 ? 'pass' : 'fail', fetchCalls > 0 ? `${untimedFetch}/${fetchCalls} 미보호` : '해당 없음', untimedFetch, 0));
    ...
    results.push(check('R03', 'reliability', { ko: '리소스 정리', en: 'Resource cleanup' }, { ko: 'finally/cleanup/removeEventListener 존재 여부', en: 'Cleanup patterns presence' }, eventListeners === 0 || cleanupPatterns > 0 ? 'pass' : 'warn', `cleanup: ${cleanupPatterns}건, listeners: ${eventListeners}건`));
    // ── Maintainability ──
    // M01: TODO/FIXME/HACK 잔류
    const todos = (code.match(/\/\/\s*(?:TODO|FIXME|HACK|XXX)\b/gi) ?? []).length;
    results.push(check('M01', 'maintainability', { ko: 'TODO/FIXME 잔류', en: 'TODO/FIXME remaining' }, { ko: '미완료 작업 추적', en: 'Tracks unfinished work' }, todos === 0 ? 'pass' : todos <= 3 ? 'warn' : 'fail', todos > 0 ? `${todos}건` : undefined, todos, 0));
    // M02: 미사용 import
    ...
    for (const imp of imports) {
        ...
        for (const name of names) {
            if (name && code.indexOf(name) === code.lastIndexOf(name))
                unusedImports++;
        }
    }
    results.push(check('M02', 'maintainability', { ko: '미사용 import', en: 'Unused imports' }, { ko: '번들 크기 증가', en: 'Bundle size bloat' }, unusedImports === 0 ? 'pass' : unusedImports <= 2 ? 'warn' : 'fail', unusedImports > 0 ? `${unusedImports}건` : undefined, unusedImports, 0, true));
    // M03: 매직 넘버
    const magicNumbers = (code.match(/(?<![.\w])(?:[2-9]\d{2,}|[1-9]\d{3,})(?!\w)/g) ?? []).length;
    results.push(check('M03', 'maintainability', { ko: '매직 넘버', en: 'Magic numbers' }, { ko: '의미 불명의 하드코딩 숫자', en: 'Unexplained hardcoded numbers' }, magicNumbers <= 2 ? 'pass' : magicNumbers <= 5 ? 'warn' : 'fail', magicNumbers > 0 ? `${magicNumbers}건` : undefined, magicNumbers, 2));
    // ── Security ──
    // X01: 하드코딩 시크릿
    const secretPatterns = (code.match(/(?:password|secret|token|apikey|api_key)\s*[:=]\s*['"][^'"]{8,}['"]/gi) ?? []).length;
    results.push(check('X01', 'security', { ko: '하드코딩 시크릿', en: 'Hardcoded secrets' }, { ko: 'API키/비밀번호 코드 내 노출', en: 'API keys/passwords exposed in code' }, secretPatterns === 0 ? 'pass' : 'fail', secretPatterns > 0 ? `${secretPatterns}건 — 즉시 제거 필요` : undefined, secretPatterns, 0));
    // X02: innerHTML 사용
    const innerHtml = (code.match(/innerHTML|dangerouslySetInnerHTML/g) ?? []).length;
    results.push(check('X02', 'security', { ko: 'innerHTML/dangerouslySetInnerHTML', en: 'Raw HTML injection' }, { ko: 'XSS 공격 벡터', en: 'XSS attack vector' }, innerHtml === 0 ? 'pass' : 'warn', innerHtml > 0 ? `${innerHtml}건 — 입력 검증 필요` : undefined, innerHtml, 0));
    ...
    for (let i = 0; i < lines.length; i++) {
        if (/\btry\s*\{/.test(lines[i])) {
            ...
            if (/\bcatch\b/.test(block) && /\bfinally\b/.test(block))
                tryCatchFinally++;
        }
    }
    if (tryCatchFinally > 0) {
        results.push(check('GP01', 'reliability', { ko: 'try-catch-finally 완전 쌍', en: 'Complete try-catch-finally' }, { ko: '에러 처리 + 리소스 해제 보장', en: 'Error handling + resource cleanup guaranteed' }, 'pass', `${tryCatchFinally}건 양품 패턴`, tryCatchFinally, 0));
    }
    ...
    if (totalDecls >= 3 && constRatio >= 70) {
        results.push(check('GP02', 'reliability', { ko: 'const 우선 사용', en: 'Const preference' }, { ko: '불변성 선호 — 안정성 향상', en: 'Immutability preference — improved stability' }, 'pass', `${constRatio}% const 비율 (${constDecls}/${totalDecls})`, constRatio, 70));
    }
    ...
    for (const line of lines) {
        if (/typeof\s+\w+\s*(?:===|!==)\s*['"]/.test(line))
            typeNarrowing++;
        if (/\w+\s+instanceof\s+\w+/.test(line))
            typeNarrowing++;
    }
    if (typeNarrowing > 0) {
        results.push(check('GP03', 'safety', { ko: '타입 narrowing 가드', en: 'Type narrowing guards' }, { ko: 'typeof/instanceof 런타임 타입 검사', en: 'Runtime type checking via typeof/instanceof' }, 'pass', `${typeNarrowing}건 양품 패턴`, typeNarrowing, 0));
    }
    ...
    if (optChain >= 3 || nullCoal >= 2) {
        results.push(check('GP04', 'safety', { ko: '현대적 null 방어', en: 'Modern null defense' }, { ko: '?. ?? 연산자 적극 사용', en: 'Active use of ?. ?? operators' }, 'pass', `?. ${optChain}건, ?? ${nullCoal}건`, optChain + nullCoal, 3));
    }
    // GP05: Early return / Guard clause
    let earlyReturns = 0;
    for (const line of lines) {
        if (/^\s*if\s*\(.*\)\s*(?:return|throw)\b/.test(line))
            earlyReturns++;
    }
    if (earlyReturns >= 2) {
        results.push(check('GP05', 'maintainability', { ko: 'Early return 가드 절', en: 'Early return guard clauses' }, { ko: '중첩 감소 — 가독성 향상', en: 'Reduced nesting — improved readability' }, 'pass', `${earlyReturns}건 가드 절`, earlyReturns, 2));
    }
    // GP06: JSDoc 문서화
    const jsdocBlocks = (code.match(/\/\*\*[\s\S]*?\*\//g) ?? []).length;
    if (jsdocBlocks >= 2) {
        results.push(check('GP06', 'maintainability', { ko: 'JSDoc 문서화', en: 'JSDoc documentation' }, { ko: 'public API 문서화 존재', en: 'Public API documentation present' }, 'pass', `${jsdocBlocks}건 JSDoc 블록`, jsdocBlocks, 2));
    }
    ...
    if (abortControllers > 0 && fetchCalls > 0) {
        results.push(check('GP07', 'reliability', { ko: 'AbortController fetch 보호', en: 'AbortController fetch protection' }, { ko: 'fetch 호출의 타임아웃·취소 제어', en: 'Timeout/cancel control for fetch calls' }, 'pass', `${abortControllers}건 AbortController 사용`, abortControllers, 0));
    }
    return results;
}
...
export function selectPrecisionTargets(tier1) {
...
export function buildPrecisionPrompt(code, fileName, tier1, targets) {
...
// ============================================================
export function generateChecklistReport(fileName, tier1, tier2 = []) {
