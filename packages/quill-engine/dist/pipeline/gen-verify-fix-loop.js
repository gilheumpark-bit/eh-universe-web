 ...
 */
async function callAI(systemInstruction, userMessage, signal, temperature = 0.3) {
    let result = '';
    try {
 ...
 */
function extractActionableFindings(stages) {
    const findings = [];
    for (const stage of stages) {
        if (stage.status === 'pass')
            continue;
        const severity = stage.status === 'fail' ? 'hard-fail' : 'review';
 ...
 */
function buildFixPrompt(code, findings, language) {
...
async function generateCode(task, language, signal) {
    ...
    const { text: retryRaw, tokenUsage: retryTokens } = await callAI(GENERATE_SYSTEM, userMsg, signal, 0.2);
    const combined = {
...
async function fixCode(code, findings, language, signal) {
    const emptyTokens = { promptEstimate: 0, completionEstimate: 0, totalEstimate: 0 };
...
// ============================================================
function verifyCode(code, language) {
    let pipelineResult;
    try {
 ...
 */
function shouldContinueLoop(iterations, currentScore, targetScore, configMaxRounds) {
 ...
 */
function calculateCostEstimate(iterations) {
    let totalPrompt = 0;
 ...
 */
export async function runGenVerifyFixLoop(task, options) {
    ...
    if (!initialCode) {
        return buildFinalResult('', 0, [], { stages: [], overallScore: 0, overallStatus: 'fail', timestamp: Date.now() }, 'generation-failed');
    }
    let currentCode = initialCode;
    /** Track the best code seen so far — never lose a working version */
    let bestCode = initialCode;
    let bestScore = -1;
    let accumulatedTokens = { ...genTokens };
    for (let round = 1; round <= ADAPTIVE_MAX_ROUNDS; round++) {
        ...
        iterations.push(iteration);
        onProgress?.(iteration);
        const { shouldContinue, reason } = shouldContinueLoop(iterations, score, targetScore, configMaxRounds);
        if (!shouldContinue) {
        ...
        if (findings.length > 0) {
            const { fixedCode, tokenUsage: fixTokens } = await fixCode(currentCode, findings, language, signal);
    ...
    const stopReason = bestScore >= targetScore ? 'target-reached' : 'max-rounds';
    return buildFinalResult(bestCode, bestScore, iterations, finalVerification.result, stopReason);
}
// IDENTITY_SEAL: PART-8 | role=main-loop | inputs=task,options | outputs=GenVerifyFixResult
// ============================================================
// PART 9 — Result Builder
// ============================================================
function buildFinalResult(finalCode, finalScore, iterations, pipelineResult, stopReason) {
    return {
        finalCode,
        finalScore,
        iterations,
