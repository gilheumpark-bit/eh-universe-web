...
${_FP_SUPPRESSION}`;
function buildUserPrompt(code, language, fileName) {
 ...
 */
export async function findBugs(code, language, fileName, signal) {
    if (!code.trim())
        return [];
 ...
 */
export function findBugsStatic(code, language) {
    if (!code.trim())
        return [];
...
function detectUnusedVariables(lines, language, reports) {
...
function detectNullDereference(lines, reports) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (let j = i + 1; j < searchEnd; j++) {
            ...
            const accessPattern = new RegExp(`\\b${escapeRegex(varName)}\\s*\\.\\s*[a-zA-Z_]`);
            // Check for null guard like if (varName) or varName != null etc.
            const guardPattern = new RegExp(`(?:if\\s*\\(\\s*${escapeRegex(varName)}|${escapeRegex(varName)}\\s*(?:!==?|===?)\\s*(?:null|undefined)|${escapeRegex(varName)}\\s*\\?\\.)`);
            if (guardPattern.test(line))
                break; // guarded — stop looking
...
function detectUnreachableCode(lines, reports) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (let j = i + 1; j < lines.length; j++) {
            ...
            if (next === '}' || next === ')' || next.startsWith('case ') || next === 'default:')
                break;
...
function detectEmptyCatch(lines, language, reports) {
    const isJsTsFamily = /^(javascript|typescript|jsx|tsx|js|ts)$/i.test(language);
    const isPython = /^python$/i.test(language);
    for (let i = 0; i < lines.length; i++) {
        const trimmed = lines[i].trim();
...
function detectMissingSwitchDefault(lines, language, reports) {
...
function detectDivisionByZero(lines, reports) {
 ...
 */
export async function findBugsAst(code, language) {
    const isJsTsFamily = /^(javascript|typescript|jsx|tsx|js|ts)$/i.test(language);
    if (!isJsTsFamily || !code.trim())
        return [];
    ...
    let sourceFile;
    try {
        sourceFile = ts.createSourceFile(ext, code, ts.ScriptTarget?.Latest ?? 99, true);
    }
    catch {
        return [];
    }
    ...
    function detectSwitchNoDefault(node) {
        if (node.kind === SK.SwitchStatement) {
            const caseBlock = node.caseBlock;
            if (caseBlock && caseBlock.clauses) {
                const hasDefault = caseBlock.clauses.some((c) => c.kind === SK.DefaultClause);
                if (!hasDefault) {
                    reports.push({
                        id: nextId(),
                        severity: 'low',
    ...
    function detectAsyncWithoutTryCatch(node) {
        const isAsyncFunc = (node.kind === SK.FunctionDeclaration ||
            node.kind === SK.ArrowFunction ||
            node.kind === SK.FunctionExpression ||
            node.kind === SK.MethodDeclaration) &&
            node.modifiers?.some((m) => m.kind === SK.AsyncKeyword);
        if (isAsyncFunc && node.body) {
            let hasAwait = false;
            let hasTryCatch = false;
            function walkBody(n) {
                if (n.kind === SK.AwaitExpression)
                    hasAwait = true;
                if (n.kind === SK.TryStatement)
                    hasTryCatch = true;
    ...
    function detectUnhandledPromise(node) {
    ...
    function detectNullDerefAst(node) {
        if (node.kind === SK.VariableDeclaration && node.initializer) {
            const init = node.initializer;
            const isNullish = init.kind === SK.NullKeyword ||
            ...
            if (isNullish && node.name) {
                ...
                for (let ln = declLine; ln < scanEnd; ln++) {
                    ...
                    if (!lineText)
                        continue;
                    const guardRe = new RegExp(`(?:if\\s*\\(\\s*${escapeRegex(varName)}|${escapeRegex(varName)}\\s*(?:!==?|===?)\\s*(?:null|undefined)|${escapeRegex(varName)}\\s*\\?\\.|${escapeRegex(varName)}\\s*\\?\\s*\\.)`);
                    if (guardRe.test(lineText))
                        break;
 ...
 */
function validateAiResults(reports, lineCount) {
 ...
 */
export function deduplicateBugReports(reports) {
    const SOURCE_PRIORITY = { ast: 3, static: 2, ai: 1 };
 ...
 */
function areSimilarMessages(a, b) {
    const normalize = (s) => s
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, '')
        .split(/\s+/)
 ...
 */
export async function findBugsCombined(code, language, fileName, signal) {
    if (!code.trim())
        return [];
    ...
    const [staticResults, astResults, aiResults] = await Promise.all([
        ...
        findBugsAst(code, language).catch(() => []),
        findBugs(code, language, fileName, signal).catch(() => []),
    ]);
