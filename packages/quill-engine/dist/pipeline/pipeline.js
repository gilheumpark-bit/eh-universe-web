...
import { detectGoodPatterns } from './good-pattern-detector';
import { applyScopePolicy } from '../scope-policy';
// ============================================================
// PART 1 — Team 1: Simulation (런타임 동작 예측)
// ============================================================
function analyzeSimulation(code, _language) {
    const findings = [];
    let score = 100;
...
// ============================================================
function analyzeGeneration(code, _language) {
    const findings = [];
    let score = 100;
    const lines = code.split('\n');
    // ============================================================
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (m) {
            ...
            if (Number.isFinite(partNum))
                partStartIdxs.push({ part: partNum, start: i });
        }
    }
    ...
    // ============================================================
    const scopeStartLines = [];
    const scopeEndLines = [];
    const blockIdLines = new Map();
    for (let i = 0; i < lines.length; i++) {
        if (/\[SCOPE_START:/i.test(lines[i]))
            scopeStartLines.push(i + 1);
        if (/\[SCOPE_END\]/i.test(lines[i]))
            scopeEndLines.push(i + 1);
    ...
    if (scopeStartLines.length !== scopeEndLines.length) {
        findings.push(`SCOPE marker mismatch: starts=${scopeStartLines.length} (e.g. L${scopeStartLines[0] ?? '?'}) ends=${scopeEndLines.length} (e.g. L${scopeEndLines[0] ?? '?'})`);
        score -= 15;
    }
    // 함수 길이 검사
    let inFunc = false;
...
// ============================================================
function analyzeValidation(code, language) {
    const findings = [];
    let score = 100;
    const lines = code.split('\n');
    // any 타입 사용
...
// ============================================================
function analyzeSizeDensity(code, _language) {
    const findings = [];
    let score = 100;
    const lines = code.split('\n');
...
// ============================================================
function analyzeAssetTrace(code, _language) {
    const findings = [];
    let score = 100;
...
// ============================================================
function analyzeStability(code, _language) {
    const findings = [];
    let score = 100;
    const lines = code.split('\n');
    // O(n^2) 패턴: 중첩 루프
...
// ============================================================
function analyzeReleaseIP(code, _language) {
    const findings = [];
    let score = 100;
    const lines = code.split('\n');
    // 하드코딩된 시크릿
...
// ============================================================
function analyzeGovernance(code, _language) {
    const findings = [];
    let score = 100;
    const lines = code.split('\n');
    if (lines.length > 300) {
...
function detectUnusedImportsAST(ts, sourceFile, code) {
    const findings = [];
...
function detectUnusedVariablesAST(ts, sourceFile) {
    const findings = [];
...
function detectMissingReturnTypeAST(ts, sourceFile) {
    const findings = [];
    const check = (node) => {
...
function detectEmptyFunctionsAST(ts, sourceFile) {
    const findings = [];
    const check = (node) => {
        let body;
        let name = '<anonymous>';
 ...
 */
async function runASTAnalysis(code, fileName) {
 ...
 */
function mergeASTFindings(stage, astFindings, category) {
...
function deduplicateFindings(stage) {
    const seen = new Set();
    const unique = [];
 ...
 */
export function runStaticPipeline(code, language, filePath) {
    const stages = [
 ...
 */
export async function runStaticPipelineWithAST(code, language, fileName) {
 ...
 */
export function parseReviewResponse(raw, reviewerId) {
    try {
...
// ============================================================
import { runTeam1Simulation, runTeam2Generation, runTeam3Validation, runTeam4SizeDensity, runTeam5AssetTrace, runTeam6Stability, runTeam7ReleaseIP, runTeam8Governance, } from './pipeline-teams';
 ...
 */
export async function runFullPipeline(code, language, fileName, callbacks) {
    const results = [];
        ...
        const promises = parallelTeams.map(async (team) => {
            ...
            try {
                ...
                const errorResult = {
                    ...
                    status: 'fail',
                    score: 0,
                    findings: [{ severity: 'critical', message: `오류: ${err.message}`, rule: 'TEAM_ERROR' }],
                };
