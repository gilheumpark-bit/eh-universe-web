...
export function classifyLine(text) {
    ...
    if (DECORATOR_RE.test(text))
        return 'decorator';
    if (BLOCK_BOUNDARY_RE.test(text))
        return 'block-boundary';
    if (RETURN_RE.test(text))
        return 'return';
...
// ============================================================
function generateSuggestions(lines) {
    const suggestions = [];
...
// PART 5 — Public API
// ============================================================
export function analyzeCodeRhythm(code) {
    if (!code.trim()) {
        return {
            lines: [],
            sections: [],
    ...
    const denseCount = lines.filter((l) => l.type === 'logic' || l.type === 'return' || l.type === 'import' || l.type === 'type').length;
    return {
        lines,
        sections,
        suggestions,
