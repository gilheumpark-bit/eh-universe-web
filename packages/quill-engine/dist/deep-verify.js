...
function checkDeclarationOrder(code, fileName) {
    const findings = [];
    ...
    const declarations = new Map(); // varName → line number
    const usages = [];
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        // Skip imports, comments, type annotations
        if (/^(import |\/\/|\/\*|\*|type |interface |export type |export interface )/.test(line))
            continue;
        ...
        if (identifiers) {
            for (const ident of identifiers) {
                if (IGNORE_NAMES.has(ident))
                    continue;
                if (/^(const|let|var|function|class|import|export|return|if|else|for|while|new|typeof|await|async|try|catch|throw|break|continue|switch|case|default|from|as|of|in)$/.test(ident))
                    continue;
                if (line.includes(`const ${ident}`) || line.includes(`let ${ident}`) || line.includes(`var ${ident}`))
                    continue;
...
function checkBraceBalance(code, fileName) {
    const findings = [];
...
// ============================================================
function checkAsyncPatterns(code, fileName) {
    const findings = [];
    const lines = code.split('\n');
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
...
// ============================================================
function checkUnsafeCasts(code, fileName) {
    const findings = [];
    const lines = code.split('\n');
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
...
// ============================================================
function checkResourceLeaks(code, fileName) {
    const findings = [];
    const lines = code.split('\n');
...
// ============================================================
function checkMathLogic(code, fileName) {
    const findings = [];
    const lines = code.split('\n');
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
...
// ============================================================
export function runDeepVerify(code, fileName) {
...
// ============================================================
export function runDeepVerifyProject(rootPath) {
