...
export const PROVIDERS: Record<AIProviderId, AIProviderConfig> = {
  gemini: {
    id: 'gemini',
    name: 'Google Gemini',
...
// Type aliases used by some engine pipeline files
...
function notImplemented(name: string): never {
  throw new Error(
    `[quill-engine] ${name}() called from engine code — AI calls must go through the host (main/cli), not the engine itself.`,
  );
}

export function callProvider(): never {
...
export async function streamChat(..._args: unknown[]): Promise<unknown> {
...
export function getActiveProvider(): AIProviderId | null {
  return null;
}
