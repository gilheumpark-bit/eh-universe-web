...
const alias_1 = require("../core/alias");
const VERSION = '0.1.0';
// ============================================================
...
program
    ...
    try {
        ...
        checks.push({ name: 'npm', score: 0, detail: '미설치' });
    }
    // git
    try {
        ...
        checks.push({ name: 'git', score: 0, detail: '미설치' });
    }
    // TypeScript
    try {
        ...
        checks.push({ name: 'TypeScript', score: 50, detail: '미설치 (선택)' });
    }
    // AI 키
    ...
    if (keyCount === 0)
        console.log(`  ${icons.info} AI 기능 활성화: ${colors.cyan('cs config set-key <provider> <key>')}\n`);
    if (!hasCS)
        console.log(`  ${icons.info} 프로젝트 초기화: ${colors.cyan('cs init')}\n`);
});
// ============================================================
...
if (args.length > 0) {
    const resolved = (0, alias_1.resolveAlias)(args[0]);
    if (resolved !== args[0]) {
