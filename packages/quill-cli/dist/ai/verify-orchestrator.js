...
function staticToAgentFindings(teams, file) {
    ...
    for (const team of teams) {
        for (const f of team.findings) {
            ...
            idx++;
            if (idx > 50)
                break; // cap per-team to avoid token overflow
        }
    }
    return findings;
}
...
// ============================================================
async function orchestrateVerify(code, staticResult, filePath) {
    ...
    if (judgeResult) {
        for (const f of judgeResult.findings) {
            if (f.verdict === 'dismiss')
                dismissedIds.add(f.id);
        }
    }
    if (verdict) {
