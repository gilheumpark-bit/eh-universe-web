...
exports.chainReceipt = chainReceipt;
...
function computeReceiptHash(data) {
...
function verifyReceiptHash(receipt, previousHash) {
    const payload = JSON.stringify({ ...receipt, receiptHash: undefined }) + (previousHash ?? 'GENESIS');
    const expected = (0, crypto_1.createHmac)('sha256', HMAC_SECRET).update(payload).digest('hex');
    return expected === receipt.receiptHash;
}
function chainReceipt(receipt) {
...
// ============================================================
function formatReceipt(receipt, lang = 'ko') {
