...
exports.runAxeAccessibility = runAxeAccessibility;
exports.checkBundleSize = checkBundleSize;
exports.runLighthouse = runLighthouse;
exports.analyzeTreeShaking = analyzeTreeShaking;
exports.runFullWebQualityAnalysis = runFullWebQualityAnalysis;
// ============================================================
// PART 1 — Axe-Core (접근성 테스트)
// ============================================================
async function runAxeAccessibility(htmlContent) {
    const findings = [];
    try {
        ...
        const results = await axe.default.run(document.documentElement);
...
// ============================================================
async function checkBundleSize(rootPath) {
    const { readFileSync, existsSync } = require('fs');
    const { join } = require('path');
    const findings = [];
    const pkgPath = join(rootPath, 'package.json');
    if (!existsSync(pkgPath))
        return { findings, score: 100, engine: 'bundle-size' };
...
async function runLighthouse(url) {
    ...
    try {
        const output = execSync(`npx lighthouse "${url}" --output=json --quiet --chrome-flags="--headless --no-sandbox" 2>/dev/null`, { encoding: 'utf-8', timeout: 60000 });
...
// ============================================================
async function analyzeTreeShaking(rootPath) {
    const { readFileSync, existsSync, readdirSync, statSync } = require('fs');
    const { join, relative } = require('path');
    const findings = [];
    const pkgPath = join(rootPath, 'package.json');
    if (!existsSync(pkgPath))
        return { findings, score: 100, engine: 'tree-shaking' };
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf-8'));
    ...
    const badImports = [];
    const checkImports = (dir, depth = 0) => {
        if (depth > 3 || badImports.length >= 10)
            return;
        try {
            for (const entry of readdirSync(dir, { withFileTypes: true })) {
                if (entry.name.startsWith('.') || IGNORE.has(entry.name))
...
// ============================================================
async function runFullWebQualityAnalysis(rootPath, lighthouseUrl) {
    const results = [];
    // Bundle size
    ...
    try {
        ...
        const { join } = require('path');
        const htmlFiles = [];
        const findHtml = (dir, depth = 0) => {
            if (depth > 2 || htmlFiles.length >= 3)
                return;
            try {
                for (const e of readdirSync(dir, { withFileTypes: true })) {
                    if (['node_modules', '.next', '.git', 'dist'].includes(e.name))
                        continue;
                    const full = join(dir, e.name);
                    if (e.isDirectory()) {
                        findHtml(full, depth + 1);
                        continue;
                    }
...
function detectDevUrl(rootPath) {
    try {
        const { readFileSync, existsSync } = require('fs');
        const { join } = require('path');
        const pkgPath = join(rootPath, 'package.json');
