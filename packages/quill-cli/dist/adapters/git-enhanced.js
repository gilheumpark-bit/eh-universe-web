...
exports.resolveConflictRule = resolveConflictRule;
exports.resolveConflictWithAI = resolveConflictWithAI;
exports.applyConflictResolutions = applyConflictResolutions;
exports.generateCommitMessage = generateCommitMessage;
exports.suggestBranchName = suggestBranchName;
exports.getStaleLocalBranches = getStaleLocalBranches;
...
exports.getBranchAgeWarnings = getBranchAgeWarnings;
const { execSync } = require('child_process');
const { readFileSync, writeFileSync, existsSync } = require('fs');
const { join } = require('path');
function detectConflicts(rootPath) {
    const results = [];
    try {
...
function resolveConflictRule(conflict, strategy) {
    switch (strategy) {
...
async function resolveConflictWithAI(conflict, context) {
    try {
...
function applyConflictResolutions(rootPath, file, resolutions) {
    const fullPath = join(rootPath, file);
    if (!existsSync(fullPath))
        return false;
    const content = readFileSync(fullPath, 'utf-8');
...
// ============================================================
function generateCommitMessage(rootPath) {
    try {
...
function getStaleLocalBranches(rootPath, daysSince = 30) {
    try {
        const output = execSync('git for-each-ref --sort=committerdate --format="%(refname:short)|%(committerdate:unix)" refs/heads/ 2>/dev/null', { cwd: rootPath, encoding: 'utf-8', stdio: 'pipe' });
...
// ============================================================
function getRepoHealth(rootPath) {
    try {
        const totalCommits = parseInt(execSync('git rev-list --count HEAD 2>/dev/null', { cwd: rootPath, encoding: 'utf-8', stdio: 'pipe' }).trim(), 10) || 0;
        const contributors = parseInt(execSync('git shortlog -sn --no-merges 2>/dev/null | wc -l', { cwd: rootPath, encoding: 'utf-8', stdio: 'pipe' }).trim(), 10) || 0;
...
// ============================================================
function getCommitFrequency(rootPath, days = 90) {
    try {
        const since = new Date(Date.now() - days * 86400000).toISOString().slice(0, 10);
        const output = execSync(`git log --since="${since}" --format="%ai" --no-merges 2>/dev/null`, { cwd: rootPath, encoding: 'utf-8', stdio: 'pipe' });
...
// ============================================================
function getHotFiles(rootPath, days = 60, limit = 15) {
    try {
        const since = new Date(Date.now() - days * 86400000).toISOString().slice(0, 10);
        const output = execSync(`git log --since="${since}" --name-only --pretty=format:"COMMIT_SEP|%an|%ai" --no-merges 2>/dev/null`, { cwd: rootPath, encoding: 'utf-8', stdio: 'pipe' });
        ...
        for (const line of output.split('\n')) {
            if (line.startsWith('COMMIT_SEP|')) {
                ...
                const existing = fileStats.get(file) ?? { commits: 0, authors: new Set(), lastDate: '' };
                existing.commits++;
                if (currentAuthor)
...
// ============================================================
function getContributorStats(rootPath, days = 90) {
    try {
        const since = new Date(Date.now() - days * 86400000).toISOString().slice(0, 10);
        const output = execSync(`git log --since="${since}" --pretty=format:"AUTHOR_SEP|%an|%ai" --numstat --no-merges 2>/dev/null`, { cwd: rootPath, encoding: 'utf-8', stdio: 'pipe' });
        const stats = new Map();
        let current = '';
        let currentDate = '';
        for (const line of output.split('\n')) {
            if (line.startsWith('AUTHOR_SEP|')) {
                ...
                if (currentDate < s.firstCommit)
                    s.firstCommit = currentDate;
                if (currentDate > s.lastCommit)
                    s.lastCommit = currentDate;
            }
...
// ============================================================
function getBranchAgeWarnings(rootPath) {
    try {
        const output = execSync('git for-each-ref --sort=-committerdate --format="%(refname:short)|%(committerdate:unix)|%(committerdate:short)|%(authorname)" refs/heads/ 2>/dev/null', { cwd: rootPath, encoding: 'utf-8', stdio: 'pipe' });
        ...
        for (const line of output.split('\n').filter(Boolean)) {
            ...
            let aheadBehind = '';
            try {
                const ab = execSync(`git rev-list --left-right --count "${defaultBranch}...${branch}" 2>/dev/null`, { cwd: rootPath, encoding: 'utf-8', stdio: 'pipe' }).trim();
