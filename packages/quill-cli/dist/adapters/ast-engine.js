...
exports.analyzeWithTypeScript = analyzeWithTypeScript;
exports.analyzeWithTsMorph = analyzeWithTsMorph;
exports.analyzeWithAcorn = analyzeWithAcorn;
exports.analyzeWithBabel = analyzeWithBabel;
exports.runFullASTAnalysis = runFullASTAnalysis;
exports.computeASTMetrics = computeASTMetrics;
exports.analyzeModuleCoupling = analyzeModuleCoupling;
// ============================================================
// PART 1 — TypeScript Compiler API (Level 2: 타입 추론)
// ============================================================
async function analyzeWithTypeScript(code, fileName = 'temp.ts') {
...
// ============================================================
async function analyzeWithTsMorph(code, fileName = 'temp.ts') {
...
// ============================================================
async function analyzeWithAcorn(code) {
    ...
    try {
        ...
        return { findings: [{ line: 1, message: `Parse error: ${e.message}`, severity: 'error' }] };
    }
    ...
    const consoleLogs = esquery.query(ast, 'CallExpression[callee.object.name="console"]');
...
// ============================================================
async function analyzeWithBabel(code) {
    ...
    try {
        ...
        findings.push({ line: 1, message: `Babel parse error: ${e.message}`, severity: 'error' });
    }
    return { findings };
}
...
// ============================================================
async function runFullASTAnalysis(code, fileName = 'temp.ts') {
    const results = [];
...
async function analyzeWithEsquery(code) {
    const acorn = require('acorn');
    const esquery = require('esquery');
    const findings = [];
    let ast;
    ...
    const evalCalls = esquery.query(ast, 'CallExpression[callee.name="eval"]');
    for (const node of evalCalls) {
        findings.push({ line: node.loc?.start?.line ?? 1, message: 'eval() 호출 — 보안 위험', severity: 'critical' });
    }
    // new Function()
    const newFn = esquery.query(ast, 'NewExpression[callee.name="Function"]');
    for (const node of newFn) {
        findings.push({ line: node.loc?.start?.line ?? 1, message: 'new Function() — eval 동등', severity: 'critical' });
    }
    // 3중 루프 탐지
    const tripleLoop = esquery.query(ast, ':matches(ForStatement, WhileStatement, ForOfStatement) :matches(ForStatement, WhileStatement, ForOfStatement) :matches(ForStatement, WhileStatement, ForOfStatement)');
    if (tripleLoop.length > 0) {
        findings.push({ line: tripleLoop[0].loc?.start?.line ?? 1, message: '3중 중첩 루프 — O(n³) 복잡도', severity: 'warning' });
    }
    return { findings };
}
async function computeASTMetrics(code, fileName = 'temp.ts') {
    const ts = require('typescript');
    const sourceFile = ts.createSourceFile(fileName, code, ts.ScriptTarget.Latest, true);
    const functions = [];
    const importModules = [];
    ...
    function collectInfo(node) {
        const lineOf = (n) => sourceFile.getLineAndCharacterOfPosition(n.getStart()).line + 1;
        const endLineOf = (n) => sourceFile.getLineAndCharacterOfPosition(n.getEnd()).line + 1;
        // Functions
        ...
        // Exports
        if (ts.isExportDeclaration(node) || (node.modifiers && node.modifiers.some((m) => m.kind === ts.SyntaxKind.ExportKeyword))) {
    ...
    const deepNested = [];
    function measureNesting(node, depth, context) {
        const lineOf = (n) => sourceFile.getLineAndCharacterOfPosition(n.getStart()).line + 1;
        const nestTypes = [
        ...
        ts.forEachChild(node, (child) => {
            ...
            if (childMax > maxChildDepth)
                maxChildDepth = childMax;
        });
...
// ============================================================
async function analyzeModuleCoupling(rootPath) {
    const fs = require('fs');
    const path = require('path');
    const importGraph = new Map(); // file -> set of imported files
    const importedByGraph = new Map(); // file -> set of files that import it
