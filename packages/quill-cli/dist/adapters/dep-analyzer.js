...
exports.runKnip = runKnip;
exports.runDependencyCruiser = runDependencyCruiser;
exports.runPublint = runPublint;
exports.runAttw = runAttw;
exports.runOxlint = runOxlint;
exports.detectCodemodOpportunities = detectCodemodOpportunities;
exports.detectCircularDeps = detectCircularDeps;
exports.detectVersionMismatches = detectVersionMismatches;
exports.detectUnusedDepsLocal = detectUnusedDepsLocal;
exports.runFullDepAnalysis = runFullDepAnalysis;
// ============================================================
// PART 1 — depcheck (미사용 의존성 탐지)
// ============================================================
async function runDepcheck(rootPath) {
...
// ============================================================
async function runKnip(rootPath) {
    const { execSync } = require('child_process');
    try {
...
// ============================================================
async function runDependencyCruiser(rootPath) {
    const { execSync } = require('child_process');
    try {
        const output = execSync('npx depcruise --output-type json src/ 2>/dev/null', { cwd: rootPath, encoding: 'utf-8', timeout: 30000 });
        const data = JSON.parse(output);
        const violations = data.summary?.violations ?? [];
        const totalModules = data.summary?.totalCruised ?? 0;
        const circular = violations.filter((v) => v.rule?.severity === 'error' && v.cycle);
        return {
            totalModules,
            violations: violations.length,
            circular: circular.length,
            orphans: data.summary?.orphans ?? 0,
            details: violations.slice(0, 10).map((v) => ({
                from: v.from,
                to: v.to,
...
// ============================================================
async function runPublint(rootPath) {
    const { execSync } = require('child_process');
    try {
        ...
        return {
            ...
            messages: messages.slice(0, 10).map((m) => ({ type: m.type, message: m.message ?? String(m) })),
...
// ============================================================
async function runAttw(rootPath) {
    const { execSync } = require('child_process');
    try {
        ...
        return {
            ...
            problems: problems.slice(0, 10).map((p) => ({ kind: p.kind, entrypoint: p.entrypoint, resolution: p.resolution })),
...
// ============================================================
async function runOxlint(rootPath) {
    const { execSync } = require('child_process');
    try {
...
// ============================================================
async function detectCodemodOpportunities(rootPath) {
...
// ============================================================
async function detectCircularDeps(rootPath) {
    const fs = require('fs');
...
// ============================================================
async function detectVersionMismatches(rootPath) {
    const fs = require('fs');
    const path = require('path');
    const mismatches = [];
    const warnings = [];
    ...
    if (!fs.existsSync(pkgPath))
        return { mismatches, warnings, score: 100, engine: 'version-mismatch' };
...
// ============================================================
async function detectUnusedDepsLocal(rootPath) {
    const fs = require('fs');
    const path = require('path');
    const pkgPath = path.join(rootPath, 'package.json');
    if (!fs.existsSync(pkgPath))
        return { unused: [], phantomDeps: [], score: 100, engine: 'unused-dep-local' };
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
    ...
    for (const cf of configFiles) {
        ...
        if (fs.existsSync(cfPath)) {
            try {
                configContent += fs.readFileSync(cfPath, 'utf-8') + '\n';
            }
            catch { /* skip */ }
        }
    }
    ...
    for (const dep of devDeps) {
        ...
        if (dep.startsWith('@types/'))
            continue; // type packages are implicit
        const escapedDep = dep.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const re = new RegExp(`['"]${escapedDep}['"/]|from\\s+['"]${escapedDep}|require\\(['"]${escapedDep}`);
        if (!re.test(combinedSource)) {
...
// ============================================================
async function runFullDepAnalysis(rootPath) {
    const results = [];
    // depcheck
