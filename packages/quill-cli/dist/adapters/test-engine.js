...
exports.runPropertyTests = runPropertyTests;
exports.runFastCheck = runFastCheck;
exports.autoFuzzFunction = autoFuzzFunction;
exports.runStryker = runStryker;
exports.runMocha = runMocha;
exports.detectTestRunner = detectTestRunner;
exports.enforceCoverageThresholds = enforceCoverageThresholds;
exports.detectFlakyTests = detectFlakyTests;
exports.runFullTestAnalysis = runFullTestAnalysis;
async function runVitest(rootPath) {
    try {
...
async function runPropertyTests(specs) {
    const fc = require('fast-check');
    const results = [];
    function resolveArbitrary(desc) {
        switch (desc) {
            ...
            default: {
                ...
                if (arrayMatch)
                    return fc.array(resolveArbitrary(arrayMatch[1]));
                return fc.anything();
            }
        }
    }
    for (const spec of specs) {
        try {
            ...
            if (arbitraries.length === 1) {
                fc.assert(fc.property(arbitraries[0], (a) => spec.predicate([a])), { numRuns: spec.numRuns ?? 100, endOnFailure: true });
            }
            else if (arbitraries.length === 2) {
                fc.assert(fc.property(arbitraries[0], arbitraries[1], (a, b) => spec.predicate([a, b])), { numRuns: spec.numRuns ?? 100, endOnFailure: true });
            }
            else if (arbitraries.length === 3) {
                fc.assert(fc.property(arbitraries[0], arbitraries[1], arbitraries[2], (a, b, c) => spec.predicate([a, b, c])), { numRuns: spec.numRuns ?? 100, endOnFailure: true });
            }
            else {
                fc.assert(fc.property(fc.tuple(...arbitraries), (tuple) => spec.predicate(tuple)), { numRuns: spec.numRuns ?? 100, endOnFailure: true });
            }
...
async function runFastCheck(specs) {
    return runPropertyTests(specs);
}
// Convenience: 함수에 대해 자동 PBT 생성
async function autoFuzzFunction(fnCode, fnName, paramTypes = ['string'], numRuns = 100) {
...
async function runStryker(rootPath) {
    const { execSync } = require('child_process');
    try {
...
async function runMocha(rootPath) {
    const { execSync } = require('child_process');
    try {
...
function detectTestRunner(rootPath) {
    ...
    for (const cfg of vitestConfigs) {
        if (existsSync(join(rootPath, cfg)))
            return { runner: 'vitest', configFile: cfg };
    }
    ...
    for (const cfg of jestConfigs) {
        if (existsSync(join(rootPath, cfg)))
            return { runner: 'jest', configFile: cfg };
    }
    ...
    for (const cfg of mochaConfigs) {
        if (existsSync(join(rootPath, cfg)))
            return { runner: 'mocha', configFile: cfg };
    }
    ...
    try {
        ...
        if (existsSync(pkgPath)) {
            ...
            if (allDeps['vitest'] || scripts.includes('vitest'))
                return { runner: 'vitest', configFile: null };
            if (allDeps['jest'] || scripts.includes('jest'))
                return { runner: 'jest', configFile: null };
            if (allDeps['mocha'] || scripts.includes('mocha'))
                return { runner: 'mocha', configFile: null };
            // Check for jest config in package.json
            if (pkg.jest)
                return { runner: 'jest', configFile: 'package.json (jest key)' };
        }
    }
    catch { /* skip */ }
    return { runner: 'unknown', configFile: null };
}
async function enforceCoverageThresholds(rootPath, thresholds = { lines: 80, branches: 70, functions: 80, statements: 80 }) {
    ...
    if (actual.lines === 0 && actual.branches === 0) {
        try {
            ...
            let cmd = '';
            if (detected.runner === 'vitest')
                cmd = 'npx vitest run --coverage --reporter=json 2>/dev/null';
            else if (detected.runner === 'jest')
                cmd = 'npx jest --coverage --coverageReporters=json-summary 2>/dev/null';
            if (cmd) {
...
async function detectFlakyTests(rootPath, runs = 3) {
    const { execSync } = require('child_process');
    const detected = detectTestRunner(rootPath);
    ...
    for (let r = 0; r < runs; r++) {
        try {
            ...
            if (detected.runner === 'vitest')
                ...
                cmd = 'npx jest --json 2>/dev/null';
            else if (detected.runner === 'mocha')
                cmd = 'npx mocha --reporter json --recursive 2>/dev/null';
            else
                cmd = 'npx vitest run --reporter=json 2>/dev/null';
...
// ============================================================
async function runFullTestAnalysis(rootPath, opts) {
    const results = [];
