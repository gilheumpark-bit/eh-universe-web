...
exports.runFalsePositiveFilter = runFalsePositiveFilter;
exports.printFilterSummary = printFilterSummary;
const FP_CHECKLIST = [
 ...
 */
function detectGoodPatterns(code) {
    const detected = new Set();
    // GQ-NL-010: 타입 narrowing → RTE-001,002,003 억제
    if (/!==\s*(null|undefined)|typeof\s+\w+\s*[!=]==/.test(code))
        detected.add('GQ-NL-010');
    // GQ-TS-001: strict: true → TYP-012,013,014 억제
    // (tsconfig는 별도 체크이므로 파일 내 strict 관련 패턴만)
    // GQ-AS-005: try-catch-finally → ASY-003 억제
    if (/try\s*\{[\s\S]*?catch[\s\S]*?finally/.test(code))
        detected.add('GQ-AS-005');
    // GQ-EH-003: catch에서 복구 또는 재throw → ERR-001,002 억제
    if (/catch\s*\([^)]*\)\s*\{[^}]*throw/.test(code))
        detected.add('GQ-EH-003');
    // GQ-FN-004: Early return / Guard clause → CMX-007 억제
    if (/^\s*if\s*\([^)]*\)\s*(return|throw)\b/m.test(code))
        detected.add('GQ-FN-004');
    // GQ-FN-009: const 우선 → VAR-008 억제
    const constCount = (code.match(/\bconst\b/g) || []).length;
    const letCount = (code.match(/\blet\b/g) || []).length;
    if (constCount > letCount * 3)
        detected.add('GQ-FN-009');
    // GQ-AS-002: Promise.all → ASY-002, PRF-004 억제
    if (/Promise\.all\s*\(/.test(code))
        detected.add('GQ-AS-002');
    // GQ-SC-003: process.env 사용 → SEC-009 억제
    if (/process\.env\.\w+/.test(code) && !/sk-[a-zA-Z]{20}|AIza/.test(code))
        detected.add('GQ-SC-003');
    // GQ-NL-007: JSON.parse try-catch → RTE-008 억제
    if (/try\s*\{[^}]*JSON\.parse/.test(code))
        detected.add('GQ-NL-007');
    // GQ-TS-004: unknown 사용 → TYP-001 (any) 억제
    if (/:\s*unknown\b/.test(code) && !/:\s*any\b/.test(code))
        detected.add('GQ-TS-004');
    // GQ-FN-010: 스프레드 불변 업데이트 → LOG-014, LOG-020 억제
    if (/\{\s*\.\.\./.test(code))
        detected.add('GQ-FN-010');
    // GQ-FN-012: 원본 비변형 slice/concat → LOG-014 억제
    if (/\.slice\(|\.concat\(|\.map\(|\.filter\(/.test(code))
        detected.add('GQ-FN-012');
    // GQ-NL-001: optional chaining → RTE-003 억제
    if (/\?\.\w/.test(code))
        detected.add('GQ-NL-001');
    // GQ-NL-002: nullish coalescing → RTE-004 억제
    if (/\?\?/.test(code))
        detected.add('GQ-NL-002');
    // GQ-NL-005: 구조분해 기본값 → RTE-007 억제
    if (/=\s*['"\d\[\{]/.test(code) && /const\s*\{/.test(code))
        detected.add('GQ-NL-005');
    // GQ-AS-007: removeEventListener → ASY-009, PRF-006 억제
    if (/removeEventListener|\.off\(/.test(code))
        detected.add('GQ-AS-007');
    // GQ-AS-008: clearTimeout → RES-003 억제
    if (/clearTimeout|clearInterval/.test(code))
        detected.add('GQ-AS-008');
    // GQ-EH-001: 커스텀 Error 클래스 → ERR-005 억제
    if (/extends\s+Error/.test(code))
        detected.add('GQ-EH-001');
    // GQ-SC-007: bcrypt/argon2 → SEC-011, SEC-012 억제
    if (/bcrypt|argon2|scrypt/.test(code))
        detected.add('GQ-SC-007');
    // GQ-SC-009: zod/joi 검증 → SEC-001, SEC-002 억제
    if (/zod|Joi|z\.object|z\.string/.test(code))
        detected.add('GQ-SC-009');
    // GQ-PF-005: debounce/throttle → PRF-009 억제
    if (/debounce|throttle/.test(code))
        detected.add('GQ-PF-005');
    // GQ-PF-010: RegExp 루프 밖 정의 → PRF-008 억제
    if (/const\s+\w+\s*=\s*\//.test(code))
        detected.add('GQ-PF-010');
    // GQ-RS-001: try-finally → RES-001, RES-002 억제
    if (/finally\s*\{/.test(code))
        detected.add('GQ-RS-001');
    // GQ-CF-007: .env.example → CFG-010 억제
    if (/\.env\.example|process\.env/.test(code))
        detected.add('GQ-CF-007');
    // GQ-EH-010: never exhaustive → CMX-012 억제
    if (/:\s*never\b|assertNever/.test(code))
        detected.add('GQ-EH-010');
    // ── 5대 양품 마스터 패턴 ──
    // 패턴1: HashMap 최적화 → PRF-002 억제
    if (/new\s+Map\(|new\s+Set\(|\.has\(/.test(code))
        detected.add('GQ-PF-002');
    // 패턴2: Dictionary Dispatch → CMX-012 억제
    if (/Record<.*,.*>\s*=\s*\{/.test(code))
        detected.add('GQ-SL-006');
    // 패턴3: Promise.allSettled → ASY-002 억제
    if (/Promise\.allSettled/.test(code))
        detected.add('GQ-AS-003');
    // 패턴4: exhaustive never check → CMX-012 억제
    if (/const\s+_\w*:\s*never\s*=/.test(code))
        detected.add('GQ-FP-004');
    // 패턴5: 선언적 파이프라인 → PRF-002 억제
    if (/\.filter\([^)]+\)\s*\.\s*map\(|\.map\([^)]+\)\s*\.\s*filter\(/.test(code))
        detected.add('GQ-FN-008');
    return detected;
}
/** 양품→불량 억제 매핑 */
const SUPPRESS_MAP = {
...
// ============================================================
function runFalsePositiveFilter(findings, filePath, code) {
        ...
        if (!isDismissed)
            for (const rule of FP_CHECKLIST) {
                if (rule.check(finding, context)) {
                    ...
                    if (rule.stage === 1)
                        stats.stage1++;
                    else if (rule.stage === 2)
                        stats.stage2++;
                    else if (rule.stage === 3)
                        stats.stage3++;
                    else
                        stats.stage4++;
                    isDismissed = true;
...
// ============================================================
function printFilterSummary(result) {
    const lines = [
        `  정수 필터: ${result.stats.total}건 → ${result.stats.kept}건 (${result.stats.total - result.stats.kept}건 제거)`,
    ];
    if (result.stats.stage1 > 0)
        lines.push(`    Stage 1 환경: ${result.stats.stage1}건 (CLI console.log 등)`);
    if (result.stats.stage2 > 0)
        lines.push(`    Stage 2 문법: ${result.stats.stage2}건 (문자열/주석/CSS)`);
    if (result.stats.stage3 > 0)
        lines.push(`    Stage 3 컨텍스트: ${result.stats.stage3}건 (catch/mock/useRef)`);
    if (result.stats.stage4 > 0)
        lines.push(`    Stage 4 자기참조: ${result.stats.stage4}건 (규칙 코드)`);
    return lines.join('\n');
}
