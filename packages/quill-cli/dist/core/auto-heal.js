...
exports.runAutoHeal = runAutoHeal;
exports.healFile = healFile;
...
async function runAutoHeal(code, functionName, onProgress) {
    const guard = (0, loop_guard_1.createLoopGuard)({ maxRounds: 6, passThreshold: 80 });
...
`;
            ...
            if (crashed) {
                ...
                try {
                    ...
                    if (parsed.error)
                        errorMsg = `${parsed.error} (input: ${fuzz.label})`;
                }
                catch { /* use stderr */ }
                if (errorMsg)
                    errors.push({ input: fuzz.label, error: errorMsg.slice(0, 200) });
            }
        }
        ...
        try {
            ...
            if (/cannot read prop|is not a function|is undefined|is null/i.test(errorMsg)) {
                const nullGuard = currentCode.replace(/(\w+)\.(\w+)\(/g, (match, obj, method) => `${obj}?.${method}(`);
            ...
            if (!localFixed && /maximum call stack|invalid array length/i.test(errorMsg)) {
                if (!currentCode.includes('if (depth >') && currentCode.includes('function')) {
                    currentCode = currentCode.replace(/(function\s+\w+\s*\([^)]*)\)/, '$1, _depth = 0)').replace(/\{(\s*\n)/, '{\n  if (_depth > 100) return undefined;\n  _depth++;\n$1');
...
// ============================================================
async function healFile(code, fileName, onProgress) {
