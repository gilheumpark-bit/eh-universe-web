...
exports.runStaticPipeline = runStaticPipeline;
exports.scanForHollowCode = scanForHollowCode;
exports.scanDeadCode = scanDeadCode;
exports.runDesignLint = runDesignLint;
exports.analyzeCognitiveLoad = analyzeCognitiveLoad;
exports.findBugsStatic = findBugsStatic;
exports.runVerificationLoop = runVerificationLoop;
exports.runProjectAudit = runProjectAudit;
exports.formatAuditReport = formatAuditReport;
exports.getScenarios = getScenarios;
exports.analyzeStress = analyzeStress;
exports.scanProject = scanProject;
// ── 팀별 정수 필터 헬퍼 ──
function filterTeamFindings(teamResult, code, filePath = 'analysis.ts') {
    try {
        const { runFalsePositiveFilter } = require('./false-positive-filter');
        const mapped = teamResult.findings.map((f) => ({
        ...
        const errors = kept.filter((f) => f.severity === 'error' || f.severity === 'critical').length;
...
async function runStaticPipeline(code, language) {
    const teams = [];
    ...
    try {
        ...
        const allFindings = engineResult.findings.map((f) => ({
            line: f.line ?? 0,
            message: f.message,
        ...
        if (multiEngine) {
            ...
            if (allFindings.length > 0) {
                allFindings[0]._engines = engineResult.enginesUsed.join('+');
            }
        }
        // 이하 기존 코드와 호환
        const seen = new Set();
        ...
        const score = Math.max(0, 100 - capped.filter((f) => f.severity === 'error').length * 10 - capped.filter((f) => f.severity === 'warning').length * 2);
    ...
    try {
        ...
        teams.push(filterTeamFindings({ name: 'design-lint', score, findings: [
                ...designFindings,
                ...(prettierResult.isFormatted ? [] : [{ line: 0, message: 'Prettier 포맷 불일치', severity: 'warning' }]),
            ] }, code));
    }
    catch {
    ...
    try {
        ...
        const findings = deepResult.findings.map((f) => ({
            line: f.line ?? 0, message: f.message,
            severity: (f.severity === 'P0' ? 'error' : 'warning'),
        }));
    ...
    const verdictTeams = teams.map((t) => ({
        name: t.name,
        findings: t.findings.slice(0, 15).map((f) => ({
    ...
    const verdict = hardFail > 0 ? 'fail' : reviewRequired > 0 ? 'review' : 'pass';
    // 하위 호환 score
...
function mapToLevel(severity, _message) {
...
function mapToConfidence(severity, _message) {
    if (severity === 'critical')
        return 'high';
    if (severity === 'error')
        return 'high';
...
// ============================================================
function runRegexTeam(code, _language) {
    const findings = [];
...
function runASTFallback(code, _language) {
    const findings = [];
    const lines = code.split('\n');
    // 함수 길이 검사
    let fnStart = -1;
    let braceDepth = 0;
    for (let i = 0; i < lines.length; i++) {
        if (/function\s|=>|class\s/.test(lines[i]))
            fnStart = i;
...
function runDeadCodeCheck(code) {
    const findings = [];
    const lines = code.split('\n');
    for (let i = 0; i < lines.length; i++) {
        // return 이후 코드
...
function runDesignLintCheck(code) {
    const findings = [];
    // z-index 하드코딩
...
function runCognitiveLoadCheck(code) {
    const findings = [];
    const lines = code.split('\n');
...
function runBugPatternCheck(code, _language) {
    const findings = [];
    const lines = code.split('\n');
    const bugPatterns = [
...
function runSecurityPatternCheck(code, _language) {
    const findings = [];
    const lines = code.split('\n');
    const secPatterns = [
    ...
    for (let i = 0; i < lines.length; i++) {
        if (ruleDefPattern.test(lines[i]))
            continue;
...
// ============================================================
async function scanForHollowCode(code, fileName = 'unknown') {
    const result = runHollowCheck(code);
    return { fileName, ...result };
}
async function scanDeadCode(code, _language = 'typescript') {
    return runDeadCodeCheck(code);
}
async function runDesignLint(code) {
    return runDesignLintCheck(code);
}
async function analyzeCognitiveLoad(code) {
    return runCognitiveLoadCheck(code);
}
async function findBugsStatic(code, language = 'typescript') {
...
// ============================================================
async function runVerificationLoop(code, language, maxRounds = 3) {
...
async function runProjectAudit(rootPath, _onProgress) {
    ...
    function walk(dir, depth = 0) {
        ...
        try {
            for (const entry of readdirSync(dir, { withFileTypes: true })) {
                if (entry.name.startsWith('.') || ['node_modules', '.next', 'dist', 'build'].includes(entry.name))
                    continue;
    ...
    const avgLines = files.length > 0
        ? Math.round(files.slice(0, 50).reduce((s, f) => { try {
            return s + readFileSync(f, 'utf-8').split('\n').length;
        }
        catch {
            return s;
        } }, 0) / Math.min(files.length, 50))
        : 0;
    ...
    if (secretCount > 0)
        urgent.push(`하드코딩 시크릿 ${secretCount}건`);
    // 4. 의존성
    ...
    for (const f of files.slice(0, 50)) {
        try {
            anyCount += (readFileSync(f, 'utf-8').match(/:\s*any\b/g) ?? []).length;
        }
        catch { /* skip */ }
    }
    ...
    for (const f of files.slice(0, 50)) {
        try {
            emptyCatch += (readFileSync(f, 'utf-8').match(/catch\s*\(\s*\)\s*\{/g) ?? []).length;
        }
        catch { /* skip */ }
    }
    ...
    for (const f of files.slice(0, 20)) {
        try {
            ...
            if (withSemi > 5 && withoutSemi > 5)
                inconsistentSemicolon++;
        }
        catch { /* skip */ }
    }
    ...
    try {
        ...
        areas.push({ name: 'ESLint', score: 70, findings: ['lint-engine 미설치'], category: 'quality' });
    }
    // 10. 미사용 의존성
    try {
        ...
        areas.push({ name: '미사용 의존성', score: 80, findings: ['depcheck 미설치'], category: 'performance' });
    }
    ...
    try {
        ...
        if (audit.critical > 0)
            urgent.push(`npm audit critical ${audit.critical}건`);
    }
    catch { /* skip */ }
    ...
    try {
        ...
        if (sampleCode) {
            ...
            const score = Math.max(0, 100 - findings.length * 10);
            areas.push({ name: 'AST 복잡도', score, findings: findings.slice(0, 3).map((f) => f.message), category: 'quality' });
        }
    }
    catch { /* skip */ }
    // 15. 구버전 API
    try {
        ...
        if (sampleCode) {
            ...
            areas.push({ name: '구버전 API', score, findings: deps.slice(0, 3).map((d) => d.message), category: 'quality' });
        }
    }
    catch { /* skip */ }
    // 16. 번들 크기
...
function formatAuditReport(report, _lang = 'ko') {
    const lines = [];
...
async function analyzeStress(code, _scenario) {
    const risks = [];
...
async function scanProject(rootPath) {
    ...
    function walk(dir, depth = 0) {
        ...
        try {
            for (const entry of readdirSync(dir, { withFileTypes: true })) {
                if (entry.name.startsWith('.') || ['node_modules', '.next', 'dist'].includes(entry.name))
                    continue;
                const full = join(dir, entry.name);
                if (entry.isDirectory())
                    walk(full, depth + 1);
                    ...
                    if (entry.name === 'pipeline-bridge.ts' || entry.name === 'pipeline-bridge.js')
                        continue;
                    try {
