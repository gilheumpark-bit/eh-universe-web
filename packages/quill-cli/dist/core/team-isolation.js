...
exports.computeTeamVerdict = computeTeamVerdict;
exports.aggregateIsolated = aggregateIsolated;
// ============================================================
// PART 2 — Team-level Verdict
// ============================================================
const BAIL_OUT_THRESHOLD = 30; // 팀당 findings 이 이상이면 bail-out
function computeTeamVerdict(teamName, findings) {
    ...
    for (const f of findings) {
        if (f.severity === 'critical')
            hardFail++;
        else if (f.severity === 'error' || f.severity === 'warning')
            review++;
        else
            note++;
    }
    const verdict = hardFail > 0 ? 'fail'
        : review > 0 ? 'review'
            : 'pass';
 ...
 */
function aggregateIsolated(teamVerdicts) {
    ...
    const overallVerdict = hasHardFail ? 'fail'
        : hasReview ? 'review'
            : 'pass';
    return {
        teamVerdicts,
