...
exports.SEED_PATTERNS = exports.CATEGORIES = void 0;
...
exports.buildReferencePrompt = buildReferencePrompt;
exports.seedDB = seedDB;
exports.getRefStats = getRefStats;
exports.loadExternalReferences = loadExternalReferences;
...
// ============================================================
function getDBDir() {
    return (0, path_1.join)((0, config_1.getGlobalConfigDir)(), 'references');
}
function getDBPath(category) {
    return (0, path_1.join)(getDBDir(), `${category}.json`);
}
function loadCategory(category) {
    ...
    if (!(0, fs_1.existsSync)(path))
        return [];
    try {
        const db = JSON.parse((0, fs_1.readFileSync)(path, 'utf-8'));
        return db.patterns;
    }
    catch {
        return [];
...
function saveCategory(category, patterns) {
    (0, fs_1.mkdirSync)(getDBDir(), { recursive: true });
    const db = { version: 1, patterns };
    (0, fs_1.writeFileSync)(getDBPath(category), JSON.stringify(db, null, 2));
}
function loadAllPatterns() {
    const all = [];
    for (const cat of exports.CATEGORIES) {
...
function removePattern(category, patternId) {
...
function recordUsage(category, patternId) {
    const patterns = loadCategory(category);
...
// ============================================================
function searchPatterns(query, framework, limit = 3) {
...
// ============================================================
function buildReferencePrompt(patterns) {
...
function seedDB() {
    let count = 0;
    for (const seed of exports.SEED_PATTERNS) {
        const existing = loadCategory(seed.category);
        if (existing.some(p => p.name === seed.name))
            continue;
        addPattern(seed);
        count++;
    }
...
function loadExternalReferences(basePath) {
    const { readdirSync, statSync } = require('fs');
    let loaded = 0;
    let skipped = 0;
    // 1. ai-reference-by-workflow
    const workflowDir = (0, path_1.join)(basePath, 'ai-reference-by-workflow');
    if ((0, fs_1.existsSync)(workflowDir)) {
        for (const [dirName, meta] of Object.entries(WORKFLOW_MAP)) {
            const dir = (0, path_1.join)(workflowDir, dirName);
            if (!(0, fs_1.existsSync)(dir))
                continue;
            for (const file of readdirSync(dir).filter((f) => f.endsWith('.ts') || f.endsWith('.tsx') || f.endsWith('.css'))) {
                ...
                if (existing.some(p => p.name === `wf:${name}`)) {
                    skipped++;
                    continue;
                }
                addPattern({
    ...
    // 2. reference-dataset
    const datasetDir = (0, path_1.join)(basePath, 'reference-dataset');
    if ((0, fs_1.existsSync)(datasetDir)) {
        for (const [dirName, meta] of Object.entries(DATASET_MAP)) {
            const dir = (0, path_1.join)(datasetDir, dirName);
            if (!(0, fs_1.existsSync)(dir))
                continue;
            for (const file of readdirSync(dir).filter((f) => f.endsWith('.ts') || f.endsWith('.js'))) {
                ...
                const existing = loadCategory(meta.category);
                if (existing.some(p => p.name === `ds:${name}`)) {
                    skipped++;
                    continue;
                }
                addPattern({
