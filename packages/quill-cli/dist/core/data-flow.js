...
exports.trackNullFlow = trackNullFlow;
exports.trackCrossFileFlow = trackCrossFileFlow;
exports.trackTaintFlow = trackTaintFlow;
exports.runFullDataFlowAnalysis = runFullDataFlowAnalysis;
...
// ============================================================
async function trackNullFlow(code, fileName) {
    const chains = [];
    const findings = [];
    try {
        ...
        for (const decl of sourceFile.getVariableDeclarations()) {
            ...
            const isNullableSource = /await\s/.test(initText) ||
            ...
            for (const ref of refs) {
                ...
                if (refLine <= decl.getStartLineNumber())
                    continue;
...
// ============================================================
async function trackCrossFileFlow(rootPath) {
    const findings = [];
    const chains = [];
    try {
        const { Project, SyntaxKind } = require('ts-morph');
                ...
                for (const ref of refs) {
                    ...
                    if (parent?.getKind() === SyntaxKind.CallExpression) {
                        ...
                        if (grandParent?.getKind() === SyntaxKind.VariableDeclaration) {
                            ...
                            const hasGuard = linesAfter.slice(0, 10).some(l => l.includes(`!${varName}`) || l.includes(`${varName} == null`) ||
                                l.includes(`${varName} === null`) || l.includes(`${varName}?.`) ||
                                l.includes(`${varName} === undefined`));
                            if (!hasGuard) {
                                // Check if there's a property access without guard
                                const hasUnsafeAccess = linesAfter.slice(0, 10).some(l => new RegExp(`\\b${varName}\\.(?!\\?)\\w`).test(l));
                                if (hasUnsafeAccess) {
                                    findings.push({
...
// ============================================================
async function trackTaintFlow(code, fileName) {
    const findings = [];
    const chains = [];
    try {
        const { Project, SyntaxKind } = require('ts-morph');
        ...
        sourceFile.forEachDescendant(node => {
            ...
            if (/req\.body|req\.params|req\.query|req\.headers|process\.env|location\.search|document\.cookie|innerHTML|localStorage\.getItem/.test(text)) {
                ...
                if (parent?.getKind() === SyntaxKind.VariableDeclaration) {
                    ...
                    if (varDecl)
                        taintSources.add(varDecl.getName());
                }
            }
        });
