...
exports.AI_PROFILES = exports.TEMPERATURE_MAP = void 0;
exports.getTemperature = getTemperature;
exports.routeTask = routeTask;
exports.getSingleKeyStrategy = getSingleKeyStrategy;
exports.recommendSecondKey = recommendSecondKey;
exports.printAIProfileSummary = printAIProfileSummary;
exports.TEMPERATURE_MAP = {
...
function getTemperature(task) {
    return exports.TEMPERATURE_MAP[task];
}
exports.AI_PROFILES = [
    // ── Anthropic ──
    {
...
function routeTask(task, availableKeys) {
    ...
    const candidates = availableKeys.map(key => {
        const profile = exports.AI_PROFILES.find(p => p.provider === key.provider && p.model === key.model)
            ?? exports.AI_PROFILES.find(p => p.provider === key.provider);
        if (!profile)
            return { key, score: 0, profile: null };
        let score = 0;
        ...
        switch (task) {
            ...
            case 'translate':
                ...
                if (profile.costTier === 'free' || profile.costTier === 'cheap')
                    score += 10;
                break;
            case 'conflict':
...
// ============================================================
function getSingleKeyStrategy(provider, model) {
    const profile = exports.AI_PROFILES.find(p => p.provider === provider && p.model === model)
        ?? exports.AI_PROFILES.find(p => p.provider === provider);
    const base = {
...
// ============================================================
function recommendSecondKey(currentProvider) {
...
function printAIProfileSummary() {
    const lines = ['🦔 AI 모델 프로필\n'];
    for (const p of exports.AI_PROFILES) {
