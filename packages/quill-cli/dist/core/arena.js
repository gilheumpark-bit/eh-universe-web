...
exports.getAgentOpinion = getAgentOpinion;
exports.runArena = runArena;
...
// ============================================================
async function collectEvidence(code, fileName) {
    const evidence = [];
    // 1. Static pipeline
    try {
...
// ============================================================
async function getAgentOpinion(code, evidence, agentRole) {
    const { streamChat } = require('./ai-bridge');
    const { getTemperature } = require('./ai-config');
    const evidenceSummary = evidence.map(e => `[${e.type}] ${e.source}: ${e.score}/100 — ${e.findings.slice(0, 5).join('; ')}`).join('\n');
    const rolePrompts = {
...
// ============================================================
async function runArena(code, fileName, onProgress) {
    ...
    opinions = [attacker, defender, judge].filter((o) => o !== null);
    ...
    if (opinions.length === 0) {
        ...
        const allDataFindings = evidence.flatMap(e => {
            const data = e.data;
            if (Array.isArray(data?.stages))
                return data.stages.filter(s => s.findings > 3).map(s => `${s.name}: ${s.findings}건`);
            if (typeof data?.message === 'string')
                return [data.message];
            return [];
        });
        let offlineVerdict = 'approve';
        const offlineCritiques = [];
        if (evidenceScore < 50) {
