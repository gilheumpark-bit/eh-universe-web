...
exports.createLoopGuard = createLoopGuard;
...
function createLoopGuard(config = {}) {
    ...
    function check(score, fixCount, costUsd) {
        state.round++;
        ...
        if (state.round >= cfg.maxRounds)
            return halt('max-iterations');
        // 3. 진전 없음
        if (state.round > 1 && score - state.previousScore < cfg.noProgressDelta) {
            return halt('no-progress');
        }
        // 4. 수정 대상 없음
        if (fixCount === 0 && state.round > 1)
            return halt('no-fixes');
        // 5. 예산 초과
        if (state.totalCostUsd >= cfg.budgetLimitUsd)
            return halt('budget-exceeded');
