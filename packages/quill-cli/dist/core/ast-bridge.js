...
exports.runEnhancedPipeline = runEnhancedPipeline;
exports.runASTHollowScan = runASTHollowScan;
...
function mapFindingToTeam(finding) {
    const msg = finding.message;
    // Team7 Release-IP: 보안 (가장 먼저 — 보안은 최우선)
    if (/eval|security|xss|injection|secret|credential|보안|개인키|API 키|password|패스워드/i.test(msg))
        return 'release-ip';
    // Team1 Simulation: 무한루프, 재귀
    if (/loop|recursive|infinite|stack overflow|루프|재귀/i.test(msg))
        return 'simulation';
    // Team6 Stability: 에러핸들링, try-catch, await
    if (/try.?catch|exception|reject|await.*without|빈 catch|unhandled/i.test(msg))
        return 'stability';
    // Team4 Size-Density: 복잡도, 중첩, 길이, 파일 크기
    if (/nest|depth|complex|cognitive|줄 초과|줄 길이|중첩|깊이|함수.*줄|파일.*줄|파라미터.*개|삼항/i.test(msg))
        return 'size-density';
    // Team5 Asset-Trace: 미사용, 데드코드
    if (/unused|dead|unreachable|orphan|미사용|@ts-ignore/i.test(msg))
        return 'asset-trace';
    // Team3 Validation: 타입, null, ===/!==
    if (/null|undefined|any 타입|===|!==|==\s|!=\s|타입.*안전|nullable|optional/i.test(msg))
        return 'validation';
    // Team2 Generation: 빈함수, TODO, console
    if (/empty function|빈 함수|todo|fixme|hack|console\.|stub|스텁/i.test(msg))
        return 'generation';
    // Team8 Governance: 아키텍처, 의존성
    if (/import|dependency|circular|architecture|의존/i.test(msg))
        return 'governance';
...
// ============================================================
async function runEnhancedPipeline(code, language, fileName, regexResult) {
    const findings = [];
    const engines = [];
    ...
    if (regexResult) {
        ...
        for (const stage of regexResult.teams) {
            for (const finding of stage.findings) {
                regexFindingCount++;
                const msg = typeof finding === 'string' ? finding : finding.message ?? String(finding);
                findings.push({
                    engine: 'regex',
                    line: typeof finding === 'object' ? finding.line ?? 0 : 0,
                    message: msg,
                    severity: 'warning',
...
// ============================================================
async function runASTHollowScan(code, fileName) {
...
async function runASTHollowScanSingle(code, fileName) {
    const findings = [];
    try {
        ...
        sourceFile.forEachDescendant(node => {
            if (node.getKind() === SyntaxKind.AwaitExpression) {
                ...
                while (parent) {
                    ...
                    if (parent.getKind() === SyntaxKind.ArrowFunction || parent.getKind() === SyntaxKind.FunctionDeclaration)
                        break;
        ...
        sourceFile.forEachDescendant(node => {
            if (loopKinds.has(node.getKind())) {
                ...
                let parent = node.getParent();
                while (parent) {
                    if (loopKinds.has(parent.getKind()))
                        depth++;
                    parent = parent.getParent();
                }
        ...
        for (const detector of registry.getDetectors()) {
            ...
            for (const pf of pFindings) {
                ...
                let mappedSev = 'warning';
                if (ruleMeta.severity === 'critical')
                    mappedSev = 'critical';
                else if (ruleMeta.severity === 'high')
                    mappedSev = 'error';
                else if (ruleMeta.severity === 'info')
                    mappedSev = 'info';
                // Find confidence math
                let mappedConf = 0.8;
                if (ruleMeta.confidence === 'high')
                    mappedConf = 0.95;
                else if (ruleMeta.confidence === 'low')
                    mappedConf = 0.6;
                findings.push({
