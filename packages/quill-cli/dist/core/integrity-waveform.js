...
exports.analyzeWaveform = analyzeWaveform;
exports.adjustForAnomalies = adjustForAnomalies;
// ============================================================
// PART 2 — Waveform Calculator
// ============================================================
function analyzeWaveform(fileFindings) {
    ...
    const points = fileFindings.map(f => {
        const zScore = stdDev > 0 ? (f.findings - mean) / stdDev : 0;
        let status = 'normal';
        if (f.findings > threshold)
            status = 'anomaly';
        else if (zScore > 1.5)
            status = 'elevated';
 ...
 */
function adjustForAnomalies(allDetails, anomalyFiles) {
