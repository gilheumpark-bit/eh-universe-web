...
exports.CHALLENGES = exports.BADGES = void 0;
exports.buildBadgeContext = buildBadgeContext;
...
exports.generateReadmeBadge = generateReadmeBadge;
...
function getBadgePath() {
    return (0, path_1.join)((0, config_1.getGlobalConfigDir)(), 'badges.json');
}
...
function saveUserBadges(badges) {
    ...
    (0, fs_1.writeFileSync)(getBadgePath(), JSON.stringify(badges, null, 2));
}
function buildBadgeContext() {
    const receiptDir = (0, path_1.join)(process.cwd(), '.cs', 'receipts');
    const receipts = [];
    if ((0, fs_1.existsSync)(receiptDir)) {
        for (const f of (0, fs_1.readdirSync)(receiptDir).filter(f => f.endsWith('.json')).sort().reverse()) {
            try {
                receipts.push(JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(receiptDir, f), 'utf-8')));
            }
            catch { /* skip */ }
        }
    }
    ...
    for (const r of receipts) {
        if (r.pipeline.overallStatus === 'pass')
            consecutivePasses++;
        else
            break;
    }
...
function evaluateChallenges() {
    const ctx = buildBadgeContext();
    return exports.CHALLENGES.map(c => ({ challenge: c, ...c.check(ctx) }));
}
// ============================================================
// PART 3 — Share Card (ASCII)
// ============================================================
function generateShareCard(projectName, score, badges) {
...
function generateReadmeBadge(projectName, score) {
