...
exports.recordAcceptance = recordAcceptance;
...
// ============================================================
function getDBPath() {
    return (0, path_1.join)((0, config_1.getGlobalConfigDir)(), 'fix-memory.json');
}
function loadDB() {
...
function saveDB(db) {
    ...
    (0, fs_1.writeFileSync)(getDBPath(), JSON.stringify(db, null, 2));
}
...
function recordFix(pattern) {
    const db = loadDB();
    // Check for existing similar pattern
    const existing = db.patterns.find(p => p.category === pattern.category &&
...
function recordAcceptance(patternId, accepted) {
    const db = loadDB();
...
function findSimilarFixes(description, category) {
    const db = loadDB();
    return db.patterns
