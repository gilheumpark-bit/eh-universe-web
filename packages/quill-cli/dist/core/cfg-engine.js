...
exports.findRiskPaths = findRiskPaths;
exports.sliceContext = sliceContext;
exports.buildCFGReviewPrompt = buildCFGReviewPrompt;
exports.runBrainAnalysis = runBrainAnalysis;
...
async function buildCFG(code, fileName) {
...
async function buildCFGWithAST(code, fileName) {
    ...
    function visitStatement(stmt, prevId) {
        ...
        if (ts.isIfStatement(stmt)) {
            ...
            const thenEnd = ts.isBlock(stmt.thenStatement)
                ? visitBlock(stmt.thenStatement.statements, branchId)
            ...
            if (stmt.elseStatement) {
                const elseEnd = ts.isBlock(stmt.elseStatement)
                    ? visitBlock(stmt.elseStatement.statements, branchId)
            ...
            if (stmt.statement && ts.isBlock(stmt.statement)) {
                const bodyEnd = visitBlock(stmt.statement.statements, loopId);
            ...
            let lastId = tryId;
            if (stmt.tryBlock)
                lastId = visitBlock(stmt.tryBlock.statements, tryId);
            ...
            for (const clause of stmt.caseBlock.clauses) {
                ...
                const caseEnd = visitBlock(clause.statements, caseId);
            ...
            if (stmt.body)
                visitBlock(stmt.body.statements, fnId);
            return fnId;
        }
        else {
            // 일반 statement
            const type = /await\s|\.then\(|\bfetch\(/.test(text) ? 'call' : 'statement';
...
function buildCFGRegex(code, fileName) {
    nodeCounter = 0;
    const nodes = new Map();
...
function extractVariables(line) {
    ...
    for (const id of identifiers) {
        if (!keywords.has(id) && !defined.includes(id))
            used.push(id);
    }
...
// ============================================================
function findRiskPaths(graph) {
    const paths = [];
    // Find nullable paths: variable from API call used without null check
    const nullableSources = new Set();
    const guardedVars = new Set();
...
// ============================================================
function sliceContext(code, graph, riskPaths) {
...
// ============================================================
function buildCFGReviewPrompt(fileName, riskPaths, slicedContext, graph) {
    const lines = [
...
// ============================================================
async function runBrainAnalysis(code, fileName) {
