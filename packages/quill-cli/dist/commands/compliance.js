...
exports.LICENSE_COMPAT_MATRIX = void 0;
...
exports.collectSBOMComponents = collectSBOMComponents;
...
exports.LICENSE_COMPAT_MATRIX = LICENSE_COMPAT_MATRIX;
function checkLicenseCompatibility(projectLicense, deps) {
    const issues = [];
    const matrix = LICENSE_COMPAT_MATRIX[projectLicense];
    if (!matrix)
        return issues; // Unknown project license, can't check
...
async function runCompliance(_opts) {
        ...
        if (existsSync(nodeModules)) {
            ...
            for (const dir of dirs.slice(0, 200)) {
                ...
                try {
                    ...
                    if (/\b(A?GPL)\b/i.test(lic) && !/LGPL/i.test(lic))
                        copyleftCount++;
                }
                catch { /* skip */ }
            }
    ...
    if (existsSync(srcDir)) {
        function scanSecrets(dir) {
            ...
            for (const entry of entries) {
                ...
                if (!/\.(ts|tsx|js|jsx|json|env)$/.test(entry.name))
                    continue;
                try {
    ...
    try {
        ...
        if (audit.critical > 0) {
            depPassed = false;
            depDetail += ` | critical ${audit.critical}건`;
        }
        else
            depDetail += ` | 취약점 없음`;
    }
    ...
    try {
        ...
        if (retire.vulnerableCount > 0) {
            depPassed = false;
            depDetail += ` | retire ${retire.vulnerableCount}건`;
        }
    }
    ...
    try {
        ...
        if (existsSync(srcDirQ)) {
            const sampleFiles = readdirSync(srcDirQ).filter((f) => f.endsWith('.ts') || f.endsWith('.tsx')).slice(0, 5);
        ...
        codeQualityDetail = 'verify 실행 실패 — cs verify 로 수동 확인';
    }
...
// ============================================================
async function generateSBOM(format = 'cyclonedx') {
    const pkgPath = join(process.cwd(), 'package.json');
    if (!existsSync(pkgPath))
        return JSON.stringify({ error: 'package.json not found' });
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf-8'));
...
function generateCycloneDX(pkg, components) {
...
function generateSPDX(pkg, components) {
 ...
 */
async function exportSBOM(format = 'cyclonedx', outputPath) {
