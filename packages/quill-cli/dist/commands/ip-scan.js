...
exports.SPDX_LICENSE_DB = void 0;
exports.runIpScan = runIpScan;
exports.matchLicense = matchLicense;
exports.scanDependencyLicenses = scanDependencyLicenses;
exports.calculateIPScore = calculateIPScore;
...
exports.SPDX_LICENSE_DB = SPDX_LICENSE_DB;
/**
 ...
 */
function matchLicense(licenseStr) {
    if (!licenseStr)
        return null;
    ...
    for (const entry of SPDX_LICENSE_DB) {
        if (entry.regex.test(licenseStr))
            return entry;
    }
    return null;
}
 ...
 */
function scanDependencyLicenses(rootPath) {
    const results = [];
    const seen = new Set();
    ...
    if (existsSync(lockPath)) {
        try {
            ...
            if (lockfile.dependencies && results.length === 0) {
                function walkLockDeps(deps, path) {
...
function readLicenseFromNodeModules(rootPath, name) {
...
function scanNodeModulesDir(nodeModules, results, seen, directDeps) {
    try {
...
function readDepLicenseInfo(depPath, name, isDirect) {
...
function scanSourceCode(rootPath) {
    const findings = [];
    function walk(dir) {
        let entries;
        try {
...
function calculateIPScore(findings, depLicenses) {
...
// ============================================================
async function runIpScan(_path, _opts) {
