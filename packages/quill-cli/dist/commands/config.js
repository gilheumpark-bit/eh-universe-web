...
const { loadGlobalConfig, saveGlobalConfig, addKey, removeKey, getGlobalConfigPath, } = require('../core/config');
const { readFileSync, writeFileSync, existsSync } = require('fs');
const { join } = require('path');
const VALID_LANGUAGES = ['ko', 'en', 'ja', 'zh'];
const VALID_LEVELS = ['easy', 'normal', 'pro'];
const VALID_STRUCTURES = ['auto', 'on', 'off'];
const VALID_FILE_MODES = ['safe', 'auto', 'yolo'];
const VALID_PROVIDERS = ['anthropic', 'openai', 'google', 'groq', 'mistral', 'ollama', 'lm-studio'];
const VALID_ROLES = ['generate', 'verify', 'judge'];
function validateConfig(config) {
    ...
    if (config.keys && Array.isArray(config.keys)) {
        for (const [i, key] of config.keys.entries()) {
            ...
            if (!VALID_PROVIDERS.includes(key.provider)) {
...
function diffConfigs(oldConfig, newConfig) {
    const diffs = [];
    const simpleFields = ['language', 'level', 'structure', 'fileMode', 'framework'];
...
function exportConfig(config, includeSensitive = false) {
    const exported = {
...
// ============================================================
async function runConfig(action, extraArg) {
    ...
    switch (action) {
        ...
        case 'keys-add': {
            ...
            const prompt = (q, def) => new Promise(r => {
            ...
            // Validate roles
            const parsedRoles = roles.split(',').map((r) => r.trim());
        ...
        case 'keys-remove': {
            ...
            const idx = await new Promise(r => rl2.question('  삭제할 번호: ', (a) => { rl2.close(); r(a.trim()); }));
        ...
        case 'structure': {
            const oldConfig = JSON.parse(JSON.stringify(config));
            const validValues = ['auto', 'on', 'off'];
            if (extraArg && validValues.includes(extraArg)) {
                config.structure = extraArg;
            }
            else {
        ...
        case 'level': {
            const oldConfig = JSON.parse(JSON.stringify(config));
            const validLevels = ['easy', 'normal', 'pro'];
            if (extraArg && validLevels.includes(extraArg)) {
                config.level = extraArg;
            }
            else {
        ...
        case 'filemode': {
            const oldConfig = JSON.parse(JSON.stringify(config));
            const validModes = ['safe', 'auto', 'yolo'];
            if (extraArg && validModes.includes(extraArg)) {
                config.fileMode = extraArg;
            }
            else {
        ...
        case 'language': {
            const oldConfig = JSON.parse(JSON.stringify(config));
            if (extraArg && VALID_LANGUAGES.includes(extraArg)) {
                config.language = extraArg;
            }
            else {
        ...
        case 'import': {
            ...
            try {
                ...
                if (imported.keys)
                    merged.keys = imported.keys;
                const diffs = diffConfigs(oldConfig, merged);
                if (diffs.length > 0) {
                    printDiff(diffs);
                    // Apply
        ...
        case 'diff': {
            ...
            try {
                ...
                const merged = { ...loadGlobalConfig(), ...otherConfig };
                if (otherConfig.keys)
                    merged.keys = otherConfig.keys;
