...
async function runBench(path, opts) {
    ...
    if (worst && worst.score < 70) {
        ...
        if (worst.length > 50)
            console.log('     → 함수 분리 (PART 구조)');
        if (worst.complexity > 10)
            console.log('     → early return 패턴으로 중첩 줄이기');
    }
    console.log('');
    try {
        const { recordCommand } = require('../core/session');
        recordCommand(`bench ${path}`);
    }
    catch { }
    const result = { functions: scored, timestamp: Date.now() };
    // Save baseline
    if (opts.save) {
        const benchDir = (0, path_1.join)(process.cwd(), '.cs', 'bench');
        (0, fs_1.mkdirSync)(benchDir, { recursive: true });
        (0, fs_1.writeFileSync)((0, path_1.join)(benchDir, `${opts.save}.json`), JSON.stringify(result, null, 2));
        console.log(`  📌 베이스라인 저장: ${opts.save}\n`);
    }
    // Compare with baseline
    if (opts.compare) {
        const benchPath = (0, path_1.join)(process.cwd(), '.cs', 'bench', `${opts.compare}.json`);
        if (!(0, fs_1.existsSync)(benchPath)) {
            console.log(`  ⚠️  베이스라인 "${opts.compare}" 없음`);
            return;
        }
        const baseline = JSON.parse((0, fs_1.readFileSync)(benchPath, 'utf-8'));
