...
function detectFrameworks() {
    ...
    try {
        ...
        const unknownSignificant = Object.keys(allDeps).filter(dep => {
            if (KNOWN_FRAMEWORKS[dep])
                return false;
            if (dep.startsWith('@types/'))
                return false;
            // Heuristic: scoped packages with "framework-like" names
            if (dep.startsWith('@') && /\/(core|cli|server|client|app)$/.test(dep))
                return true;
            return false;
        });
...
function detectMonorepo() {
    ...
    const result = { type: 'none', workspaces: [], packages: [] };
    try {
        // Check for Turborepo
        ...
        if (existsSync(lernaPath)) {
            ...
            try {
                ...
                if (lerna.packages)
                    result.workspaces = lerna.packages;
            }
            catch { /* skip */ }
        }
...
function detectTestFramework() {
    const cwd = process.cwd();
    const result = { framework: null, configFile: null, testDirs: [], hasTestScript: false };
    try {
        const pkgPath = join(cwd, 'package.json');
        if (!existsSync(pkgPath))
            return result;
        ...
        if (!result.framework && result.hasTestScript) {
            ...
            else if (/jest/.test(testScript))
                result.framework = 'Jest';
            else if (/mocha/.test(testScript))
                result.framework = 'Mocha';
            else if (/playwright/.test(testScript))
                result.framework = 'Playwright';
            else if (/cypress/.test(testScript))
...
async function runInit() {
    ...
    const langMap = { '1': 'ko', '2': 'en', '3': 'ja', '4': 'zh', 'ko': 'ko', 'en': 'en', 'ja': 'ja', 'zh': 'zh' };
    ...
    if (config.keys.length === 0) {
        ...
        if (addKeyAnswer === 'y' || addKeyAnswer === 'Y') {
            ...
            const providers = { '1': 'anthropic', '2': 'openai', '3': 'google', '4': 'groq', '5': 'ollama' };
    ...
    const structMap = { '1': 'auto', '2': 'on', '3': 'off' };
    ...
    const levelMap = { '1': 'easy', '2': 'normal', '3': 'pro' };
    ...
    const seeded = seedDB();
    if (seeded > 0)
        console.log(`  📚 내장 레퍼런스: ${seeded}개 패턴 추가`);
    // new1/ 외부 레퍼런스 자동 탐색
    const refCandidates = [
    ...
    for (const refPath of refCandidates) {
        if (existsSync(refPath)) {
            ...
            if (ext.loaded > 0)
                console.log(`  📚 외부 레퍼런스: ${ext.loaded}개 로드 (${refPath.split(/[/\\]/).pop()})`);
            break;
        }
    }
