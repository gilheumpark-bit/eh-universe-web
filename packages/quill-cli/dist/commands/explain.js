...
// ============================================================
async function runExplain(path) {
    ...
    const code = (0, fs_1.readFileSync)(path, 'utf-8');
    ...
    try {
        ...
        try {
            ...
            const funcNames = functions.map((f) => f.getName() || '<anonymous>');
            const classNames = classes.map((c) => c.getName() || '<anonymous>');
            const ifaceNames = interfaces.map((i) => i.getName());
            const typeNames = typeAliases.map((t) => t.getName());
            ...
            if (funcNames.length > 0)
                console.log(`                 ${funcNames.slice(0, 6).join(', ')}${funcNames.length > 6 ? '...' : ''}`);
            ...
            if (typeNames.length > 0)
                console.log(`     타입:       ${typeNames.length}개 (${typeNames.join(', ')})`);
            if (enums.length > 0)
                console.log(`     Enum:       ${enums.length}개`);
            console.log(`     Export:     ${exportedDecls.size}개`);
            if (exportedArrows.length > 0)
                console.log(`                 arrow exports: ${exportedArrows.join(', ')}`);
            ...
            for (const fn of functions) {
                ...
                if (complexity > 3)
                    complexFns.push({ name: fn.getName() || '<anon>', complexity });
            }
