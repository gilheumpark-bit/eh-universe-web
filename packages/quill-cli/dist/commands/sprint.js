...
function buildExecutionWaves(tasks, deps) {
    const waves = [];
...
// ============================================================
async function runSprint(tasksInput) {
    ...
    for (let waveIdx = 0; waveIdx < waves.length; waveIdx++) {
        ...
        if (isParallel) {
            ...
            const wavePromises = wave.map(async (taskIdx) => {
                ...
                try {
                    await (0, generate_1.runGenerate)(task, { mode: 'full', structure: 'auto' });
                    return { task, index: taskIdx, success: true, duration: Math.round(performance.now() - start), wave: waveIdx };
                }
                catch (err) {
                    return { task, index: taskIdx, success: false, duration: Math.round(performance.now() - start), wave: waveIdx, error: err.message };
                }
            });
    ...
    try {
        ...
        recordCommand(`sprint ${tasks.length}`);
    }
    catch { }
}
