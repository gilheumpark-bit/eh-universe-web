...
async function runLearn(topic) {
    ...
    const files = (0, fs_1.readdirSync)(receiptDir).filter(f => f.endsWith('.json')).sort().reverse();
    if (files.length === 0) {
        console.log('  📭 영수증이 없습니다.\n');
        return;
    }
    const latest = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(receiptDir, files[0]), 'utf-8'));
...
Use Korean. Be encouraging. Use 💡 for tips. Keep it SHORT (3-5 lines per section).`;
        ...
        for (const team of problemTeams) {
            ...
            if (catalogRules.length > 0 && categories.length > 0) {
                ...
                const relevantRules = catalogRules.filter((r) => categories.some(cat => r.category === cat || r.category?.includes(cat)));
                // Group by severity for actionable display
                const critical = relevantRules.filter((r) => r.severity === 'critical' || r.severity === 'high');
                const medium = relevantRules.filter((r) => r.severity === 'medium');
                // Show what this team checks based on actual rule titles
                const sampleTitles = relevantRules.slice(0, 4).map((r) => r.title).join(', ');
