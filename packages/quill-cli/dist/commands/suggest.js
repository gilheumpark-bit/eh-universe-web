...
function analyzeProject() {
    ...
    if ((0, fs_1.existsSync)(tsconfigPath)) {
        try {
            ...
            const raw = (0, fs_1.readFileSync)(tsconfigPath, 'utf-8').replace(/\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '');
    ...
    let totalFiles = 0;
    let totalLines = 0;
    const dirStructure = new Set();
    function scanStructure(dir) {
        try {
            const entries = (0, fs_1.readdirSync)(dir, { withFileTypes: true });
            for (const e of entries) {
                if (e.name === 'node_modules' || e.name === '.git' || e.name === '.next' || e.name.startsWith('.'))
                    continue;
                ...
                totalFiles++;
                try {
                    totalLines += (0, fs_1.readFileSync)(full, 'utf-8').split('\n').length;
                }
                catch { /* skip */ }
            }
        }
    ...
    const srcDir = (0, path_1.join)(root, 'src');
    if ((0, fs_1.existsSync)(srcDir))
        scanStructure(srcDir);
    // ── Check for missing tests ──
    const hasTests = (0, fs_1.existsSync)((0, path_1.join)(root, '__tests__')) || (0, fs_1.existsSync)((0, path_1.join)(root, 'src/__tests__')) || (0, fs_1.existsSync)((0, path_1.join)(root, 'tests'));
    ...
    if (!(0, fs_1.existsSync)((0, path_1.join)(root, 'LICENSE')) && !(0, fs_1.existsSync)((0, path_1.join)(root, 'LICENSE.md'))) {
        suggestions.push({
    ...
    if ((0, fs_1.existsSync)((0, path_1.join)(root, '.env')) && !(0, fs_1.existsSync)((0, path_1.join)(root, '.env.example'))) {
        suggestions.push({
    ...
    if ((0, fs_1.existsSync)((0, path_1.join)(root, '.gitignore'))) {
        try {
            const gitignore = (0, fs_1.readFileSync)((0, path_1.join)(root, '.gitignore'), 'utf-8');
            const shouldIgnore = ['.env', 'node_modules', '.next', 'dist', 'build'];
            const missing = shouldIgnore.filter(item => !gitignore.includes(item));
            if (missing.length > 0 && (0, fs_1.existsSync)((0, path_1.join)(root, missing[0]))) {
                suggestions.push({
    ...
    else if ((0, fs_1.existsSync)((0, path_1.join)(root, '.git'))) {
        suggestions.push({
        ...
        try {
            ...
            for (const e of entries) {
                ...
                if (!/\.(ts|tsx|js|jsx)$/.test(e.name))
                    continue;
                try {
                    const c = (0, fs_1.readFileSync)(full, 'utf-8');
