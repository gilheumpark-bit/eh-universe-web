...
function loadReceipts(filter) {
    ...
    if (!(0, fs_1.existsSync)(receiptDir))
        return [];
    const files = (0, fs_1.readdirSync)(receiptDir).filter(f => f.endsWith('.json'));
    const receipts = [];
    const now = Date.now();
    ...
    for (const file of files) {
        try {
            const data = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(receiptDir, file), 'utf-8'));
...
async function runReport(opts) {
    ...
    if (receipts.length >= 2) {
        ...
        for (const [name, data] of teamTrends) {
            if (data.recent.length === 0 || data.older.length === 0)
                continue;
