...
exports.getQuillMood = getQuillMood;
exports.generateCodePoem = generateCodePoem;
exports.runQuiz = runQuiz;
exports.generateCodeArt = generateCodeArt;
exports.showFortune = showFortune;
exports.createApp = createApp;
exports.showChallenge = showChallenge;
...
const QUILL_ART_SAD = `
  ...
  /||||||\\
`;
function getQuillMood(score) {
    if (score >= 95)
        return QUILL_ART_CELEBRATE;
    if (score >= 70)
        return QUILL_ART;
...
// ============================================================
async function generateCodePoem(topic) {
...
async function runQuiz() {
    ...
    try {
        ...
        const raw = await quickAsk('Generate a JavaScript/TypeScript code quiz. Return ONLY valid JSON with this exact structure: {"question":"...","code":"...","options":["A) ...","B) ...","C) ...","D) ..."],"answer":0,"explanation":"..."}. The answer field is the 0-based index of the correct option. Make it tricky but educational. Language: Korean for text, English for code.', 'You are a programming quiz master. Output ONLY raw JSON, no markdown fences, no extra text.', 'creative');
...
// ============================================================
async function generateCodeArt(subject) {
...
async function showFortune() {
    ...
    try {
        ...
        const raw = await quickAsk(`Today is ${today}. Generate a fun developer fortune cookie. Return ONLY valid JSON: {"fortune":"...","lucky":"...","avoid":"..."}. fortune: a witty coding fortune in Korean with one emoji. lucky: a lucky programming keyword. avoid: something to avoid today. Be creative and different each time.`, 'You are a mystical developer fortune teller. Output ONLY raw JSON, no markdown.', 'creative');
        const parsed = JSON.parse(raw.replace(/```json?\s*/g, '').replace(/```/g, '').trim());
...
async function createApp(description) {
    ...
    try {
        ...
        if (result.installCommand)
            console.log(`  📦 설치: ${result.installCommand}`);
        if (result.startCommand)
            console.log(`  🚀 실행: ${result.startCommand}`);
        console.log(`\n  ${result.summary}\n`);
    }
    catch {
...
async function showChallenge() {
    ...
    try {
        const { quickAsk } = require('../core/ai-bridge');
        const raw = await quickAsk('Generate a unique coding challenge for a developer. Return ONLY valid JSON: {"title":"...","desc":"...","difficulty":"쉬움|보통|어려움|매우 어려움","hints":["...","..."]}. Title and description in Korean. Make it practical and educational. Include 2 hints.', 'You are a coding challenge designer. Output ONLY raw JSON, no markdown.', 'creative');
        const parsed = JSON.parse(raw.replace(/```json?\s*/g, '').replace(/```/g, '').trim());
...
// PART 8 — Fun Runner
// ============================================================
async function runFun(action, args) {
    switch (action) {
        case 'poem':
