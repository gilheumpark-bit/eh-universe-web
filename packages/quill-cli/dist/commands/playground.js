...
async function runPlayground(opts) {
    ...
    function countSrc(dir) {
        try {
            ...
            for (const e of entries) {
                if (e.name === 'node_modules' || e.name === '.next' || e.name === '.git' || e.name.startsWith('.'))
                    continue;
    ...
    function sampleAST(dir, limit) {
        if (astPipelineCount >= limit)
            return;
        try {
            const entries = readdirSync(dir, { withFileTypes: true });
            for (const e of entries) {
                ...
                try {
                    ...
                    const astTeam = result.teams?.find((t) => t.name === 'ast');
                    if (astTeam) {
    ...
    let qualCount = 0;
    const qualFileQueue = [];
    function collectQualFiles(dir, limit) {
        if (qualFileQueue.length >= limit)
            return;
        try {
            const entries = readdirSync(dir, { withFileTypes: true });
            for (const e of entries) {
    ...
    function scanSecrets(dir) {
        try {
            ...
            for (const e of entries) {
                ...
                if (!/\.(ts|tsx|js|jsx|json)$/.test(e.name) || e.name === 'package-lock.json')
                    continue;
                try {
                    const c = readFileSync(p, 'utf-8');
                    if (/sk-[a-zA-Z0-9]{20,}|AIza[a-zA-Z0-9_-]{30,}|password\s*=\s*["'][^"']{5,}["']/i.test(c))
                        secretHits++;
                }
                catch { /* skip */ }
            }
