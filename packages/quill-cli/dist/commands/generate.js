...
const loop_guard_1 = require("../core/loop-guard");
const receipt_1 = require("../formatters/receipt");
// IDENTITY_SEAL: PART-1 | role=types | inputs=none | outputs=GenerateOptions,GeneratedPart
// ============================================================
// PART 2 — SEAL Header Generator
// ============================================================
function generateSealHeader(contract) {
    return [
        `// ============================================================`,
...
function generateSealFooter(contract) {
...
function shouldUseParts(totalLines, structure) {
    if (structure === 'on')
        return true;
    if (structure === 'off')
        return false;
...
// PART 3 — Code Merger
// ============================================================
function mergeGeneratedParts(parts, structure) {
...
function buildQualityRulesFromCatalog() {
    const filtered = GOOD_PATTERN_CATALOG.filter((p) => p.signal === 'boost' && p.confidence === 'high');
    const groups = {};
...
async function runGenerate(prompt, opts) {
    ...
    const context = (0, fs_1.existsSync)(pkgPath) ? (0, fs_1.readFileSync)(pkgPath, 'utf-8').slice(0, 2000) : undefined;
    // ── Step 1: Plan ──
    ...
    try {
        ...
        const refPath = (0, path_1.join)(process.cwd(), '..', 'new1');
        const { existsSync: refExists } = require('fs');
        // 프로젝트 상위 또는 직박구리 내 new1 폴더 탐색
        const candidates = [refPath, (0, path_1.join)(process.cwd(), 'new1'), (0, path_1.join)(process.cwd(), '..', '..', 'new1')];
        for (const p of candidates) {
            if (refExists(p)) {
                loadExternalReferences(p);
                break;
            }
        }
    }
    ...
    const planPrompt = (0, planner_1.buildPlannerPrompt)(prompt, extraContext || undefined);
    ...
    let planRaw = '';
    await streamChat({
        systemInstruction: planner_1.PLANNER_SYSTEM_PROMPT,
        messages: [{ role: 'user', content: planPrompt }],
        onChunk: (t) => { planRaw += t; },
        temperature: getTemperature('plan'),
    });
    let plan = (0, planner_1.parsePlanResult)(planRaw);
    ...
    if (!plan) {
        ...
        plan = (0, planner_1.parsePlanResult)(planRaw);
    }
    if (!plan) {
    ...
    if (opts.dryRun) {
        ...
        console.log(`     PART:      ${plan.totalParts}개 (${(0, planner_1.buildExecutionWaves)(plan.contracts).length} wave 병렬)`);
        ...
        if (references.length > 0)
            console.log(`     레퍼런스:  ${references.length}개`);
        console.log('\n  (--dry-run: 실행하지 않음)');
        return;
    }
    // ── Step 2: Parallel Generate ──
    const waves = (0, planner_1.buildExecutionWaves)(plan.contracts);
    ...
    for (let wi = 0; wi < waves.length; wi++) {
        ...
        const waveResults = await Promise.all(wave.map(async (partNum) => {
            ...
            code = code.replace(/^```\w*\n?/gm, '').replace(/```$/gm, '').trim();
            return { part: partNum, code, contract };
        }));
        ...
        try {
            ...
            const judgeFindings = pipelineResult.teams.flatMap((s) => {
                ...
                return findings.map((f, fi) => ({
                    ...
                    message: typeof f === 'string' ? f : typeof f === 'object' && f !== null && 'message' in f ? String(f.message) : String(f),
    ...
    const guard = (0, loop_guard_1.createLoopGuard)({ passThreshold: opts.mode === 'strict' ? 85 : 77 });
    ...
    // ── Step 6: Save + Receipt ──
    console.log('\n  [6/6] 💾 저장 + 영수증 발급...');
    const csDir = (0, path_1.join)(process.cwd(), '.cs', 'generated');
    (0, fs_1.mkdirSync)(csDir, { recursive: true });
    const filePath = (0, path_1.join)(csDir, fileName);
    (0, fs_1.writeFileSync)(filePath, finalCode, 'utf-8');
    console.log(`        → ${filePath}`);
    // Receipt
    const codeHash = (0, crypto_1.createHash)('sha256').update(finalCode).digest('hex');
    const receiptData = {
    ...
    const receiptHash = (0, receipt_1.computeReceiptHash)(receiptData);
    const receipt = { ...receiptData, receiptHash };
    (0, receipt_1.chainReceipt)(receipt);
    // Save receipt
    const receiptDir = (0, path_1.join)(process.cwd(), '.cs', 'receipts');
    (0, fs_1.mkdirSync)(receiptDir, { recursive: true });
    (0, fs_1.writeFileSync)((0, path_1.join)(receiptDir, `${receipt.id}.json`), JSON.stringify(receipt, null, 2));
    console.log('\n' + (0, receipt_1.formatReceipt)(receipt, 'ko'));
    // Deprecation check
    try {
    ...
    try {
        ...
        for (const stage of pipelineResult.teams) {
            for (const finding of stage.findings) {
                recordFix({
                    category: stage.name,
                    description: typeof finding === 'string' ? finding : finding.message ?? String(finding),
                    beforePattern: '',
                    afterPattern: '',
                    confidence: 0.5,
                });
    ...
    if (opts.withTests) {
        ...
        try {
            ...
            await streamChat({
                ...
                onChunk: (t) => { testCode += t; },
            });
            testCode = testCode.replace(/^```\w*\n?/gm, '').replace(/```$/gm, '').trim();
            const testPath = (0, path_1.join)(csDir, fileName.replace('.ts', '.test.ts'));
            (0, fs_1.writeFileSync)(testPath, testCode, 'utf-8');
    ...
    if (opts.commit) {
        ...
        try {
            ...
            try {
                ...
                if (aiMsg.trim().length > 5)
                    commitMsg = aiMsg.trim().split('\n')[0];
            }
    ...
    try {
        ...
        if (newBadges.length > 0) {
            for (const b of newBadges)
                console.log(`  🏆 ${b.icon} ${b.name} 획득! — ${b.description}`);
        }
    }
