...
function discoverFiles(rootPath) {
    ...
    function walk(dir, depth = 0) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isSymbolicLink())
                continue; // symlink 공격 방지
            const fullPath = (0, path_1.join)(dir, entry.name);
            if (entry.isDirectory()) {
                walk(fullPath, depth + 1);
            }
            else if (SUPPORTED_EXTENSIONS.has((0, path_1.extname)(entry.name))) {
                try {
    ...
    try {
        const stat = (0, fs_1.statSync)(rootPath);
        if (stat.isFile()) {
            if (stat.size > MAX_FILE_SIZE)
                return [];
            const content = (0, fs_1.readFileSync)(rootPath, 'utf-8');
            const ext = (0, path_1.extname)(rootPath);
            return [{
                    path: rootPath,
...
async function runVerify(path, opts) {
    ...
    if (useParallel)
        console.log(`  ${icons.rocket} 워커풀 병렬 모드 (${Math.min(files.length, 4)} workers)`);
    console.log('');
    // Run pipeline on each file — try enhanced (AST) first, fallback to regex
    let useEnhanced = true;
    let enhancedImport = null;
    try {
        ...
        if (process.env.CS_DEBUG)
            console.error('  [DEBUG] ast-bridge load failed:', e.message);
    }
    const { runStaticPipeline } = require('../core/pipeline-bridge');
    const allTeamScores = new Map();
    const allTeamFindings = new Map();
    let totalFindings = 0;
    let astFindingsTotal = 0;
    // ── 파일 검증 함수 ──
    async function verifyOneFile(file) {
        if (useEnhanced && enhancedImport) {
            try {
                ...
                result._details = enhanced.findings.map(f => ({
                ...
                if (process.env.CS_DEBUG)
                    console.error(`  [DEBUG] enhanced failed for ${file.relativePath}:`, enhErr.message?.slice(0, 100));
    ...
    // ── 양품 패턴 감지 추적 ──
    let goodPatternCatalog = null;
    let detectGoodPatternsFn = null;
    try {
    ...
    let goodDowngradedFindings = 0;
    {
        for (let fi = 0; fi < files.length; fi++) {
            ...
            let fileDetails = result._details ?? [];
            ...
            if (aiOrchestrator && fileDetails.length > 0) {
                try {
                    const aiResult = await aiOrchestrator.orchestrateVerify(file.content.slice(0, 4000), {
                    ...
                    if (aiResult.aiVerified && aiResult.falsePositivesRemoved > 0) {
                        ...
                        for (const refined of aiResult.teams) {
                            const stage = (result.teams ?? []).find((t) => t.name === refined.name);
                            if (stage) {
                                stage.findings = refined.findings.map((f) => typeof f === 'string' ? f : f.message ?? f);
                                if (stage.score !== undefined)
                                    stage.score = refined.score;
                            }
                        }
                    }
                }
                        ...
                        if (meta) {
                            ...
                            if (meta.signal === 'suppress-fp' || meta.suppresses) {
                                ...
                                for (const fd of fileDetails) {
                                    if (fd.ruleId && suppressedIds.includes(fd.ruleId)) {
    ...
    try {
        ...
        if (baseline && !opts.showBaseline) {
            ...
            if (baselineSuppressed > 0) {
                const keptSet = new Set(result.kept.map((k) => `${k.line}:${k.message}`));
    ...
    function classifyLevel(severity, _message) {
    ...
    const overallStatus = overallVerdict.toLowerCase();
    const duration = Math.round(performance.now() - startTime);
    // ── 양품 감지 리포트 데이터 ──
    const goodPatternReport = {
        total: totalGoodPatterns,
        byQuality: Object.fromEntries(Object.entries(goodPatternsByQuality).map(([k, v]) => [k, { count: v.count, examples: v.examples }])),
    ...
    if (totalGoodPatterns > 0) {
        ...
        if (goodSuppressedFindings > 0)
            parts.push(`불량 ${goodSuppressedFindings}건 억제`);
        if (goodDowngradedFindings > 0)
            parts.push(`${goodDowngradedFindings}건 신뢰도 하향`);
    ...
    if (opts.watch) {
        ...
        watch(path, { recursive: true }, (_, filename) => {
            if (!filename || !SUPPORTED_EXTENSIONS.has((0, path_1.extname)(filename)))
                return;
            if (isRunning)
                return; // Skip if already verifying
            if (debounceTimer)
