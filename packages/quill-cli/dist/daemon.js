...
exports.formatFindingsForVSCode = formatFindingsForVSCode;
...
function formatFindingsForVSCode(teams, source = 'CS Quill') {
    const result = [];
    for (const team of teams) {
        for (const f of team.findings) {
            ...
            result.push({
                ...
                fix: finding.fix ? {
                    range: {
                        startLine: Number(finding.fix.range?.startLine ?? finding.line ?? 0),
                        endLine: Number(finding.fix.range?.endLine ?? finding.line ?? 0),
                    },
                    newText: String(finding.fix.newText ?? ''),
                } : undefined,
            });
        }
    }
...
function mapSeverity(severity, teamScore) {
    if (severity === 'P0' || severity === 'error' || severity === 'critical')
        return 'error';
    if (severity === 'P1' || severity === 'warning')
        return 'warning';
    if (severity === 'P2' || severity === 'info')
        return 'info';
    // 팀 점수 기반 폴백
...
function acceptWebSocket(req, socket) {
    ...
    const accept = (0, crypto_1.createHash)('sha1').update(key + GUID).digest('base64');
...
async function _handleMessageInner(msg, session, tracker, requestId) {
    switch (msg.type) {
        case 'ping': {
        ...
        case 'analyze_file': {
            ...
            const analyzeWithTimeout = async (fn, timeoutMs = 30000) => {
                return Promise.race([
                    fn(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Analysis timeout')), timeoutMs)),
                ]);
            };
            try {
                ...
                sendWS(session.socket, {
                    type: 'analysis_error',
                    id: requestId,
                    payload: { filePath, error: e.message },
                });
            }
            break;
        }
        ...
        case 'analyze_batch': {
            ...
            for (let i = 0; i < files.length; i++) {
                try {
                    ...
                    const findings = result.teams.flatMap(t => t.findings.map((f) => ({
                        line: typeof f === 'object' ? f.line ?? 0 : 0,
                        message: typeof f === 'object' ? f.message ?? String(f) : String(f),
                        severity: t.score < 50 ? 'error' : t.score < 80 ? 'warning' : 'info',
        ...
        case 'get_fix': {
            ...
            const { filePath, content, findingIndex, findingMessage } = msg.payload;
            try {
                ...
                sendWS(session.socket, { type: 'fix_error', id: requestId, payload: { error: e.message } });
            }
            break;
        }
        case 'explain_code': {
            // 코드 해설 요청 (VS Code 호버 확장용)
            const { content: codeSnippet, line: codeLine } = msg.payload;
            try {
                const { streamChat } = require('./core/ai-bridge');
                let explanation = '';
                await streamChat({
        ...
        case 'get_config': {
            ...
            try {
                ...
                sendWS(session.socket, { type: 'config', id: requestId, payload: { ...config, keys: config.keys?.map((k) => ({ ...k, key: '***' })) } });
            }
            catch (e) {
                sendWS(session.socket, { type: 'error', id: requestId, payload: { message: e.message } });
            }
            break;
        }
        case 'subscribe_file': {
            // Step 8: 파일 워치 구독 (데몬이 파일 변경 감지 → 자동 분석)
            const { filePath: watchPath } = msg.payload;
            try {
                ...
                const watcher = watch(watchPath, { persistent: false }, async () => {
                    try {
                        ...
                        const findings = result.teams.flatMap(t => t.findings.map((f) => ({
                            line: typeof f === 'object' ? f.line ?? 0 : 0,
                            message: typeof f === 'object' ? f.message ?? String(f) : String(f),
                            severity: t.score < 50 ? 'error' : 'warning',
                            source: `CS Quill (${t.name})`,
                        })));
...
function startDaemon(config = {}) {
                ...
                if (frame.opcode === 0x01) {
                    try {
                        ...
                        handleMessage(msg, session, tracker).catch((e) => {
                            sendWS(socket, { type: 'error', payload: { message: e.message } });
                        });
                    }
                    catch {
    ...
    const cleanupInterval = setInterval(() => {
        ...
        if (removed > 0)
            console.log(`  🧹 비활성 세션 ${removed}개 정리`);
    }, 60000);
...
// 프레임 크기 계산 헬퍼
function calculateFrameSize(buffer) {
    if (buffer.length < 2)
        return 0;
    const masked = (buffer[1] & 0x80) !== 0;
    let payloadLength = buffer[1] & 0x7f;
